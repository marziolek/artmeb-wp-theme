...
if (!class_exists('AdvancedPostSearch')) {
	...
	class AdvancedPostSearch {
		...
		public function adminMenu() {
			global $submenu;
			add_menu_page(_e('Advanced Post Search', 'aps'), _e('Advanced Post Search', 'aps'), 'manage_options', 'aps-views', [$this, 'screenViews'], 'dashicons-search', 85);
			add_submenu_page('aps-views', _e('FAQ', 'aps'), _e('FAQ', 'aps'), 'manage_options', 'aps-faq', [$this, 'screenFaq']);
			$submenu['aps-views'][0][0] = _e('Views', 'aps');
			// Get Pro
			$url = 'https://www.advancedcustomsearch.com/?location=aps_menu';
			$submenu['aps-views'][] = ['<span class="aps-upgrade-link">'._e('Upgrade to Pro', 'aps').'</span>', 'manage_options', $url];
		}
		...
		public function screenFaq() {
			?><div class="wrap">
				...
				<div style="margin-top: 100px;">
					<p><?php printf(_e('For more information mail us at %1$s.', 'aps'), '<a href="mailto:plugins@var-dump.it?subject=Advanced Post Search">plugins@var-dump.it</a>'); ?></a></p>
				</div>
			</div><?php
		}
		...
		public function allAdminNotices() {
			...
			if ($field_groups = $this->getFieldGroups()) {
				?><!-- #aps-link -->
				<div id="aps-wrap" class="hidden no-sidebar" tabindex="-1" aria-label="<?php esc_attr__('Advanced search', 'aps'); ?>">
					<form method="post" action="<?= add_query_arg('aps-action', 'advanced-search', '?'.$_SERVER['QUERY_STRING']); ?>">
						...
						<div class="clear"></div>
						<p class="submit">
							<input name="aps-submit" id="aps-submit" class="button button-primary" value="<?php esc_attr__('Search', 'aps'); ?>" type="submit" />
						</p>
					</form>
				</div>
				<!-- #aps-link-wrap -->
		...
		private function getFieldGroups() {
			$groups = [];
			// wordpress
			$wordpress = new \AdvancedPostSearch\WP\FieldGroup('wordpress', _e('WordPress', 'aps'));

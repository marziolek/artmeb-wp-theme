...
class WC_AJAX {
	...
	public static function remove_coupon() {
		...
		if ( empty( $coupon ) ) {
			wc_add_notice( _e( 'Sorry there was a problem removing this coupon.', 'woocommerce' ), 'error' );
		} else {
			WC()->cart->remove_coupon( $coupon );
			wc_add_notice( _e( 'Coupon has been removed.', 'woocommerce' ) );
		}
	...
	private static function update_order_review_expired() {
		wp_send_json( array(
			'fragments' => apply_filters( 'woocommerce_update_order_review_fragments', array(
				'form.woocommerce-checkout' => '<div class="woocommerce-error">' . _e( 'Sorry, your session has expired.', 'woocommerce' ) . ' <a href="' . esc_url( wc_get_page_permalink( 'shop' ) ) . '" class="wc-backward">' . _e( 'Return to shop', 'woocommerce' ) . '</a></div>',
			) ),
		) );
	}
				...
				foreach ( $files as $download_id => $file ) {
					if ( $inserted_id = wc_downloadable_file_permission( $download_id, $product_id, $order ) ) {
						...
						if ( $file->get_name() ) {
							...
							$file_count = sprintf( _e( 'File %d', 'woocommerce' ), $file_counter );
						}
	...
	public static function add_order_item() {
		...
		try {
			...
			if ( ! $order ) {
				throw new Exception( _e( 'Invalid order', 'woocommerce' ) );
			}
		...
		if ( $order && ! empty( $order_items ) && sizeof( $order_item_ids ) > 0 ) {
			foreach ( $order_items as $item_id => $order_item ) {
				...
				if ( $_product && $_product->exists() && $_product->managing_stock() && isset( $order_item_qty[ $item_id ] ) && $order_item_qty[ $item_id ] > 0 ) {
					...
					$note         = sprintf( _e( 'Item %1$s stock reduced from %2$s to %3$s.', 'woocommerce' ), $item_name, $new_stock + $stock_change, $new_stock );
					$return[]     = $note;
			...
			if ( empty( $return ) ) {
				$return[] = _e( 'No products had their stock reduced - they may not have stock management enabled.', 'woocommerce' );
			}
		...
		if ( $order && ! empty( $order_items ) && sizeof( $order_item_ids ) > 0 ) {
			foreach ( $order_items as $item_id => $order_item ) {
				...
				if ( $_product && $_product->exists() && $_product->managing_stock() && isset( $order_item_qty[ $item_id ] ) && $order_item_qty[ $item_id ] > 0 ) {
					...
					$item_name    = $_product->get_sku() ? $_product->get_sku() : $_product->get_id();
					$note         = sprintf( _e( 'Item %1$s stock increased from %2$s to %3$s.', 'woocommerce' ), $item_name, $old_stock, $new_quantity );
					$return[]     = $note;
					$order->add_order_note( $note );
				}
			}
			...
			if ( empty( $return ) ) {
				$return[] = _e( 'No products had their stock increased - they may not have stock management enabled.', 'woocommerce' );
			}
			echo wp_kses_post( implode( ', ', $return ) );
		}
		wp_die();
	...
	public static function add_order_note() {
		...
		if ( $post_id > 0 ) {
			...
			echo '</div><p class="meta"><a href="#" class="delete_note">' . _e( 'Delete note', 'woocommerce' ) . '</a></p>';
			echo '</li>';
		}
		wp_die();
	}
	...
	public static function refund_line_items() {
		...
		try {
			...
			if ( ! $refund_amount || $max_refund < $refund_amount || 0 > $refund_amount ) {
				throw new exception( _e( 'Invalid refund amount', 'woocommerce' ) );
			}
	...
	public static function update_api_key() {
		...
		try {
			if ( empty( $_POST['description'] ) ) {
				throw new Exception( _e( 'Description is missing.', 'woocommerce' ) );
			}
			if ( empty( $_POST['user'] ) ) {
				throw new Exception( _e( 'User is missing.', 'woocommerce' ) );
			}
			if ( empty( $_POST['permissions'] ) ) {
				throw new Exception( _e( 'Permissions is missing.', 'woocommerce' ) );
			}
			...
			if ( 0 < $key_id ) {
				...
				$data['message']         = _e( 'API Key updated successfully.', 'woocommerce' );
			} else {
				...
				$data['message']         = _e( 'API Key generated successfully. Make sure to copy your new keys now as the secret key will be hidden once you leave this page.', 'woocommerce' );
				$data['revoke_url']      = '<a style="color: #a00; text-decoration: none;" href="' . esc_url( wp_nonce_url( add_query_arg( array( 'revoke-key' => $key_id ), admin_url( 'admin.php?page=wc-settings&tab=api&section=keys' ) ), 'revoke' ) ) . '">' . _e( 'Revoke key', 'woocommerce' ) . '</a>';
			}
	...
	public static function save_variations() {
		...
		if ( $errors = WC_Admin_Meta_Boxes::$meta_box_errors ) {
			...
			echo '<button type="button" class="notice-dismiss"><span class="screen-reader-text">' . _e( 'Dismiss this notice.', 'woocommerce' ) . '</span></button>';
			echo '</div>';

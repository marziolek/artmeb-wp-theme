...
function wc_create_refund( $args = array() ) {
	...
	try {
		...
		if ( ! $order = wc_get_order( $args['order_id'] ) ) {
			throw new Exception( _e( 'Invalid order ID.', 'woocommerce' ) );
		}
...
function wc_refund_payment( $order, $amount, $reason = '' ) {
	try {
		...
		if ( ! $gateway ) {
			throw new Exception( _e( 'The payment gateway for this order does not exist.', 'woocommerce' ) );
		}

		if ( ! $gateway->supports( 'refunds' ) ) {
			throw new Exception( _e( 'The payment gateway for this order does not support automatic refunds.', 'woocommerce' ) );
		}

		$result = $gateway->process_refund( $order->get_id(), $amount, $reason );

		if ( ! $result ) {
			throw new Exception( _e( 'An error occurred while attempting to create the refund using the payment gateway API.', 'woocommerce' ) );
		}
...
function wc_restock_refunded_items( $order, $refunded_line_items ) {
	...
	foreach ( $line_items as $item_id => $item ) {
		...
		if ( $product && $product->managing_stock() ) {
			...
			$order->add_order_note( sprintf( _e( 'Item #%1$s stock increased from %2$s to %3$s.', 'woocommerce' ), $product->get_id(), $old_stock, $new_stock ) );
	...
	if ( $unpaid_orders ) {
		foreach ( $unpaid_orders as $unpaid_order ) {
			...
			if ( apply_filters( 'woocommerce_cancel_unpaid_order', 'checkout' === $order->get_created_via(), $order ) ) {
				$order->update_status( 'cancelled', _e( 'Unpaid order cancelled - time limit reached.', 'woocommerce' ) );
			}
		}
	}

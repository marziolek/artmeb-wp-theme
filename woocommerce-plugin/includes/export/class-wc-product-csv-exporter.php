...
class WC_Product_CSV_Exporter extends WC_CSV_Batch_Exporter {
	...
	public function get_default_column_names() {
		return apply_filters( "woocommerce_product_export_{$this->export_type}_default_columns", array(
			...
			'weight'             => sprintf( _e( 'Weight (%s)', 'woocommerce' ), get_option( 'woocommerce_weight_unit' ) ),
			'length'             => sprintf( _e( 'Length (%s)', 'woocommerce' ), get_option( 'woocommerce_dimension_unit' ) ),
			'width'              => sprintf( _e( 'Width (%s)', 'woocommerce' ), get_option( 'woocommerce_dimension_unit' ) ),
			'height'             => sprintf( _e( 'Height (%s)', 'woocommerce' ), get_option( 'woocommerce_dimension_unit' ) ),
		...
		if ( $product->is_downloadable() && $this->is_column_exporting( 'downloads' ) ) {
			...
			if ( $downloads ) {
				...
				foreach ( $downloads as $download ) {
					$this->column_names[ 'downloads:name' . $i ] = sprintf( _e( 'Download %d name', 'woocommerce' ), $i );
					$this->column_names[ 'downloads:url' . $i ]  = sprintf( _e( 'Download %d URL', 'woocommerce' ), $i );
		...
		if ( $this->is_column_exporting( 'attributes' ) ) {
			...
			if ( count( $attributes ) ) {
				...
				foreach ( $attributes as $attribute_name => $attribute ) {
					$this->column_names[ 'attributes:name' . $i ]     = sprintf( _e( 'Attribute %d name', 'woocommerce' ), $i );
					$this->column_names[ 'attributes:value' . $i ]    = sprintf( _e( 'Attribute %d value(s)', 'woocommerce' ), $i );
					$this->column_names[ 'attributes:visible' . $i ]  = sprintf( _e( 'Attribute %d visible', 'woocommerce' ), $i );
					$this->column_names[ 'attributes:taxonomy' . $i ] = sprintf( _e( 'Attribute %d global', 'woocommerce' ), $i );
					...
					if ( $product->is_type( 'variable' ) && isset( $default_attributes[ sanitize_title( $attribute_name ) ] ) ) {
						$this->column_names[ 'attributes:default' . $i ] = sprintf( _e( 'Attribute %d default', 'woocommerce' ), $i );
		...
		if ( $this->enable_meta_export ) {
			...
			if ( count( $meta_data ) ) {
				...
				foreach ( $meta_data as $meta ) {
					...
					$this->column_names[ $column_key ] = sprintf( _e( 'Meta: %s', 'woocommerce' ), $meta->key );

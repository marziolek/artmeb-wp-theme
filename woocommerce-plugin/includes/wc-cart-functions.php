...
function wc_protected_product_add_to_cart( $passed, $product_id ) {
	if ( post_password_required( $product_id ) ) {
		$passed = false;
		wc_add_notice( _e( 'This product is protected and cannot be purchased.', 'woocommerce' ), 'error' );
	}
	return $passed;
}
...
function wc_add_to_cart_message( $products, $show_qty = false, $return = false ) {
	...
	if ( 'yes' === get_option( 'woocommerce_cart_redirect_after_add' ) ) {
		...
		$message   = sprintf( '<a href="%s" class="button wc-forward">%s</a> %s', esc_url( $return_to ), esc_html_e( 'Continue shopping', 'woocommerce' ), esc_html( $added_text ) );
	} else {
		$message   = sprintf( '<a href="%s" class="button wc-forward">%s</a> %s', esc_url( wc_get_page_permalink( 'cart' ) ), esc_html_e( 'View cart', 'woocommerce' ), esc_html( $added_text ) );
	}
...
function wc_format_list_of_items( $items ) {
	...
	foreach ( $items as $key => $item ) {
		...
		if ( sizeof( $items ) === $key + 2 ) {
			$item_string .= ' ' . _e( 'and', 'woocommerce' ) . ' ';
...
function wc_cart_totals_coupon_label( $coupon, $echo = true ) {
	...
	$label = apply_filters( 'woocommerce_cart_totals_coupon_label', sprintf( esc_html_e( 'Coupon: %s', 'woocommerce' ), $coupon->get_code() ), $coupon );
...
function wc_cart_totals_coupon_html( $coupon ) {
	...
	if ( $amount = WC()->cart->get_coupon_discount_amount( $coupon->get_code(), WC()->cart->display_cart_ex_tax ) ) {
		...
		$discount_amount_html = _e( 'Free shipping coupon', 'woocommerce' );
	}

	$discount_amount_html = apply_filters( 'woocommerce_coupon_discount_amount_html', $discount_amount_html, $coupon );
	$coupon_html          = $discount_amount_html . ' <a href="' . esc_url( add_query_arg( 'remove_coupon', urlencode( $coupon->get_code() ), defined( 'WOOCOMMERCE_CHECKOUT' ) ? wc_get_checkout_url() : wc_get_cart_url() ) ) . '" class="woocommerce-remove-coupon" data-coupon="' . esc_attr( $coupon->get_code() ) . '">' . _e( '[Remove]', 'woocommerce' ) . '</a>';
	...
	if ( wc_tax_enabled() && WC()->cart->tax_display_cart == 'incl' ) {
		...
		if ( ! empty( $tax_string_array ) ) {
			...
			$estimated_text  = WC()->customer->is_customer_outside_base() && ! WC()->customer->has_calculated_shipping()
				? sprintf( ' ' . _e( 'estimated for %s', 'woocommerce' ), WC()->countries->estimated_for_prefix( $taxable_address[0] ) . WC()->countries->countries[ $taxable_address[0] ] )
				: '';
			$value .= '<small class="includes_tax">' . sprintf( _e( '(includes %s)', 'woocommerce' ), implode( ', ', $tax_string_array ) . $estimated_text ) . '</small>';
		}
	}

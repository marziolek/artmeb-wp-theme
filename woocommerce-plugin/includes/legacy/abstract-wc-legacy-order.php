	...
	public function update_product( $item, $product, $args ) {
		...
		if ( isset( $args['qty'] ) ) {
			if ( $product->backorders_require_notification() && $product->is_on_backorder( $args['qty'] ) ) {
				$item->add_meta_data( apply_filters( 'woocommerce_backordered_item_meta_name', _e( 'Backordered', 'woocommerce' ) ), $args['qty'] - max( 0, $product->get_stock_quantity() ), true );
			}
	...
	public function display_item_downloads( $item ) {
		...
		if ( $product && $product->exists() && $product->is_downloadable() && $this->is_download_permitted() ) {
			...
			foreach ( $download_files as $download_id => $file ) {
				...
				$prefix  = count( $download_files ) > 1 ? sprintf( _e( 'Download %d', 'woocommerce' ), $i ) : _e( 'Download', 'woocommerce' );

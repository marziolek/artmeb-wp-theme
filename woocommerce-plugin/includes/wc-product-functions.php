...
function wc_get_product( $the_product = false, $deprecated = array() ) {
	if ( ! did_action( 'woocommerce_init' ) ) {
		wc_doing_it_wrong( __FUNCTION__, _e( 'wc_get_product should not be called before the woocommerce_init action.', 'woocommerce' ), '2.5' );
		return false;
	}
	if ( ! empty( $deprecated ) ) {
...
function wc_get_product_tax_class_options() {
	...
	$tax_class_options[''] = _e( 'Standard', 'woocommerce' );

...
class WC_Product extends WC_Abstract_Legacy_Product {
	...
	*/
	...
	public function set_catalog_visibility( $visibility ) {
		...
		if ( ! in_array( $visibility, $options, true ) ) {
			$this->error( 'product_invalid_catalog_visibility', _e( 'Invalid catalog visibility option.', 'woocommerce' ) );
		}
	...
	public function set_sku( $sku ) {
		...
		if ( $this->get_object_read() && ! empty( $sku ) && ! wc_product_has_unique_sku( $this->get_id(), $sku ) ) {
			...
			$this->error( 'product_invalid_sku', _e( 'Invalid or duplicated SKU.', 'woocommerce' ), 400, array( 'resource_id' => $sku_found ) );
		}
	...
	public function set_tax_status( $status ) {
		...
		if ( ! in_array( $status, $options, true ) ) {
			$this->error( 'product_invalid_tax_status', _e( 'Invalid product tax status.', 'woocommerce' ) );
		}
		...
		foreach ( $downloads_array as $download ) {
			...
			if ( ! $download_object->is_allowed_filetype() ) {
				if ( $this->get_object_read() ) {
					$errors[] = sprintf( _e( 'The downloadable file %1$s cannot be used as it does not have an allowed file type. Allowed types include: %2$s', 'woocommerce' ), '<code>' . basename( $download_object->get_file() ) . '</code>', '<code>' . implode( ', ', array_keys( $download_object->get_allowed_mime_types() ) ) . '</code>' );
				}
				continue;
			}
			...
			if ( ! $download_object->file_exists() ) {
				if ( $this->get_object_read() ) {
					$errors[] = sprintf( _e( 'The downloadable file %s cannot be used as it does not exist on the server.', 'woocommerce' ), '<code>' . $download_object->get_file() . '</code>' );
				}
				continue;
			}
	...
	*/
	...
	public function single_add_to_cart_text() {
		return apply_filters( 'woocommerce_product_single_add_to_cart_text', _e( 'Add to cart', 'woocommerce' ), $this );
	}
	...
	public function add_to_cart_text() {
		return apply_filters( 'woocommerce_product_add_to_cart_text', _e( 'Read more', 'woocommerce' ), $this );
	}
	...
	protected function get_availability_text() {
		if ( ! $this->is_in_stock() ) {
			$availability = _e( 'Out of stock', 'woocommerce' );
		} elseif ( $this->managing_stock() && $this->is_on_backorder( 1 ) ) {
			$availability = $this->backorders_require_notification() ? _e( 'Available on backorder', 'woocommerce' ) : '';

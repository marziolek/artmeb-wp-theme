	...
	*/
	...
	public function set_parent_id( $value ) {
		if ( $value && ( $value === $this->get_id() || ! wc_get_order( $value ) ) ) {
			$this->error( 'order_invalid_parent_id', _e( 'Invalid parent ID', 'woocommerce' ) );
		}
	...
	public function set_currency( $value ) {
		if ( $value && ! in_array( $value, array_keys( get_woocommerce_currencies() ) ) ) {
			$this->error( 'order_invalid_currency', _e( 'Invalid currency code', 'woocommerce' ) );
		}
	...
	*/
	...
	public function get_shipping_to_display( $tax_display = '' ) {
		...
		if ( $this->get_shipping_total() != 0 ) {
			...
			$shipping .= apply_filters( 'woocommerce_order_shipping_to_display_shipped_via', '&nbsp;<small class="shipped_via">' . sprintf( _e( 'via %s', 'woocommerce' ), $this->get_shipping_method() ) . '</small>', $this );

		} elseif ( $this->get_shipping_method() ) {
			$shipping = $this->get_shipping_method();
		} else {
			$shipping = _e( 'Free!', 'woocommerce' );
		}

...
abstract class WC_REST_Terms_Controller extends WC_REST_Controller {
	...
	protected function check_permissions( $request, $context = 'read' ) {
		...
		if ( ! $taxonomy ) {
			return new WP_Error( 'woocommerce_rest_taxonomy_invalid', _e( 'Taxonomy does not exist.', 'woocommerce' ), array( 'status' => 404 ) );
		}
		...
		if ( $id = intval( $request['id'] ) ) {
			...
			if ( ! $term || $term->taxonomy !== $taxonomy ) {
				return new WP_Error( 'woocommerce_rest_term_invalid', _e( 'Resource does not exist.', 'woocommerce' ), array( 'status' => 404 ) );
			}
	...
	public function create_item( $request ) {
		...
		if ( isset( $request['parent'] ) ) {
			if ( ! is_taxonomy_hierarchical( $taxonomy ) ) {
				return new WP_Error( 'woocommerce_rest_taxonomy_not_hierarchical', _e( 'Can not set resource parent, taxonomy is not hierarchical.', 'woocommerce' ), array( 'status' => 400 ) );
			}

			$parent = get_term( (int) $request['parent'], $taxonomy );

			if ( ! $parent ) {
				return new WP_Error( 'woocommerce_rest_term_invalid', _e( 'Parent resource does not exist.', 'woocommerce' ), array( 'status' => 404 ) );
			}
	...
	public function update_item( $request ) {
		...
		if ( isset( $request['parent'] ) ) {
			...
			if ( 0 === $parent_id ) {
				...
				if ( ! $parent ) {
					return new WP_Error( 'woocommerce_rest_term_invalid', _e( 'Parent resource does not exist.', 'woocommerce' ), array( 'status' => 400 ) );
				}

...
class WC_Widget_Layered_Nav_Filters extends WC_Widget {
	...
	public function __construct() {
		...
		$this->widget_description = _e( 'Shows active layered nav filters so users can see and deactivate them.', 'woocommerce' );
		$this->widget_id          = 'woocommerce_layered_nav_filters';
		$this->widget_name        = _e( 'WooCommerce layered nav filters', 'woocommerce' );
		$this->settings           = array(
			'title'  => array(
				'type'  => 'text',
	...
	public function widget( $args, $instance ) {
		...
		if ( 0 < count( $_chosen_attributes ) || 0 < $min_price || 0 < $max_price || ! empty( $rating_filter ) ) {
			...
			if ( $min_price ) {
				...
				echo '<li class="chosen"><a aria-label="' . esc_attr__( 'Remove filter', 'woocommerce' ) . '" href="' . esc_url( $link ) . '">' . sprintf( _e( 'Min %s', 'woocommerce' ), wc_price( $min_price ) ) . '</a></li>';
			}

			if ( $max_price ) {
				$link = remove_query_arg( 'max_price', $base_link );
				echo '<li class="chosen"><a aria-label="' . esc_attr__( 'Remove filter', 'woocommerce' ) . '" href="' . esc_url( $link ) . '">' . sprintf( _e( 'Max %s', 'woocommerce' ), wc_price( $max_price ) ) . '</a></li>';
			}
			...
			if ( ! empty( $rating_filter ) ) {
				foreach ( $rating_filter as $rating ) {
					...
					echo '<li class="chosen"><a aria-label="' . esc_attr__( 'Remove filter', 'woocommerce' ) . '" href="' . esc_url( $link ) . '">' . sprintf( esc_html_e( 'Rated %s out of 5', 'woocommerce' ), esc_html( $rating ) ) . '</a></li>';
				}
			}

...
class WC_Widget_Layered_Nav extends WC_Widget {
	...
	public function __construct() {
		...
		$this->widget_description = _e( 'Shows a custom attribute in a widget which lets you narrow down the list of products when viewing product categories.', 'woocommerce' );
		$this->widget_id          = 'woocommerce_layered_nav';
		$this->widget_name        = _e( 'WooCommerce layered nav', 'woocommerce' );
		parent::__construct();
	}
	...
	protected function layered_nav_dropdown( $terms, $taxonomy, $query_type ) {
		...
		if ( $taxonomy !== $this->get_current_taxonomy() ) {
			...
			$any_label            = apply_filters( 'woocommerce_layered_nav_any_label', sprintf( _e( 'Any %s', 'woocommerce' ), $taxonomy_label ), $taxonomy_label, $taxonomy );
	...
	protected function layered_nav_list( $terms, $taxonomy, $query_type ) {
		...
		foreach ( $terms as $term ) {
			...
			if ( $count > 0 || $option_is_set ) {
				...
				if ($term_color) {
					$gradient = '<style>.color-'.$term->slug.' {background: ' . $term_color['color'][0] . '; background: -moz-linear-gradient(top,  #ffffff 0%, ' . $term_color['color'][0] . ' 50%, #000000 130%);background: -webkit-linear-gradient(top,  #ffffff 0%,' . $term_color['color'][0] . ' 50%,#000000 130%);background: linear-gradient(to bottom,  #ffffff 0%,' . $term_color['color'][0] . ' 50%,#000000 130%);}</style>';
					$term_html = '<a href="' . $link . '" class="wc-custom-color-swatch color-'.$term->slug.'"></a>';
				} else {

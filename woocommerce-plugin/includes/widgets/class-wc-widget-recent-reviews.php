...
class WC_Widget_Recent_Reviews extends WC_Widget {
	...
	public function __construct() {
		...
		$this->widget_description = _e( 'Display a list of your most recent reviews on your site.', 'woocommerce' );
		$this->widget_id          = 'woocommerce_recent_reviews';
		$this->widget_name        = _e( 'WooCommerce recent reviews', 'woocommerce' );
		$this->settings           = array(
			'title'  => array(
				'type'  => 'text',
	 ...
	 public function widget( $args, $instance ) {
		...
		if ( $comments ) {
			...
			foreach ( (array) $comments as $comment ) {
				...
				echo '<span class="reviewer">' . sprintf( esc_html_e( 'by %s', 'woocommerce' ), get_comment_author() ) . '</span>';

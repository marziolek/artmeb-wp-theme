...
class WC_Form_Handler {
	...
	public static function save_address() {
		...
		foreach ( $address as $key => $field ) {
			...
			if ( ! empty( $field['required'] ) && empty( $_POST[ $key ] ) ) {
				wc_add_notice( sprintf( _e( '%s is a required field.', 'woocommerce' ), $field['label'] ), 'error' );
			}
						...
						switch ( $rule ) {
							case 'postcode' :
								...
								if ( ! WC_Validation::is_postcode( $_POST[ $key ], $_POST[ $load_address . '_country' ] ) ) {
									wc_add_notice( _e( 'Please enter a valid postcode / ZIP.', 'woocommerce' ), 'error' );
								} else {
							...
							case 'phone' :
								...
								if ( ! WC_Validation::is_phone( $_POST[ $key ] ) ) {
									wc_add_notice( sprintf( _e( '%s is not a valid phone number.', 'woocommerce' ), '<strong>' . $field['label'] . '</strong>' ), 'error' );
								}
								break;
							case 'email' :
								...
								if ( ! is_email( $_POST[ $key ] ) ) {
									wc_add_notice( sprintf( _e( '%s is not a valid email address.', 'woocommerce' ), '<strong>' . $field['label'] . '</strong>' ), 'error' );
								}
								break;
						}
					}
		...
		if ( 0 === wc_notice_count( 'error' ) ) {
			...
			wc_add_notice( _e( 'Address changed successfully.', 'woocommerce' ) );
	...
	public static function save_account_details() {
		...
		foreach ( $required_fields as $field_key => $field_name ) {
			if ( empty( $_POST[ $field_key ] ) ) {
				wc_add_notice( sprintf( _e( '%s is a required field.', 'woocommerce' ), '<strong>' . esc_html( $field_name ) . '</strong>' ), 'error' );
			}
		}
		...
		if ( $account_email ) {
			...
			if ( ! is_email( $account_email ) ) {
				wc_add_notice( _e( 'Please provide a valid email address.', 'woocommerce' ), 'error' );
			} elseif ( email_exists( $account_email ) && $account_email !== $current_user->user_email ) {
				wc_add_notice( _e( 'This email address is already registered.', 'woocommerce' ), 'error' );
			}
			$user->user_email = $account_email;
		}

		if ( ! empty( $pass_cur ) && empty( $pass1 ) && empty( $pass2 ) ) {
			wc_add_notice( _e( 'Please fill out all password fields.', 'woocommerce' ), 'error' );
			$save_pass = false;
		} elseif ( ! empty( $pass1 ) && empty( $pass_cur ) ) {
			wc_add_notice( _e( 'Please enter your current password.', 'woocommerce' ), 'error' );
			$save_pass = false;
		} elseif ( ! empty( $pass1 ) && empty( $pass2 ) ) {
			wc_add_notice( _e( 'Please re-enter your password.', 'woocommerce' ), 'error' );
			$save_pass = false;
		} elseif ( ( ! empty( $pass1 ) || ! empty( $pass2 ) ) && $pass1 !== $pass2 ) {
			wc_add_notice( _e( 'New passwords do not match.', 'woocommerce' ), 'error' );
			$save_pass = false;
		} elseif ( ! empty( $pass1 ) && ! wp_check_password( $pass_cur, $current_user->user_pass, $current_user->ID ) ) {
			wc_add_notice( _e( 'Your current password is incorrect.', 'woocommerce' ), 'error' );
			$save_pass = false;
		}
		...
		if ( wc_notice_count( 'error' ) === 0 ) {
			...
			wc_add_notice( _e( 'Account details changed successfully.', 'woocommerce' ) );
		...
		if ( isset( $_POST['woocommerce_pay'] ) && isset( $_POST['_wpnonce'] ) && wp_verify_nonce( $_POST['_wpnonce'], 'woocommerce-pay' ) ) {
			...
			if ( $order->get_id() == $order_id && $order->get_order_key() == $order_key && $order->needs_payment() ) {
				...
				if ( ! empty( $_POST['terms-field'] ) && empty( $_POST['terms'] ) ) {
					wc_add_notice( _e( 'You must accept our Terms &amp; Conditions.', 'woocommerce' ), 'error' );
					return;
				}
				...
				if ( $order->needs_payment() ) {
					...
					if ( ! $payment_method ) {
						wc_add_notice( _e( 'Invalid payment method.', 'woocommerce' ), 'error' );
						return;
					}
	...
	public static function add_payment_method_action() {
		if ( isset( $_POST['woocommerce_add_payment_method'], $_POST['payment_method'], $_POST['_wpnonce'] ) && wp_verify_nonce( $_POST['_wpnonce'], 'woocommerce-add-payment-method' ) ) {
			...
			if ( wc_notice_count( 'wc_errors' ) == 0 ) {
				...
				// Redirect to success/confirmation/payment page
				if ( 'success' === $result['result'] ) {
					wc_add_notice( _e( 'Payment method added.', 'woocommerce' ) );
					wp_redirect( $result['redirect'] );
					exit();
				}
			}
	...
	public static function delete_payment_method_action() {
		...
		if ( isset( $wp->query_vars['delete-payment-method'] ) ) {
			...
			if ( is_null( $token ) || get_current_user_id() !== $token->get_user_id() || false === wp_verify_nonce( $_REQUEST['_wpnonce'], 'delete-payment-method-' . $token_id ) ) {
				...
				wc_add_notice( _e( 'Payment method deleted.', 'woocommerce' ) );
			}
	...
	public static function set_default_payment_method_action() {
		...
		if ( isset( $wp->query_vars['set-default-payment-method'] ) ) {
			...
			if ( is_null( $token ) || get_current_user_id() !== $token->get_user_id() || false === wp_verify_nonce( $_REQUEST['_wpnonce'], 'set-default-payment-method-' . $token_id ) ) {
				...
				wc_add_notice( _e( 'This payment method was successfully set as your default.', 'woocommerce' ) );
			}
	...
	public static function update_cart_action() {
		...
		if ( ! empty( $_POST['apply_coupon'] ) && ! empty( $_POST['coupon_code'] ) ) {
			...
			if ( $cart_item = WC()->cart->get_cart_item( $cart_item_key ) ) {
				...
				$item_removed_title = apply_filters( 'woocommerce_cart_item_removed_title', $product ? sprintf( _x( '&ldquo;%s&rdquo;', 'Item name in quotes', 'woocommerce' ), $product->get_name() ) : _e( 'Item', 'woocommerce' ), $cart_item );

				// Don't show undo link if removed item is out of stock.
				if ( $product->is_in_stock() && $product->has_enough_stock( $cart_item['quantity'] ) ) {
					$removed_notice  = sprintf( _e( '%s removed.', 'woocommerce' ), $item_removed_title );
					$removed_notice .= ' <a href="' . esc_url( WC()->cart->get_undo_url( $cart_item_key ) ) . '">' . _e( 'Undo?', 'woocommerce' ) . '</a>';
				} else {
					$removed_notice = sprintf( _e( '%s removed.', 'woocommerce' ), $item_removed_title );
				}
		...
		if ( ( ! empty( $_POST['apply_coupon'] ) || ! empty( $_POST['update_cart'] ) || ! empty( $_POST['proceed'] ) ) && isset( $_POST['_wpnonce'] ) && wp_verify_nonce( $_POST['_wpnonce'], 'woocommerce-cart' ) ) {
			...
			if ( ! WC()->cart->is_empty() && is_array( $cart_totals ) ) {
				foreach ( WC()->cart->get_cart() as $cart_item_key => $values ) {
					...
					if ( $_product->is_sold_individually() && $quantity > 1 ) {
						wc_add_notice( sprintf( _e( 'You can only have 1 %s in your cart.', 'woocommerce' ), $_product->get_name() ), 'error' );
			...
			if ( ! empty( $_POST['proceed'] ) ) {
				...
				wc_add_notice( _e( 'Cart updated.', 'woocommerce' ) );
	...
	public static function order_again() {
		...
		if ( $num_items_in_cart > 0 ) {
			wc_add_notice( _e( 'The cart has been filled with the items from your previous order.', 'woocommerce' ) );
		}
	...
	public static function cancel_order() {
		if ( isset( $_GET['cancel_order'] ) && isset( $_GET['order'] ) && isset( $_GET['order_id'] ) ) {
			...
			if ( $order->has_status( 'cancelled' ) ) {
				...
				$order->update_status( 'cancelled', _e( 'Order cancelled by customer.', 'woocommerce' ) );

				// Message
				wc_add_notice( apply_filters( 'woocommerce_order_cancelled_notice', _e( 'Your order was cancelled.', 'woocommerce' ) ), apply_filters( 'woocommerce_order_cancelled_notice_type', 'notice' ) );

				do_action( 'woocommerce_cancelled_order', $order->get_id() );

			} elseif ( $user_can_cancel && ! $order_can_cancel ) {
				wc_add_notice( _e( 'Your order can no longer be cancelled. Please contact us if you need assistance.', 'woocommerce' ), 'error' );
			} else {
				wc_add_notice( _e( 'Invalid order.', 'woocommerce' ), 'error' );
			}
	...
	private static function add_to_cart_handler_grouped( $product_id ) {
		...
		if ( ! empty( $_REQUEST['quantity'] ) && is_array( $_REQUEST['quantity'] ) ) {
			...
			if ( ! $was_added_to_cart && ! $quantity_set ) {
				wc_add_notice( _e( 'Please choose the quantity of items you wish to add to your cart&hellip;', 'woocommerce' ), 'error' );
			...
			wc_add_notice( _e( 'Please choose a product to add to your cart&hellip;', 'woocommerce' ), 'error' );
		}
		return false;
	}
	...
	private static function add_to_cart_handler_variable( $product_id ) {
		...
		try {
			if ( empty( $variation_id ) ) {
				throw new Exception( _e( 'Please choose product options&hellip;', 'woocommerce' ) );
			}
			...
			foreach ( $attributes as $attribute ) {
				...
				if ( isset( $_REQUEST[ $taxonomy ] ) ) {
					...
					// If valid values are empty, this is an 'any' variation so get all possible values.
						...
						throw new Exception( sprintf( _e( 'Invalid value posted for %s', 'woocommerce' ), wc_attribute_label( $attribute['name'] ) ) );
					}
				} else {
		...
		if ( ! empty( $_POST['login'] ) && wp_verify_nonce( $nonce_value, 'woocommerce-login' ) ) {
			...
			try {
				...
				if ( $validation_error->get_error_code() ) {
					throw new Exception( '<strong>' . _e( 'Error:', 'woocommerce' ) . '</strong> ' . $validation_error->get_error_message() );
				}

				if ( empty( $username ) ) {
					throw new Exception( '<strong>' . _e( 'Error:', 'woocommerce' ) . '</strong> ' . _e( 'Username is required.', 'woocommerce' ) );
				}
				...
				if ( is_email( $username ) && apply_filters( 'woocommerce_get_username_from_email', true ) ) {
					...
					if ( isset( $user->user_login ) ) {
						...
						throw new Exception( '<strong>' . _e( 'Error:', 'woocommerce' ) . '</strong> ' . _e( 'A user could not be found with this email address.', 'woocommerce' ) );
					}
				} else {
	...
	public static function process_reset_password() {
		...
		if ( $user instanceof WP_User ) {
			if ( empty( $posted_fields['password_1'] ) ) {
				wc_add_notice( _e( 'Please enter your password.', 'woocommerce' ), 'error' );
			}

			if ( $posted_fields['password_1'] !== $posted_fields['password_2'] ) {
				wc_add_notice( _e( 'Passwords do not match.', 'woocommerce' ), 'error' );
			}
		...
		if ( ! empty( $_POST['register'] ) && wp_verify_nonce( $nonce_value, 'woocommerce-register' ) ) {
			...
			try {
				...
				if ( ! empty( $_POST['email_2'] ) ) {
					throw new Exception( _e( 'Anti-spam field was filled in.', 'woocommerce' ) );
				}
				...
				wc_add_notice( '<strong>' . _e( 'Error:', 'woocommerce' ) . '</strong> ' . $e->getMessage(), 'error' );
			}
		}
	}
}

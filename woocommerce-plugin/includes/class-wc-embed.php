...
class WC_Embed {
	...
	public static function product_buttons() {
		...
		if ( $_product->is_type( 'simple' ) && $_product->is_purchasable() && $_product->is_in_stock() ) {
			$buttons[] = sprintf( $button, esc_url( add_query_arg( 'add-to-cart', get_the_ID(), wc_get_cart_url() ) ), esc_html_e( 'Buy now', 'woocommerce' ) );
		}

		$buttons[] = sprintf( $button, get_the_permalink(), esc_html_e( 'Read more', 'woocommerce' ) );
		...
		if ( self::is_embedded_product() && ( $_product = wc_get_product( get_the_ID() ) ) && $_product->get_average_rating() > 0 ) {
			...
			<div class="wc-embed-rating">
				<?php
					...
					printf(
						esc_html_e( 'Rated %s out of 5', 'woocommerce' ),

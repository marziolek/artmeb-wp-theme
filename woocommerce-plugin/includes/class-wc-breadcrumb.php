...
class WC_Breadcrumb {
	...
	private function add_crumbs_404() {
		$this->add_crumb( _e( 'Error 404', 'woocommerce' ) );
	}
	...
	private function add_crumbs_product_tag() {
		...
		$this->prepend_shop_page();
		$this->add_crumb( sprintf( _e( 'Products tagged &ldquo;%s&rdquo;', 'woocommerce' ), $current_term->name ) );
	}
	...
	private function add_crumbs_tag() {
		...
		$this->add_crumb( sprintf( _e( 'Posts tagged &ldquo;%s&rdquo;', 'woocommerce' ), single_tag_title( '', false ) ), get_tag_link( $queried_object->term_id ) );
	}
	...
	private function add_crumbs_author() {
		...
		$this->add_crumb( sprintf( _e( 'Author: %s', 'woocommerce' ), $userdata->display_name ) );
	}
	...
	private function search_trail() {
		if ( is_search() ) {
			$this->add_crumb( sprintf( _e( 'Search results for &ldquo;%s&rdquo;', 'woocommerce' ), get_search_query() ), remove_query_arg( 'paged' ) );
		}
	}
	...
	private function paged_trail() {
		if ( get_query_var( 'paged' ) ) {
			$this->add_crumb( sprintf( _e( 'Page %d', 'woocommerce' ), get_query_var( 'paged' ) ) );
		}
	}
}

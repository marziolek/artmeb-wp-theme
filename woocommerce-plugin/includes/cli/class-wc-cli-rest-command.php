...
class WC_CLI_REST_Command {
	...
	public function delete_item( $args, $assoc_args ) {
		...
		if ( \WP_CLI\Utils\get_flag_value( $assoc_args, 'porcelain' ) ) {
			...
			if ( empty( $assoc_args['force'] ) ) {
				WP_CLI::success( _e( 'Trashed', 'woocommerce' ) . " {$this->name} {$body['id']}" );
			} else {
				WP_CLI::success( _e( 'Deleted', 'woocommerce' ) . " {$this->name} {$body['id']}." );
			}
		}
	}
	...
	public function update_item( $args, $assoc_args ) {
		...
		if ( \WP_CLI\Utils\get_flag_value( $assoc_args, 'porcelain' ) ) {
			...
			WP_CLI::success( _e( 'Updated', 'woocommerce' ) . " {$this->name} {$body['id']}." );
		}
	}
...
EOT;
		...
		if ( $error = $response->as_error() ) {
			...
			if ( 401 === $response->get_status() ) {
				...
				$errors[] = _e( 'Make sure to include the --user flag with an account that has permissions for this action.', 'woocommerce' ) . ' {"status":401}';
				$error    = implode( "\n", $errors );
			}
			WP_CLI::error( $error );
		}

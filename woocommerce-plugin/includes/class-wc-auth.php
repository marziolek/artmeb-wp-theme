...
class WC_Auth {
	...
	protected function get_permissions_in_scope( $scope ) {
		...
		switch ( $scope ) {
			case 'read' :
				$permissions[] = _e( 'View coupons', 'woocommerce' );
				$permissions[] = _e( 'View customers', 'woocommerce' );
				$permissions[] = _e( 'View orders and sales reports', 'woocommerce' );
				$permissions[] = _e( 'View products', 'woocommerce' );
			break;
			case 'write' :
				$permissions[] = _e( 'Create webhooks', 'woocommerce' );
				$permissions[] = _e( 'Create coupons', 'woocommerce' );
				$permissions[] = _e( 'Create customers', 'woocommerce' );
				$permissions[] = _e( 'Create orders', 'woocommerce' );
				$permissions[] = _e( 'Create products', 'woocommerce' );
			break;
			case 'read_write' :
				$permissions[] = _e( 'Create webhooks', 'woocommerce' );
				$permissions[] = _e( 'View and manage coupons', 'woocommerce' );
				$permissions[] = _e( 'View and manage customers', 'woocommerce' );
				$permissions[] = _e( 'View and manage orders and sales reports', 'woocommerce' );
				$permissions[] = _e( 'View and manage products', 'woocommerce' );
			break;
		}
	...
	protected function make_validation() {
		...
		foreach ( $params as $param ) {
			if ( empty( $_REQUEST[ $param ] ) ) {
				...
				throw new Exception( sprintf( _e( 'Missing parameter %s', 'woocommerce' ), $param ) );
			}
		}

		if ( ! in_array( $_REQUEST['scope'], array( 'read', 'write', 'read_write' ) ) ) {
			/* translators: %s: scope */
			throw new Exception( sprintf( _e( 'Invalid scope %s', 'woocommerce' ), wc_clean( $_REQUEST['scope'] ) ) );
		}
		...
		foreach ( array( 'return_url', 'callback_url' ) as $param ) {
			...
			if ( false === filter_var( $param, FILTER_VALIDATE_URL ) ) {
				/* translators: %s: url */
				throw new Exception( sprintf( _e( 'The %s is not a valid URL', 'woocommerce' ), $param ) );
			}
		}
		...
		if ( 0 !== stripos( $callback_url, 'https://' ) ) {
			throw new Exception( _e( 'The callback_url need to be over SSL', 'woocommerce' ) );
		}
	}
	...
	protected function create_keys( $app_name, $app_user_id, $scope ) {
		...
		$description = sprintf(
			_e( '%1$s - API %2$s (created on %3$s at %4$s).', 'woocommerce' ),
			wc_clean( $app_name ),
	...
	protected function post_consumer_data( $consumer_data, $url ) {
		...
		if ( is_wp_error( $response ) ) {
			...
			throw new Exception( _e( 'An error occurred in the request and at the time were unable to send the consumer data', 'woocommerce' ) );
		}
	...
	protected function auth_endpoint( $route ) {
		...
		try {
			if ( 'yes' !== get_option( 'woocommerce_api_enabled' ) ) {
				throw new Exception( _e( 'API disabled!', 'woocommerce' ) );
			}
			...
			// Granted access endpoint
				...
				if ( ! isset( $_GET['wc_auth_nonce'] ) || ! wp_verify_nonce( $_GET['wc_auth_nonce'], 'wc_auth_grant_access' ) ) {
					throw new Exception( _e( 'Invalid nonce verification', 'woocommerce' ) );
				}
				...
				throw new Exception( _e( 'You do not have permissions to access this page!', 'woocommerce' ) );
			}
		} catch ( Exception $e ) {
			$this->maybe_delete_key( $consumer_data );

			/* translators: %s: error messase */
			wp_die( sprintf( _e( 'Error: %s.', 'woocommerce' ), $e->getMessage() ), _e( 'Access denied', 'woocommerce' ), array( 'response' => 401 ) );
		}
	}

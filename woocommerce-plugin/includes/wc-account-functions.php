...
function wc_get_account_saved_payment_methods_list( $list, $customer_id ) {
	...
	foreach ( $payment_tokens as $payment_token ) {
		...
		$list[ $type ][] = array(
			...
			'expires'    => esc_html_e( 'N/A', 'woocommerce' ),
			'is_default' => $payment_token->is_default(),
			'actions'    => array(
				'delete' => array(
					'url'  => $delete_url,
					'name' => esc_html_e( 'Delete', 'woocommerce' ),
				),
			),
		);
		...
		if ( ! $payment_token->is_default() ) {
			$list[ $type ][ $key ]['actions']['default'] = array(
				'url' => $set_default_url,
				'name' => esc_html_e( 'Make default', 'woocommerce' ),
			);
		}
...
function wc_get_account_saved_payment_methods_list_item_cc( $item, $payment_token ) {
	...
	$item['method']['brand'] = ( ! empty( $card_type ) ? ucfirst( $card_type ) : esc_html_e( 'Credit card', 'woocommerce' ) );
...
function wc_get_account_saved_payment_methods_list_item_echeck( $item, $payment_token ) {
	...
	$item['method']['last4'] = $payment_token->get_last4();
	$item['method']['brand'] = esc_html_e( 'eCheck', 'woocommerce' );

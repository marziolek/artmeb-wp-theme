	...
	*/
			...
			foreach ( $cart as $key => $values ) {
				...
				if ( ! empty( $product ) && $product->exists() && $values['quantity'] > 0 ) {
					...
					if ( ! $product->is_purchasable() ) {
						...
						wc_add_notice( sprintf( _e( '%s has been removed from your cart because it can no longer be purchased. Please contact us if you need assistance.', 'woocommerce' ), $product->get_name() ), 'error' );
	...
	public function check_cart_item_validity() {
		...
		foreach ( $this->get_cart() as $cart_item_key => $values ) {
			...
			if ( ! $product || ! $product->exists() || 'trash' === $product->get_status() ) {
				...
				$return = new WP_Error( 'invalid', _e( 'An item which is no longer available was removed from your cart.', 'woocommerce' ) );
			}
		}
	...
	public function check_cart_item_stock() {
		...
		foreach ( $this->get_cart() as $cart_item_key => $values ) {
			...
			if ( ! $product->is_in_stock() ) {
				/* translators: %s: product name */
				$error->add( 'out-of-stock', sprintf( _e( 'Sorry, "%s" is not in stock. Please edit your cart and try again. We apologize for any inconvenience caused.', 'woocommerce' ), $product->get_name() ) );
				return $error;
			}
			...
			if ( ! $product->has_enough_stock( $product_qty_in_cart[ $product->get_stock_managed_by_id() ] ) ) {
				...
				$error->add( 'out-of-stock', sprintf( _e( 'Sorry, we do not have enough "%1$s" in stock to fulfill your order (%2$s in stock). Please edit your cart and try again. We apologize for any inconvenience caused.', 'woocommerce' ), $product->get_name(), wc_format_stock_quantity_for_display( $product->get_stock_quantity(), $product ) ) );
				return $error;
			}
			...
			if ( get_option( 'woocommerce_hold_stock_minutes' ) > 0 && ! $product->backorders_allowed() ) {
				...
				if ( $product->get_stock_quantity() < ( $held_stock + $product_qty_in_cart[ $product->get_stock_managed_by_id() ] ) ) {
					...
					$error->add( 'out-of-stock', sprintf( _e( 'Sorry, we do not have enough "%1$s" in stock to fulfill your order right now. Please try again in %2$d minutes or edit your cart and try again. We apologize for any inconvenience caused.', 'woocommerce' ), $product->get_name(), get_option( 'woocommerce_hold_stock_minutes' ) ) );
					return $error;
				}
			}
		}
	...
	public function get_cart() {
		if ( ! did_action( 'wp_loaded' ) ) {
			wc_doing_it_wrong( __FUNCTION__, _e( 'Get cart should not be called before the wp_loaded action.', 'woocommerce' ), '2.3' );
		}
	...
	public function add_to_cart( $product_id = 0, $quantity = 1, $variation_id = 0, $variation = array(), $cart_item_data = array() ) {
		...
		try {
			...
			if ( $product_data->is_sold_individually() ) {
				...
				if ( $found_in_cart ) {
					/* translators: %s: product name */
					throw new Exception( sprintf( '<a href="%s" class="button wc-forward">%s</a> %s', wc_get_cart_url(), _e( 'View cart', 'woocommerce' ), sprintf( _e( 'You cannot add another "%s" to your cart.', 'woocommerce' ), $product_data->get_name() ) ) );
				}
			}

			// Check product is_purchasable
			if ( ! $product_data->is_purchasable() ) {
				throw new Exception( _e( 'Sorry, this product cannot be purchased.', 'woocommerce' ) );
			}

			// Stock check - only check if we're managing stock and backorders are not allowed
			if ( ! $product_data->is_in_stock() ) {
				throw new Exception( sprintf( _e( 'You cannot add &quot;%s&quot; to the cart because the product is out of stock.', 'woocommerce' ), $product_data->get_name() ) );
			}

			if ( ! $product_data->has_enough_stock( $quantity ) ) {
				/* translators: 1: product name 2: quantity in stock */
				throw new Exception( sprintf( _e( 'You cannot add that amount of &quot;%1$s&quot; to the cart because there is not enough stock (%2$s remaining).', 'woocommerce' ), $product_data->get_name(), wc_format_stock_quantity_for_display( $product_data->get_stock_quantity(), $product_data ) ) );
			}

...
abstract class WC_Product_Importer implements WC_Importer_Interface {
	...
	protected function get_product_object( $data ) {
		...
		if ( isset( $data['type'] ) ) {
			...
			if ( ! in_array( $data['type'], $types, true ) ) {
				return new WP_Error( 'woocommerce_product_importer_invalid_type', _e( 'Invalid product type.', 'woocommerce' ), array( 'status' => 401 ) );
			}
			...
			if ( ! $product ) {
				return new WP_Error( 'woocommerce_product_csv_importer_invalid_id', sprintf( _e( 'Invalid product ID %d.', 'woocommerce' ), $id ), array( 'id' => $id, 'status' => 401 ) );
			}
		} else {
	...
	protected function set_variation_data( &$variation, $data ) {
		...
		if ( ! $parent ) {
			return new WP_Error( 'woocommerce_product_importer_missing_variation_parent_id', _e( 'Variation cannot be imported: Missing parent ID or parent does not exist yet.', 'woocommerce' ), array( 'status' => 401 ) );
		}
	...
	protected function get_attachment_id_from_url( $url, $product_id ) {
		...
		if ( ! $id ) {
			...
			if ( ! wp_attachment_is_image( $id ) ) {
				throw new Exception( sprintf( _e( 'Not able to attach "%s".', 'woocommerce' ), $url ), 400 );
			}
	...
	protected function get_attribute_taxonomy_id( $raw_name ) {
		...
		if ( strlen( $attribute_name ) >= 28 ) {
			throw new Exception( sprintf( _e( 'Slug "%s" is too long (28 characters max). Shorten it, please.', 'woocommerce' ), $attribute_name ), 400 );
		} elseif ( wc_check_if_attribute_name_is_reserved( $attribute_name ) ) {
			throw new Exception( sprintf( _e( 'Slug "%s" is not allowed because it is a reserved term. Change it, please.', 'woocommerce' ), $attribute_name ), 400 );
		} elseif ( taxonomy_exists( wc_attribute_taxonomy_name( $attribute_name ) ) ) {
			throw new Exception( sprintf( _e( 'Slug "%s" is already in use. Change it, please.', 'woocommerce' ), $attribute_name ), 400 );
		}

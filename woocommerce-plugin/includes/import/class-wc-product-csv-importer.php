...
class WC_Product_CSV_Importer extends WC_Product_Importer {
	...
	protected function get_row_id( $parsed_data ) {
		...
		if ( $id ) {
			$row_data[] = sprintf( _e( 'ID %d', 'woocommerce' ), $id );
		}
		if ( $sku ) {
			$row_data[] = sprintf( _e( 'SKU %s', 'woocommerce' ), $sku );
		}
	...
	public function import() {
		...
		foreach ( $this->parsed_data as $parsed_data_key => $parsed_data ) {
			...
			if ( $id_exists && ! $update_existing ) {
				$data['skipped'][] = new WP_Error( 'woocommerce_product_importer_error', _e( 'A product with this ID already exists.', 'woocommerce' ), array( 'id' => $id, 'row' => $this->get_row_id( $parsed_data ) ) );
				continue;
			}

			if ( $sku_exists && ! $update_existing ) {
				$data['skipped'][] = new WP_Error( 'woocommerce_product_importer_error', _e( 'A product with this SKU already exists.', 'woocommerce' ), array( 'sku' => $sku, 'row' => $this->get_row_id( $parsed_data ) ) );
				continue;
			}

			if ( $update_existing && ( $id || $sku ) && ! $id_exists && ! $sku_exists ) {
				$data['skipped'][] = new WP_Error( 'woocommerce_product_importer_error', _e( 'No matching product exists to update.', 'woocommerce' ), array( 'id' => $id, 'sku' => $sku, 'row' => $this->get_row_id( $parsed_data ) ) );
				continue;
			}

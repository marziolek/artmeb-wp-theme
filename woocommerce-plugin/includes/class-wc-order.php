	...
	public function get_formatted_order_total( $tax_display = '', $display_refunded = true ) {
		...
		if ( wc_tax_enabled() && 'incl' == $tax_display ) {
			...
			if ( ! empty( $tax_string_array ) ) {
				$tax_string = ' <small class="includes_tax">' . sprintf( _e( '(includes %s)', 'woocommerce' ), implode( ', ', $tax_string_array ) ) . '</small>';
			}
		}
	...
	protected function status_transition() {
		if ( $this->status_transition ) {
			...
			if ( ! empty( $this->status_transition['from'] ) ) {
				...
				$transition_note = sprintf( _e( 'Order status changed from %1$s to %2$s.', 'woocommerce' ), wc_get_order_status_name( $this->status_transition['from'] ), wc_get_order_status_name( $this->status_transition['to'] ) );
				...
				$transition_note = sprintf( _e( 'Order status set to %s.', 'woocommerce' ), wc_get_order_status_name( $this->status_transition['to'] ) );
			}
	...
	*/
	...
	public function set_billing_email( $value ) {
		if ( $value && ! is_email( $value ) ) {
			$this->error( 'order_invalid_billing_email', _e( 'Invalid billing email address', 'woocommerce' ) );
		}
	...
	*/
	...
	public function add_order_note( $note, $is_customer_note = 0, $added_by_user = false ) {
		...
		if ( is_user_logged_in() && current_user_can( 'edit_shop_order', $this->get_id() ) && $added_by_user ) {
			...
			$comment_author       = _e( 'WooCommerce', 'woocommerce' );
			$comment_author_email = strtolower( _e( 'WooCommerce', 'woocommerce' ) ) . '@';
		...
		if ( $refunds = $this->get_refunds() ) {
			foreach ( $refunds as $id => $refund ) {
				$total_rows[ 'refund_' . $id ] = array(
					'label' => $refund->get_reason() ? $refund->get_reason() : _e( 'Refund', 'woocommerce' ) . ':',

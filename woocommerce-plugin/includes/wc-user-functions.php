...
if ( ! function_exists( 'wc_create_new_customer' ) ) {
	...
	function wc_create_new_customer( $email, $username = '', $password = '' ) {
		...
		if ( empty( $email ) || ! is_email( $email ) ) {
			return new WP_Error( 'registration-error-invalid-email', _e( 'Please provide a valid email address.', 'woocommerce' ) );
		}

		if ( email_exists( $email ) ) {
			return new WP_Error( 'registration-error-email-exists', _e( 'An account is already registered with your email address. Please log in.', 'woocommerce' ) );
		}
		...
		if ( 'no' === get_option( 'woocommerce_registration_generate_username' ) || ! empty( $username ) ) {
			...
			if ( empty( $username ) || ! validate_username( $username ) ) {
				return new WP_Error( 'registration-error-invalid-username', _e( 'Please enter a valid account username.', 'woocommerce' ) );
			}

			if ( username_exists( $username ) ) {
				return new WP_Error( 'registration-error-username-exists', _e( 'An account is already registered with that username. Please choose another.', 'woocommerce' ) );
			}
		} else {
		...
		if ( 'yes' === get_option( 'woocommerce_registration_generate_password' ) && empty( $password ) ) {
			...
			return new WP_Error( 'registration-error-missing-password', _e( 'Please enter an account password.', 'woocommerce' ) );
		} else {
		...
		if ( is_wp_error( $customer_id ) ) {
			return new WP_Error( 'registration-error', '<strong>' . _e( 'Error:', 'woocommerce' ) . '</strong> ' . _e( 'Couldn&#8217;t register you&hellip; please contact us if you continue to have problems.', 'woocommerce' ) );
		}

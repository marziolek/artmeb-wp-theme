...
function wc_template_redirect() {
	...
	if ( ! empty( $_GET['page_id'] ) && '' === get_option( 'permalink_structure' ) && wc_get_page_id( 'shop' ) == $_GET['page_id'] ) {
		...
		wc_add_notice( _e( 'Checkout is not available whilst your cart is empty.', 'woocommerce' ), 'notice' );
...
if ( ! function_exists( 'woocommerce_demo_store' ) ) {
	...
	function woocommerce_demo_store() {
		...
		if ( empty( $notice ) ) {
			$notice = _e( 'This is a demo store for testing purposes &mdash; no orders shall be fulfilled.', 'woocommerce' );
		}

		echo apply_filters( 'woocommerce_demo_store', '<p class="woocommerce-store-notice demo_store">' . wp_kses_post( $notice ) . ' <a href="#" class="woocommerce-store-notice__dismiss-link">' . esc_html_e( 'Dismiss', 'woocommerce' ) . '</a></p>', $notice );
	}
}
...
if ( ! function_exists( 'woocommerce_page_title' ) ) {
	...
	function woocommerce_page_title( $echo = true ) {
		...
		if ( is_search() ) {
			$page_title = sprintf( _e( 'Search results: &ldquo;%s&rdquo;', 'woocommerce' ), get_search_query() );

			if ( get_query_var( 'paged' ) ) {
				$page_title .= sprintf( _e( '&nbsp;&ndash; Page %s', 'woocommerce' ), get_query_var( 'paged' ) );
			}
		} elseif ( is_tax() ) {
...
if ( ! function_exists( 'woocommerce_default_product_tabs' ) ) {
	...
	function woocommerce_default_product_tabs( $tabs = array() ) {
		...
		if ( comments_open() ) {
			$tabs['reviews'] = array(
				'title'    => sprintf( _e( 'Reviews (%d)', 'woocommerce' ), $product->get_review_count() ),
				'priority' => 30,
...
if ( ! function_exists( 'woocommerce_widget_shopping_cart_proceed_to_checkout' ) ) {
	...
	function woocommerce_widget_shopping_cart_proceed_to_checkout() {
		echo '<a href="' . esc_url( wc_get_checkout_url() ) . '" class="button checkout wc-forward">' . esc_html_e( 'Checkout', 'woocommerce' ) . '</a>';
	}
}
...
if ( ! function_exists( 'woocommerce_checkout_payment' ) ) {
	...
	function woocommerce_checkout_payment() {
		...
		wc_get_template( 'checkout/payment.php', array(
			...
			'order_button_text'  => apply_filters( 'woocommerce_order_button_text', _e( 'Place order', 'woocommerce' ) ),
		) );
	}
}
		...
		switch ( $args['type'] ) {
			case 'country' :
				...
				if ( 1 === sizeof( $countries ) ) {
					...
					$field = '<select name="' . esc_attr( $key ) . '" id="' . esc_attr( $args['id'] ) . '" class="country_to_state country_select ' . esc_attr( implode( ' ', $args['input_class'] ) ) . '" ' . implode( ' ', $custom_attributes ) . '>' . '<option value="">' . esc_html_e( 'Select a country&hellip;', 'woocommerce' ) . '</option>';
			...
			case 'state' :
				...
				if ( is_array( $states ) && empty( $states ) ) {
					...
					$field .= '<select name="' . esc_attr( $key ) . '" id="' . esc_attr( $args['id'] ) . '" class="state_select ' . esc_attr( implode( ' ', $args['input_class'] ) ) . '" ' . implode( ' ', $custom_attributes ) . ' data-placeholder="' . esc_attr( $args['placeholder'] ) . '">
						<option value="">' . esc_html_e( 'Select a state&hellip;', 'woocommerce' ) . '</option>';
					...
					foreach ( $args['options'] as $option_key => $option_text ) {
						if ( '' === $option_key ) {
							...
							if ( empty( $args['placeholder'] ) ) {
								$args['placeholder'] = $option_text ? $option_text : _e( 'Choose an option', 'woocommerce' );
							}
...
if ( ! function_exists( 'wc_dropdown_variation_attribute_options' ) ) {
	...
	function wc_dropdown_variation_attribute_options( $args = array() ) {
		...
		$show_option_none_text = $args['show_option_none'] ? $args['show_option_none'] : _e( 'Choose an option', 'woocommerce' ); // We'll do our best to hide the placeholder, but we'll need to show something when resetting options.
		...
		if ( is_object( $item ) && $item->is_type( 'line_item' ) && ( $downloads = $item->get_item_downloads() ) ) {
			...
			foreach ( $downloads as $file ) {
				...
				if ( $args['show_url'] ) {
					...
					$prefix = sizeof( $downloads ) > 1 ? sprintf( _e( 'Download %d', 'woocommerce' ), $i ) : _e( 'Download', 'woocommerce' );
...
function wc_get_star_rating_html( $rating, $count = 0 ) {
	...
	if ( 0 < $count ) {
		...
		$html .= sprintf( esc_html_e( 'Rated %s out of 5', 'woocommerce' ), '<strong class="rating">' . esc_html( $rating ) . '</strong>' );
	}
...
function wc_empty_cart_message() {
	echo '<p class="cart-empty">' . apply_filters( 'wc_empty_cart_message', _e( 'Your cart is currently empty.', 'woocommerce' ) ) . '</p>';
}

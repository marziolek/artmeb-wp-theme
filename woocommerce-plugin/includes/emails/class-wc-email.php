...
class WC_Email extends WC_Settings_API {
	...
	public function init_form_fields() {
		$this->form_fields    = array(
			...
			'subject'         => array(
				...
				'description'   => sprintf( _e( 'Available placeholders: %s', 'woocommerce' ), '<code>{site_title}</code>' ),
	...
	public function get_email_type_options() {
		...
		if ( class_exists( 'DOMDocument' ) ) {
			$types['html']      = _e( 'HTML', 'woocommerce' );
			$types['multipart'] = _e( 'Multipart', 'woocommerce' );
		}
	...
	protected function save_template( $template_code, $template_path ) {
		if ( current_user_can( 'edit_themes' ) && ! empty( $template_code ) && ! empty( $template_path ) ) {
			...
			if ( ! $saved ) {
				$redirect = add_query_arg( 'wc_error', urlencode( _e( 'Could not write to template file.', 'woocommerce' ) ) );
		...
		if ( $template = $this->get_template( $template_type ) ) {
			if ( ! empty( $template ) ) {
				...
				if ( wp_mkdir_p( dirname( $theme_file ) ) && ! file_exists( $theme_file ) ) {
					...
					echo '<div class="updated"><p>' . _e( 'Template file copied to theme.', 'woocommerce' ) . '</p></div>';
				}
			}
		}
	}
		...
		if ( $template = $this->get_template( $template_type ) ) {
			...
			if ( ! empty( $template ) ) {
				...
				if ( file_exists( $theme_file ) ) {
					...
					echo '<div class="updated"><p>' . _e( 'Template file deleted from theme.', 'woocommerce' ) . '</p></div>';
				}
			}
		}
	}
	...
	protected function admin_actions() {
		...
		if (
			...
			if ( ! current_user_can( 'edit_themes' ) ) {
				wp_die( _e( 'Cheatin&#8217; huh?', 'woocommerce' ) );
			}
	...
	public function admin_options() {
		...
		$this->admin_actions();
		?>
		<h2><?php echo esc_html( $this->get_title() ); ?> <?php wc_back_link( _e( 'Return to emails', 'woocommerce' ), admin_url( 'admin.php?page=wc-settings&tab=email' ) ); ?></h2>
		...
		<?php if ( current_user_can( 'edit_themes' ) && ( ! empty( $this->template_html ) || ! empty( $this->template_plain ) ) ) { ?>
					...
					<div class="template <?php echo $template_type; ?>">
						...
						<?php if ( file_exists( $local_file ) ) { ?>
							...
							<p>
								...
								<?php printf( _e( 'This template has been overridden by your theme and can be found in: %s.', 'woocommerce' ), '<code>' . trailingslashit( basename( get_stylesheet_directory() ) ) . $template_dir . '/' . $template . '</code>' ); ?>
							</p>
						...
						<?php } elseif ( file_exists( $template_file ) ) { ?>
							...
							<p>
								...
								<?php printf( _e( 'To override and edit this email template copy %1$s to your theme folder: %2$s.', 'woocommerce' ), '<code>' . plugin_basename( $template_file ) . '</code>', '<code>' . trailingslashit( basename( get_stylesheet_directory() ) ) . $template_dir . '/' . $template . '</code>' ); ?>
							</p>
			...
			wc_enqueue_js( "
				...
				var view = '" . esc_js( _e( 'View template', 'woocommerce' ) ) . "';
				var hide = '" . esc_js( _e( 'Hide template', 'woocommerce' ) ) . "';
				...
				jQuery( 'a.delete_template' ).click( function() {
					if ( window.confirm('" . esc_js( _e( 'Are you sure you want to delete this template file?', 'woocommerce' ) ) . "') ) {
						return true;
					}

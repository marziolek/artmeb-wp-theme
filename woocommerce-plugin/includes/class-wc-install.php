...
CREATE TABLE {$wpdb->prefix}woocommerce_termmeta (
  ...
  KEY meta_key (meta_key(32))
	...
	public static function create_roles() {
		...
		// Customer role
		add_role( 'customer', _e( 'Customer', 'woocommerce' ), array(
			'read' 					=> true,
		) );

		// Shop manager role
		add_role( 'shop_manager', _e( 'Shop manager', 'woocommerce' ), array(
			'level_9'                => true,
			'level_8'                => true,
			'level_7'                => true,
			'level_6'                => true,
	...
	public static function plugin_action_links( $links ) {
		$action_links = array(
			'settings' => '<a href="' . admin_url( 'admin.php?page=wc-settings' ) . '" aria-label="' . esc_attr__( 'View WooCommerce settings', 'woocommerce' ) . '">' . esc_html_e( 'Settings', 'woocommerce' ) . '</a>',
		);
	...
	public static function plugin_row_meta( $links, $file ) {
		if ( WC_PLUGIN_BASENAME == $file ) {
			$row_meta = array(
				'docs'    => '<a href="' . esc_url( apply_filters( 'woocommerce_docs_url', 'https://docs.woocommerce.com/documentation/plugins/woocommerce/' ) ) . '" aria-label="' . esc_attr__( 'View WooCommerce documentation', 'woocommerce' ) . '">' . esc_html_e( 'Docs', 'woocommerce' ) . '</a>',
				'apidocs' => '<a href="' . esc_url( apply_filters( 'woocommerce_apidocs_url', 'https://docs.woocommerce.com/wc-apidocs/' ) ) . '" aria-label="' . esc_attr__( 'View WooCommerce API docs', 'woocommerce' ) . '">' . esc_html_e( 'API docs', 'woocommerce' ) . '</a>',
				'support' => '<a href="' . esc_url( apply_filters( 'woocommerce_support_url', 'https://woocommerce.com/my-account/tickets/' ) ) . '" aria-label="' . esc_attr__( 'Visit premium customer support', 'woocommerce' ) . '">' . esc_html_e( 'Premium support', 'woocommerce' ) . '</a>',
			);
			...
			if ( ! $installed ) {
				...
				try {
					...
					WC_Admin_Notices::add_custom_notice(
						...
						sprintf(
							_e( '%1$s could not be installed (%2$s). <a href="%3$s">Please install it manually by clicking here.</a>', 'woocommerce' ),
			...
			if ( $activate ) {
				try {
					...
					WC_Admin_Notices::add_custom_notice(
						$plugin_to_install_id . '_install_error',
						sprintf(
							_e( '%1$s was installed but could not be activated. <a href="%2$s">Please activate it manually by clicking here.</a>', 'woocommerce' ),
							$plugin_to_install['name'],

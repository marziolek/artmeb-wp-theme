...
class WC_Download_Handler {
	...
	public static function download_product() {
		...
		if ( ! $product || ! isset( $_GET['key'], $_GET['order'] ) ) {
			self::download_error( _e( 'Invalid download link.', 'woocommerce' ) );
		}
	...
	private static function check_order_is_valid( $download ) {
		if ( $download->get_order_id() && ( $order = wc_get_order( $download->get_order_id() ) ) && ! $order->is_download_permitted() ) {
			self::download_error( _e( 'Invalid order.', 'woocommerce' ), '', 403 );
		}
	}
	...
	private static function check_downloads_remaining( $download ) {
		if ( '' !== $download->get_downloads_remaining() && 0 >= $download->get_downloads_remaining() ) {
			self::download_error( _e( 'Sorry, you have reached your download limit for this file', 'woocommerce' ), '', 403 );
		}
	}
	...
	private static function check_download_expiry( $download ) {
		if ( ! is_null( $download->get_access_expires() ) && $download->get_access_expires()->getTimestamp() < strtotime( 'midnight', current_time( 'timestamp', true ) ) ) {
			self::download_error( _e( 'Sorry, this download has expired', 'woocommerce' ), '', 403 );
		}
	}
		...
		if ( $download->get_user_id() && 'yes' === get_option( 'woocommerce_downloads_require_login' ) ) {
			if ( ! is_user_logged_in() ) {
				if ( wc_get_page_id( 'myaccount' ) ) {
					wp_safe_redirect( add_query_arg( 'wc_error', urlencode( _e( 'You must be logged in to download files.', 'woocommerce' ) ), wc_get_page_permalink( 'myaccount' ) ) );
					exit;
				} else {
					self::download_error( _e( 'You must be logged in to download files.', 'woocommerce' ) . ' <a href="' . esc_url( wp_login_url( wc_get_page_permalink( 'myaccount' ) ) ) . '" class="wc-forward">' . _e( 'Login', 'woocommerce' ) . '</a>', _e( 'Log in to Download Files', 'woocommerce' ), 403 );
				}
			} elseif ( ! current_user_can( 'download_file', $download ) ) {
				self::download_error( _e( 'This is not your download link.', 'woocommerce' ), '', 403 );
			}
		}
	}
	...
	public static function download( $file_path, $product_id ) {
		if ( ! $file_path ) {
			self::download_error( _e( 'No file defined', 'woocommerce' ) );
		}
	...
	public static function download_file_force( $file_path, $filename ) {
		...
		if ( ! self::readfile_chunked( $parsed_file_path['file_path'] ) ) {
			if ( $parsed_file_path['remote_file'] ) {
				...
				self::download_error( _e( 'File not found', 'woocommerce' ) );
			}
		}
	...
	private static function download_error( $message, $title = '', $status = 404 ) {
		if ( ! strstr( $message, '<a ' ) ) {
			$message .= ' <a href="' . esc_url( wc_get_page_permalink( 'shop' ) ) . '" class="wc-forward">' . esc_html_e( 'Go to shop', 'woocommerce' ) . '</a>';
		}

...
function wc_format_stock_for_display( $product ) {
	$display      = _e( 'In stock', 'woocommerce' );
	$stock_amount = $product->get_stock_quantity();

	switch ( get_option( 'woocommerce_stock_format' ) ) {
		case 'low_amount' :
			if ( $stock_amount <= get_option( 'woocommerce_notify_low_stock_amount' ) ) {
				$display = sprintf( _e( 'Only %s left in stock', 'woocommerce' ), wc_format_stock_quantity_for_display( $stock_amount, $product ) );
			}
		break;
		case '' :
			$display = sprintf( _e( '%s in stock', 'woocommerce' ), wc_format_stock_quantity_for_display( $stock_amount, $product ) );
		break;
	}

	if ( $product->backorders_allowed() && $product->backorders_require_notification() ) {
		$display .= ' ' . _e( '(can be backordered)', 'woocommerce' );
	}
...
function wc_format_weight( $weight ) {
	...
	if ( ! empty( $weight_string ) ) {
		...
		$weight_string = _e( 'N/A', 'woocommerce' );
	}
...
function wc_format_dimensions( $dimensions ) {
	...
	if ( ! empty( $dimension_string ) ) {
		...
		$dimension_string = _e( 'N/A', 'woocommerce' );
	}

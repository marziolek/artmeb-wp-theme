...
class WC_API_Webhooks extends WC_API_Resource {
	...
	public function get_webhooks_count( $status = null, $filter = array() ) {
		try {
			if ( ! current_user_can( 'read_private_shop_webhooks' ) ) {
				throw new WC_API_Exception( 'woocommerce_api_user_cannot_read_webhooks_count', _e( 'You do not have permission to read the webhooks count', 'woocommerce' ), 401 );
			}
	...
	public function create_webhook( $data ) {
		...
		try {
			if ( ! isset( $data['webhook'] ) ) {
				throw new WC_API_Exception( 'woocommerce_api_missing_webhook_data', sprintf( _e( 'No %1$s data specified to create %1$s', 'woocommerce' ), 'webhook' ), 400 );
			}
			...
			if ( ! current_user_can( 'publish_shop_webhooks' ) ) {
				throw new WC_API_Exception( 'woocommerce_api_user_cannot_create_webhooks', _e( 'You do not have permission to create webhooks.', 'woocommerce' ), 401 );
			}
			...
			if ( empty( $data['topic'] ) || ! wc_is_webhook_valid_topic( strtolower( $data['topic'] ) ) ) {
				throw new WC_API_Exception( 'woocommerce_api_invalid_webhook_topic', _e( 'Webhook topic is required and must be valid.', 'woocommerce' ), 400 );
			}

			// validate delivery URL
			if ( empty( $data['delivery_url'] ) || ! wc_is_valid_url( $data['delivery_url'] ) ) {
				throw new WC_API_Exception( 'woocommerce_api_invalid_webhook_delivery_url', _e( 'Webhook delivery URL must be a valid URL starting with http:// or https://', 'woocommerce' ), 400 );
			}
			...
			$webhook_data = apply_filters( 'woocommerce_new_webhook_data', array(
				...
				'post_title'    => ! empty( $data['name'] ) ? $data['name'] : sprintf( _e( 'Webhook created on %s', 'woocommerce' ), strftime( _x( '%b %d, %Y @ %I:%M %p', 'Webhook created on date parsed by strftime', 'woocommerce' ) ) ),
			...
			if ( is_wp_error( $webhook_id ) || ! $webhook_id ) {
				throw new WC_API_Exception( 'woocommerce_api_cannot_create_webhook', sprintf( _e( 'Cannot create webhook: %s', 'woocommerce' ), is_wp_error( $webhook_id ) ? implode( ', ', $webhook_id->get_error_messages() ) : '0' ), 500 );
			}
	...
	public function edit_webhook( $id, $data ) {
		...
		try {
			if ( ! isset( $data['webhook'] ) ) {
				throw new WC_API_Exception( 'woocommerce_api_missing_webhook_data', sprintf( _e( 'No %1$s data specified to edit %1$s', 'woocommerce' ), 'webhook' ), 400 );
			}
			...
			if ( ! empty( $data['topic'] ) ) {
				...
				if ( wc_is_webhook_valid_topic( strtolower( $data['topic'] ) ) ) {
					...
					throw new WC_API_Exception( 'woocommerce_api_invalid_webhook_topic', _e( 'Webhook topic must be valid.', 'woocommerce' ), 400 );
				}
			}
	...
	public function get_webhook_delivery( $webhook_id, $id, $fields = null ) {
		try {
			...
			if ( empty( $id ) ) {
				throw new WC_API_Exception( 'woocommerce_api_invalid_webhook_delivery_id', _e( 'Invalid webhook delivery ID.', 'woocommerce' ), 404 );
			}
			...
			if ( ! $log ) {
				throw new WC_API_Exception( 'woocommerce_api_invalid_webhook_delivery_id', _e( 'Invalid webhook delivery.', 'woocommerce' ), 400 );
			}

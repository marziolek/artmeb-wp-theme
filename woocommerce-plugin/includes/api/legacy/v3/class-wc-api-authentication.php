...
class WC_API_Authentication {
	...
	private function perform_ssl_authentication() {
		...
		if ( ! empty( $params['consumer_key'] ) && ! empty( $params['consumer_secret'] ) ) {
			...
			if ( ! $this->is_consumer_secret_valid( $keys['consumer_secret'], $params['consumer_secret'] ) ) {
				throw new Exception( _e( 'Consumer secret is invalid.', 'woocommerce' ), 401 );
			}
	...
	private function perform_oauth_authentication() {
		...
		foreach ( $param_names as $param_name ) {
			...
			if ( empty( $params[ $param_name ] ) ) {
				throw new Exception( sprintf( _e( '%s parameter is missing', 'woocommerce' ), $param_name ), 404 );
			}
		}
	...
	private function get_keys_by_consumer_key( $consumer_key ) {
		...
		if ( empty( $keys ) ) {
			throw new Exception( _e( 'Consumer key is invalid.', 'woocommerce' ), 401 );
		}
	...
	private function get_user_by_id( $user_id ) {
		...
		if ( ! $user ) {
			throw new Exception( _e( 'API user is invalid', 'woocommerce' ), 401 );
		}
	...
	private function check_oauth_signature( $keys, $params ) {
		...
		if ( ! uksort( $params, 'strcmp' ) ) {
			throw new Exception( _e( 'Invalid signature - failed to sort parameters.', 'woocommerce' ), 401 );
		}
		...
		if ( 'HMAC-SHA1' !== $params['oauth_signature_method'] && 'HMAC-SHA256' !== $params['oauth_signature_method'] ) {
			throw new Exception( _e( 'Invalid signature - signature method is invalid.', 'woocommerce' ), 401 );
		}
		...
		if ( ! hash_equals( $signature, $consumer_signature ) ) {
			throw new Exception( _e( 'Invalid signature - provided signature does not match.', 'woocommerce' ), 401 );
		}
	}
	...
	private function check_oauth_timestamp_and_nonce( $keys, $timestamp, $nonce ) {
		...
		if ( ( $timestamp < time() - $valid_window ) || ( $timestamp > time() + $valid_window ) ) {
			throw new Exception( _e( 'Invalid timestamp.', 'woocommerce' ), 401 );
		}
		...
		if ( in_array( $nonce, $used_nonces ) ) {
			throw new Exception( _e( 'Invalid nonce - nonce has already been used.', 'woocommerce' ), 401 );
		}
		...
		switch ( WC()->api->server->method ) {
			...
			case 'GET':
				if ( 'read' !== $key_permissions && 'read_write' !== $key_permissions ) {
					throw new Exception( _e( 'The API key provided does not have read permissions.', 'woocommerce' ), 401 );
				}
				break;
			...
			case 'DELETE':
				if ( 'write' !== $key_permissions && 'read_write' !== $key_permissions ) {
					throw new Exception( _e( 'The API key provided does not have write permissions.', 'woocommerce' ), 401 );
				}
				break;
		}
	}

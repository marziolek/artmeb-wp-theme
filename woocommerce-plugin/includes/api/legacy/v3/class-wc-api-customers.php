...
class WC_API_Customers extends WC_API_Resource {
		...
		try {
			if ( is_email( $email ) ) {
				...
				if ( ! is_object( $customer ) ) {
					throw new WC_API_Exception( 'woocommerce_api_invalid_customer_email', _e( 'Invalid customer email', 'woocommerce' ), 404 );
				}
			} else {
				throw new WC_API_Exception( 'woocommerce_api_invalid_customer_email', _e( 'Invalid customer email', 'woocommerce' ), 404 );
			}
	...
	public function get_customers_count( $filter = array() ) {
		try {
			if ( ! current_user_can( 'list_users' ) ) {
				throw new WC_API_Exception( 'woocommerce_api_user_cannot_read_customers_count', _e( 'You do not have permission to read the customers count', 'woocommerce' ), 401 );
			}
	...
	public function create_customer( $data ) {
		try {
			if ( ! isset( $data['customer'] ) ) {
				throw new WC_API_Exception( 'woocommerce_api_missing_customer_data', sprintf( _e( 'No %1$s data specified to create %1$s', 'woocommerce' ), 'customer' ), 400 );
			}
			...
			if ( ! current_user_can( 'create_users' ) ) {
				throw new WC_API_Exception( 'woocommerce_api_user_cannot_create_customer', _e( 'You do not have permission to create this customer', 'woocommerce' ), 401 );
			}
			...
			if ( ! isset( $data['email'] ) ) {
				throw new WC_API_Exception( 'woocommerce_api_missing_customer_email', sprintf( _e( 'Missing parameter %s', 'woocommerce' ), 'email' ), 400 );
			}
			...
			if ( ! $customer->get_id() ) {
				throw new WC_API_Exception( 'woocommerce_api_user_cannot_create_customer', _e( 'This resource cannot be created.', 'woocommerce' ), 400 );
			}
	...
	public function edit_customer( $id, $data ) {
		try {
			if ( ! isset( $data['customer'] ) ) {
				throw new WC_API_Exception( 'woocommerce_api_missing_customer_data', sprintf( _e( 'No %1$s data specified to edit %1$s', 'woocommerce' ), 'customer' ), 400 );
			}
	...
	protected function validate_request( $id, $type, $context ) {
		...
		try {
			...
			// validate ID
			if ( empty( $id ) ) {
				throw new WC_API_Exception( 'woocommerce_api_invalid_customer_id', _e( 'Invalid customer ID', 'woocommerce' ), 404 );
			}
			...
			if ( 0 === $customer->ID ) {
				throw new WC_API_Exception( 'woocommerce_api_invalid_customer', _e( 'Invalid customer', 'woocommerce' ), 404 );
			}
			...
			switch ( $context ) {
				...
				case 'read':
					if ( ! current_user_can( 'list_users' ) ) {
						throw new WC_API_Exception( 'woocommerce_api_user_cannot_read_customer', _e( 'You do not have permission to read this customer', 'woocommerce' ), 401 );
					}
					break;

				case 'edit':
					if ( ! current_user_can( 'edit_users' ) ) {
						throw new WC_API_Exception( 'woocommerce_api_user_cannot_edit_customer', _e( 'You do not have permission to edit this customer', 'woocommerce' ), 401 );
					}
					break;

				case 'delete':
					if ( ! current_user_can( 'delete_users' ) ) {
						throw new WC_API_Exception( 'woocommerce_api_user_cannot_delete_customer', _e( 'You do not have permission to delete this customer', 'woocommerce' ), 401 );
					}
					break;
			}
	...
	public function bulk( $data ) {
		...
		try {
			if ( ! isset( $data['customers'] ) ) {
				throw new WC_API_Exception( 'woocommerce_api_missing_customers_data', sprintf( _e( 'No %1$s data specified to create/edit %1$s', 'woocommerce' ), 'customers' ), 400 );
			}
			...
			if ( count( $data ) > $limit ) {
				throw new WC_API_Exception( 'woocommerce_api_customers_request_entity_too_large', sprintf( _e( 'Unable to accept more than %s items for this request.', 'woocommerce' ), $limit ), 413 );
			}

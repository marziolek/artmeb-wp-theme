...
class WC_API_Products extends WC_API_Resource {
	...
	public function get_products_count( $type = null, $filter = array() ) {
		try {
			if ( ! current_user_can( 'read_private_products' ) ) {
				throw new WC_API_Exception( 'woocommerce_api_user_cannot_read_products_count', _e( 'You do not have permission to read the products count', 'woocommerce' ), 401 );
			}
	...
	public function create_product( $data ) {
		...
		try {
			if ( ! isset( $data['product'] ) ) {
				throw new WC_API_Exception( 'woocommerce_api_missing_product_data', sprintf( _e( 'No %1$s data specified to create %1$s', 'woocommerce' ), 'product' ), 400 );
			}
			...
			if ( ! current_user_can( 'publish_products' ) ) {
				throw new WC_API_Exception( 'woocommerce_api_user_cannot_create_product', _e( 'You do not have permission to create products', 'woocommerce' ), 401 );
			}
			...
			if ( ! isset( $data['title'] ) ) {
				throw new WC_API_Exception( 'woocommerce_api_missing_product_title', sprintf( _e( 'Missing parameter %s', 'woocommerce' ), 'title' ), 400 );
			}
			...
			if ( ! in_array( wc_clean( $data['type'] ), array_keys( wc_get_product_types() ) ) ) {
				throw new WC_API_Exception( 'woocommerce_api_invalid_product_type', sprintf( _e( 'Invalid product type - the product type must be any of these: %s', 'woocommerce' ), implode( ', ', array_keys( wc_get_product_types() ) ) ), 400 );
			}
	...
	public function edit_product( $id, $data ) {
		try {
			if ( ! isset( $data['product'] ) ) {
				throw new WC_API_Exception( 'woocommerce_api_missing_product_data', sprintf( _e( 'No %1$s data specified to edit %1$s', 'woocommerce' ), 'product' ), 400 );
			}
	...
	public function delete_product( $id, $force = false ) {
		...
		if ( ! $result ) {
			return new WP_Error( 'woocommerce_api_cannot_delete_product', sprintf( _e( 'This %s cannot be deleted', 'woocommerce' ), 'product' ), array( 'status' => 500 ) );
		}
		...
		if ( $force ) {
			return array( 'message' => sprintf( _e( 'Permanently deleted %s', 'woocommerce' ), 'product' ) );
		} else {
			$this->server->send_status( '202' );

			return array( 'message' => sprintf( _e( 'Deleted %s', 'woocommerce' ), 'product' ) );
		}
	}
	...
	public function get_product_categories( $fields = null ) {
		try {
			...
			if ( ! current_user_can( 'manage_product_terms' ) ) {
				throw new WC_API_Exception( 'woocommerce_api_user_cannot_read_product_categories', _e( 'You do not have permission to read product categories', 'woocommerce' ), 401 );
			}
	...
	public function get_product_category( $id, $fields = null ) {
		try {
			...
			// Validate ID
			if ( empty( $id ) ) {
				throw new WC_API_Exception( 'woocommerce_api_invalid_product_category_id', _e( 'Invalid product category ID', 'woocommerce' ), 400 );
			}
			...
			if ( is_wp_error( $term ) || is_null( $term ) ) {
				throw new WC_API_Exception( 'woocommerce_api_invalid_product_category_id', _e( 'A product category with the provided ID could not be found', 'woocommerce' ), 404 );
			}
	...
	public function create_product_category( $data ) {
		...
		try {
			if ( ! isset( $data['product_category'] ) ) {
				throw new WC_API_Exception( 'woocommerce_api_missing_product_category_data', sprintf( _e( 'No %1$s data specified to create %1$s', 'woocommerce' ), 'product_category' ), 400 );
			}

			// Check permissions
			if ( ! current_user_can( 'manage_product_terms' ) ) {
				throw new WC_API_Exception( 'woocommerce_api_user_cannot_create_product_category', _e( 'You do not have permission to create product categories', 'woocommerce' ), 401 );
			}
			...
			if ( $data['parent'] ) {
				...
				if ( ! $parent ) {
					throw new WC_API_Exception( 'woocommerce_api_invalid_product_category_parent', _e( 'Product category parent is invalid', 'woocommerce' ), 400 );
				}
			}
	...
	public function edit_product_category( $id, $data ) {
		...
		try {
			if ( ! isset( $data['product_category'] ) ) {
				throw new WC_API_Exception( 'woocommerce_api_missing_product_category', sprintf( _e( 'No %1$s data specified to edit %1$s', 'woocommerce' ), 'product_category' ), 400 );
			}
			...
			// Check permissions.
			if ( ! current_user_can( 'manage_product_terms' ) ) {
				throw new WC_API_Exception( 'woocommerce_api_user_cannot_edit_product_category', _e( 'You do not have permission to edit product categories', 'woocommerce' ), 401 );
			}
			...
			if ( is_wp_error( $update ) ) {
				throw new WC_API_Exception( 'woocommerce_api_cannot_edit_product_catgory', _e( 'Could not edit the category', 'woocommerce' ), 400 );
			}
	...
	public function delete_product_category( $id ) {
		...
		try {
			// Check permissions
			if ( ! current_user_can( 'manage_product_terms' ) ) {
				throw new WC_API_Exception( 'woocommerce_api_user_cannot_delete_product_category', _e( 'You do not have permission to delete product category', 'woocommerce' ), 401 );
			}

			$id      = absint( $id );
			$deleted = wp_delete_term( $id, 'product_cat' );
			if ( ! $deleted || is_wp_error( $deleted ) ) {
				throw new WC_API_Exception( 'woocommerce_api_cannot_delete_product_category', _e( 'Could not delete the category', 'woocommerce' ), 401 );
			}
			...
			return array( 'message' => sprintf( _e( 'Deleted %s', 'woocommerce' ), 'product_category' ) );
		} catch ( WC_API_Exception $e ) {
			return new WP_Error( $e->getErrorCode(), $e->getMessage(), array( 'status' => $e->getCode() ) );
		}
	}
	...
	public function get_product_tags( $fields = null ) {
		try {
			// Permissions check
			if ( ! current_user_can( 'manage_product_terms' ) ) {
				throw new WC_API_Exception( 'woocommerce_api_user_cannot_read_product_tags', _e( 'You do not have permission to read product tags', 'woocommerce' ), 401 );
			}
	...
	public function get_product_tag( $id, $fields = null ) {
		try {
			...
			// Validate ID
			if ( empty( $id ) ) {
				throw new WC_API_Exception( 'woocommerce_api_invalid_product_tag_id', _e( 'Invalid product tag ID', 'woocommerce' ), 400 );
			}
			...
			if ( is_wp_error( $term ) || is_null( $term ) ) {
				throw new WC_API_Exception( 'woocommerce_api_invalid_product_tag_id', _e( 'A product tag with the provided ID could not be found', 'woocommerce' ), 404 );
			}
	...
	public function create_product_tag( $data ) {
		try {
			if ( ! isset( $data['product_tag'] ) ) {
				throw new WC_API_Exception( 'woocommerce_api_missing_product_tag_data', sprintf( _e( 'No %1$s data specified to create %1$s', 'woocommerce' ), 'product_tag' ), 400 );
			}

			// Check permissions
			if ( ! current_user_can( 'manage_product_terms' ) ) {
				throw new WC_API_Exception( 'woocommerce_api_user_cannot_create_product_tag', _e( 'You do not have permission to create product tags', 'woocommerce' ), 401 );
			}
	...
	public function edit_product_tag( $id, $data ) {
		try {
			if ( ! isset( $data['product_tag'] ) ) {
				throw new WC_API_Exception( 'woocommerce_api_missing_product_tag', sprintf( _e( 'No %1$s data specified to edit %1$s', 'woocommerce' ), 'product_tag' ), 400 );
			}
			...
			// Check permissions.
			if ( ! current_user_can( 'manage_product_terms' ) ) {
				throw new WC_API_Exception( 'woocommerce_api_user_cannot_edit_product_tag', _e( 'You do not have permission to edit product tags', 'woocommerce' ), 401 );
			}
			...
			if ( is_wp_error( $update ) ) {
				throw new WC_API_Exception( 'woocommerce_api_cannot_edit_product_tag', _e( 'Could not edit the tag', 'woocommerce' ), 400 );
			}
	...
	public function delete_product_tag( $id ) {
		try {
			// Check permissions
			if ( ! current_user_can( 'manage_product_terms' ) ) {
				throw new WC_API_Exception( 'woocommerce_api_user_cannot_delete_product_tag', _e( 'You do not have permission to delete product tag', 'woocommerce' ), 401 );
			}

			$id      = absint( $id );
			$deleted = wp_delete_term( $id, 'product_tag' );
			if ( ! $deleted || is_wp_error( $deleted ) ) {
				throw new WC_API_Exception( 'woocommerce_api_cannot_delete_product_tag', _e( 'Could not delete the tag', 'woocommerce' ), 401 );
			}

			do_action( 'woocommerce_api_delete_product_tag', $id, $this );

			return array( 'message' => sprintf( _e( 'Deleted %s', 'woocommerce' ), 'product_tag' ) );
		} catch ( WC_API_Exception $e ) {
			return new WP_Error( $e->getErrorCode(), $e->getMessage(), array( 'status' => $e->getCode() ) );
		}
	}
		...
		if ( isset( $data['sku'] ) ) {
			...
			if ( '' == $new_sku ) {
				...
				if ( ! empty( $new_sku ) ) {
					...
					if ( ! $unique_sku ) {
						throw new WC_API_Exception( 'woocommerce_api_product_sku_already_exists', _e( 'The SKU already exists on another product.', 'woocommerce' ), 400 );
					} else {
	...
	protected function save_variations( $product, $request ) {
		...
		foreach ( $variations as $menu_order => $data ) {
			...
			if ( ! $variation->get_slug() ) {
				...
				$variation->set_name( sprintf( _e( 'Variation #%1$s of %2$s', 'woocommerce' ), $variation->get_id(), $product->get_name() ) );
	...
	protected function upload_image_from_url( $image_url, $upload_for = 'product_image' ) {
		...
		if ( ! $parsed_url || ! is_array( $parsed_url ) ) {
			throw new WC_API_Exception( 'woocommerce_api_invalid_' . $upload_for, sprintf( _e( 'Invalid URL %s.', 'woocommerce' ), $image_url ), 400 );
		}
		...
		if ( is_wp_error( $response ) ) {
			throw new WC_API_Exception( 'woocommerce_api_invalid_remote_' . $upload_for, sprintf( _e( 'Error getting remote image %s.', 'woocommerce' ), $image_url ) . ' ' . sprintf( _e( 'Error: %s.', 'woocommerce' ), $response->get_error_message() ), 400 );
		} elseif ( 200 !== wp_remote_retrieve_response_code( $response ) ) {
			throw new WC_API_Exception( 'woocommerce_api_invalid_remote_' . $upload_for, sprintf( _e( 'Error getting remote image %s.', 'woocommerce' ), $image_url ), 400 );
		}
		...
		if ( ! $wp_filetype['type'] ) {
			...
			if ( ! $wp_filetype['type'] ) {
				throw new WC_API_Exception( 'woocommerce_api_invalid_' . $upload_for, _e( 'Invalid image type.', 'woocommerce' ), 400 );
			}
		}
		...
		if ( 0 == $filesize ) {
			@unlink( $upload['file'] );
			unset( $upload );
			throw new WC_API_Exception( 'woocommerce_api_' . $upload_for . '_upload_file_error', _e( 'Zero size file downloaded.', 'woocommerce' ), 400 );
		}
	...
	public function get_product_attributes( $fields = null ) {
		try {
			// Permissions check.
			if ( ! current_user_can( 'manage_product_terms' ) ) {
				throw new WC_API_Exception( 'woocommerce_api_user_cannot_read_product_attributes', _e( 'You do not have permission to read product attributes', 'woocommerce' ), 401 );
			}
	...
	public function get_product_attribute( $id, $fields = null ) {
		...
		try {
			...
			// Validate ID
			if ( empty( $id ) ) {
				throw new WC_API_Exception( 'woocommerce_api_invalid_product_attribute_id', _e( 'Invalid product attribute ID', 'woocommerce' ), 400 );
			}
			...
			if ( is_wp_error( $attribute ) || is_null( $attribute ) ) {
				throw new WC_API_Exception( 'woocommerce_api_invalid_product_attribute_id', _e( 'A product attribute with the provided ID could not be found', 'woocommerce' ), 404 );
			}
	...
	protected function validate_attribute_data( $name, $slug, $type, $order_by, $new_data = true ) {
		if ( empty( $name ) ) {
			throw new WC_API_Exception( 'woocommerce_api_missing_product_attribute_name', sprintf( _e( 'Missing parameter %s', 'woocommerce' ), 'name' ), 400 );
		}

		if ( strlen( $slug ) >= 28 ) {
			throw new WC_API_Exception( 'woocommerce_api_invalid_product_attribute_slug_too_long', sprintf( _e( 'Slug "%s" is too long (28 characters max). Shorten it, please.', 'woocommerce' ), $slug ), 400 );
		} elseif ( wc_check_if_attribute_name_is_reserved( $slug ) ) {
			throw new WC_API_Exception( 'woocommerce_api_invalid_product_attribute_slug_reserved_name', sprintf( _e( 'Slug "%s" is not allowed because it is a reserved term. Change it, please.', 'woocommerce' ), $slug ), 400 );
		} elseif ( $new_data && taxonomy_exists( wc_attribute_taxonomy_name( $slug ) ) ) {
			throw new WC_API_Exception( 'woocommerce_api_invalid_product_attribute_slug_already_exists', sprintf( _e( 'Slug "%s" is already in use. Change it, please.', 'woocommerce' ), $slug ), 400 );
		}

		// Validate the attribute type
		if ( ! in_array( wc_clean( $type ), array_keys( wc_get_attribute_types() ) ) ) {
			throw new WC_API_Exception( 'woocommerce_api_invalid_product_attribute_type', sprintf( _e( 'Invalid product attribute type - the product attribute type must be any of these: %s', 'woocommerce' ), implode( ', ', array_keys( wc_get_attribute_types() ) ) ), 400 );
		}

		// Validate the attribute order by
		if ( ! in_array( wc_clean( $order_by ), array( 'menu_order', 'name', 'name_num', 'id' ) ) ) {
			throw new WC_API_Exception( 'woocommerce_api_invalid_product_attribute_order_by', sprintf( _e( 'Invalid product attribute order_by type - the product attribute order_by type must be any of these: %s', 'woocommerce' ), implode( ', ', array( 'menu_order', 'name', 'name_num', 'id' ) ) ), 400 );
		}
	...
	public function create_product_attribute( $data ) {
		...
		try {
			if ( ! isset( $data['product_attribute'] ) ) {
				throw new WC_API_Exception( 'woocommerce_api_missing_product_attribute_data', sprintf( _e( 'No %1$s data specified to create %1$s', 'woocommerce' ), 'product_attribute' ), 400 );
			}
			...
			// Check permissions.
			if ( ! current_user_can( 'manage_product_terms' ) ) {
				throw new WC_API_Exception( 'woocommerce_api_user_cannot_create_product_attribute', _e( 'You do not have permission to create product attributes', 'woocommerce' ), 401 );
			}
	...
	public function edit_product_attribute( $id, $data ) {
		...
		try {
			if ( ! isset( $data['product_attribute'] ) ) {
				throw new WC_API_Exception( 'woocommerce_api_missing_product_attribute_data', sprintf( _e( 'No %1$s data specified to edit %1$s', 'woocommerce' ), 'product_attribute' ), 400 );
			}
			...
			// Check permissions.
			if ( ! current_user_can( 'manage_product_terms' ) ) {
				throw new WC_API_Exception( 'woocommerce_api_user_cannot_edit_product_attribute', _e( 'You do not have permission to edit product attributes', 'woocommerce' ), 401 );
			}
			...
			// Checks for an error in the product creation.
			if ( false === $update ) {
				throw new WC_API_Exception( 'woocommerce_api_cannot_edit_product_attribute', _e( 'Could not edit the attribute', 'woocommerce' ), 400 );
			}
	...
	public function delete_product_attribute( $id ) {
		...
		try {
			// Check permissions.
			if ( ! current_user_can( 'manage_product_terms' ) ) {
				throw new WC_API_Exception( 'woocommerce_api_user_cannot_delete_product_attribute', _e( 'You do not have permission to delete product attributes', 'woocommerce' ), 401 );
			}
			...
			if ( false === $deleted ) {
				throw new WC_API_Exception( 'woocommerce_api_cannot_delete_product_attribute', _e( 'Could not delete the attribute', 'woocommerce' ), 401 );
			}
			...
			return array( 'message' => sprintf( _e( 'Deleted %s', 'woocommerce' ), 'product_attribute' ) );
		} catch ( WC_API_Exception $e ) {
			return new WP_Error( $e->getErrorCode(), $e->getMessage(), array( 'status' => $e->getCode() ) );
		}
	}
	...
	public function get_product_attribute_terms( $attribute_id, $fields = null ) {
		try {
			// Permissions check.
			if ( ! current_user_can( 'manage_product_terms' ) ) {
				throw new WC_API_Exception( 'woocommerce_api_user_cannot_read_product_attribute_terms', _e( 'You do not have permission to read product attribute terms', 'woocommerce' ), 401 );
			}
	...
	public function get_product_attribute_term( $attribute_id, $id, $fields = null ) {
		...
		try {
			...
			// Validate ID
			if ( empty( $id ) ) {
				throw new WC_API_Exception( 'woocommerce_api_invalid_product_attribute_term_id', _e( 'Invalid product attribute ID', 'woocommerce' ), 400 );
			}
			...
			if ( is_wp_error( $term ) || is_null( $term ) ) {
				throw new WC_API_Exception( 'woocommerce_api_invalid_product_attribute_term_id', _e( 'A product attribute term with the provided ID could not be found', 'woocommerce' ), 404 );
			}
	...
	public function create_product_attribute_term( $attribute_id, $data ) {
		...
		try {
			if ( ! isset( $data['product_attribute_term'] ) ) {
				throw new WC_API_Exception( 'woocommerce_api_missing_product_attribute_term_data', sprintf( _e( 'No %1$s data specified to create %1$s', 'woocommerce' ), 'product_attribute_term' ), 400 );
			}
			...
			if ( ! isset( $data['name'] ) ) {
				throw new WC_API_Exception( 'woocommerce_api_missing_product_attribute_term_name', sprintf( _e( 'Missing parameter %s', 'woocommerce' ), 'name' ), 400 );
			}
	...
	public function edit_product_attribute_term( $attribute_id, $id, $data ) {
		...
		try {
			if ( ! isset( $data['product_attribute_term'] ) ) {
				throw new WC_API_Exception( 'woocommerce_api_missing_product_attribute_term_data', sprintf( _e( 'No %1$s data specified to edit %1$s', 'woocommerce' ), 'product_attribute_term' ), 400 );
			}
	...
	public function delete_product_attribute_term( $attribute_id, $id ) {
		...
		try {
			// Check permissions.
			if ( ! current_user_can( 'manage_product_terms' ) ) {
				throw new WC_API_Exception( 'woocommerce_api_user_cannot_delete_product_attribute_term', _e( 'You do not have permission to delete product attribute terms', 'woocommerce' ), 401 );
			}
			...
			if ( ! $term ) {
				throw new WC_API_Exception( 'woocommerce_api_cannot_delete_product_attribute_term', sprintf( _e( 'This %s cannot be deleted', 'woocommerce' ), 'product_attribute_term' ), 500 );
	...
	public function bulk( $data ) {
		...
		try {
			if ( ! isset( $data['products'] ) ) {
				throw new WC_API_Exception( 'woocommerce_api_missing_products_data', sprintf( _e( 'No %1$s data specified to create/edit %1$s', 'woocommerce' ), 'products' ), 400 );
			}
			...
			if ( count( $data ) > $limit ) {
				throw new WC_API_Exception( 'woocommerce_api_products_request_entity_too_large', sprintf( _e( 'Unable to accept more than %s items for this request.', 'woocommerce' ), $limit ), 413 );
			}
	...
	public function get_product_shipping_classes( $fields = null ) {
		try {
			// Permissions check
			if ( ! current_user_can( 'manage_product_terms' ) ) {
				throw new WC_API_Exception( 'woocommerce_api_user_cannot_read_product_shipping_classes', _e( 'You do not have permission to read product shipping classes', 'woocommerce' ), 401 );
			}
	...
	public function get_product_shipping_class( $id, $fields = null ) {
		try {
			$id = absint( $id );
			if ( ! $id ) {
				throw new WC_API_Exception( 'woocommerce_api_invalid_product_shipping_class_id', _e( 'Invalid product shipping class ID', 'woocommerce' ), 400 );
			}
			...
			if ( is_wp_error( $term ) || is_null( $term ) ) {
				throw new WC_API_Exception( 'woocommerce_api_invalid_product_shipping_class_id', _e( 'A product shipping class with the provided ID could not be found', 'woocommerce' ), 404 );
			}
	...
	public function create_product_shipping_class( $data ) {
		...
		try {
			if ( ! isset( $data['product_shipping_class'] ) ) {
				throw new WC_API_Exception( 'woocommerce_api_missing_product_shipping_class_data', sprintf( _e( 'No %1$s data specified to create %1$s', 'woocommerce' ), 'product_shipping_class' ), 400 );
			}

			// Check permissions
			if ( ! current_user_can( 'manage_product_terms' ) ) {
				throw new WC_API_Exception( 'woocommerce_api_user_cannot_create_product_shipping_class', _e( 'You do not have permission to create product shipping classes', 'woocommerce' ), 401 );
			}
			...
			if ( $data['parent'] ) {
				...
				if ( ! $parent ) {
					throw new WC_API_Exception( 'woocommerce_api_invalid_product_shipping_class_parent', _e( 'Product shipping class parent is invalid', 'woocommerce' ), 400 );
				}
			}
	...
	public function edit_product_shipping_class( $id, $data ) {
		...
		try {
			if ( ! isset( $data['product_shipping_class'] ) ) {
				throw new WC_API_Exception( 'woocommerce_api_missing_product_shipping_class', sprintf( _e( 'No %1$s data specified to edit %1$s', 'woocommerce' ), 'product_shipping_class' ), 400 );
			}
			...
			// Check permissions
			if ( ! current_user_can( 'manage_product_terms' ) ) {
				throw new WC_API_Exception( 'woocommerce_api_user_cannot_edit_product_shipping_class', _e( 'You do not have permission to edit product shipping classes', 'woocommerce' ), 401 );
			}
			...
			if ( is_wp_error( $update ) ) {
				throw new WC_API_Exception( 'woocommerce_api_cannot_edit_product_shipping_class', _e( 'Could not edit the shipping class', 'woocommerce' ), 400 );
			}
	...
	public function delete_product_shipping_class( $id ) {
		...
		try {
			// Check permissions
			if ( ! current_user_can( 'manage_product_terms' ) ) {
				throw new WC_API_Exception( 'woocommerce_api_user_cannot_delete_product_shipping_class', _e( 'You do not have permission to delete product shipping classes', 'woocommerce' ), 401 );
			}

			$id      = absint( $id );
			$deleted = wp_delete_term( $id, 'product_shipping_class' );
			if ( ! $deleted || is_wp_error( $deleted ) ) {
				throw new WC_API_Exception( 'woocommerce_api_cannot_delete_product_shipping_class', _e( 'Could not delete the shipping class', 'woocommerce' ), 401 );
			}

			do_action( 'woocommerce_api_delete_product_shipping_class', $id, $this );

			return array( 'message' => sprintf( _e( 'Deleted %s', 'woocommerce' ), 'product_shipping_class' ) );
		} catch ( WC_API_Exception $e ) {
			return new WP_Error( $e->getErrorCode(), $e->getMessage(), array( 'status' => $e->getCode() ) );
		}
	}

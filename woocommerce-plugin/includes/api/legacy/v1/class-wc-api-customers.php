...
class WC_API_Customers extends WC_API_Resource {
	...
	public function get_customers_count( $filter = array() ) {
		...
		if ( ! current_user_can( 'list_users' ) ) {
			return new WP_Error( 'woocommerce_api_user_cannot_read_customers_count', _e( 'You do not have permission to read the customers count', 'woocommerce' ), array( 'status' => 401 ) );
		}
	...
	public function create_customer( $data ) {
		...
		if ( ! current_user_can( 'create_users' ) ) {
			return new WP_Error( 'woocommerce_api_user_cannot_create_customer', _e( 'You do not have permission to create this customer', 'woocommerce' ), array( 'status' => 401 ) );
		}
	...
	protected function validate_request( $id, $type, $context ) {
		...
		// validate ID
		if ( empty( $id ) ) {
			return new WP_Error( 'woocommerce_api_invalid_customer_id', _e( 'Invalid customer ID', 'woocommerce' ), array( 'status' => 404 ) );
		}
		...
		if ( 0 === $customer->ID ) {
			return new WP_Error( 'woocommerce_api_invalid_customer', _e( 'Invalid customer', 'woocommerce' ), array( 'status' => 404 ) );
		}
		...
		switch ( $context ) {
			...
			case 'read':
				if ( ! current_user_can( 'list_users' ) ) {
					return new WP_Error( 'woocommerce_api_user_cannot_read_customer', _e( 'You do not have permission to read this customer', 'woocommerce' ), array( 'status' => 401 ) );
				}
				break;

			case 'edit':
				if ( ! current_user_can( 'edit_users' ) ) {
					return new WP_Error( 'woocommerce_api_user_cannot_edit_customer', _e( 'You do not have permission to edit this customer', 'woocommerce' ), array( 'status' => 401 ) );
				}
				break;

			case 'delete':
				if ( ! current_user_can( 'delete_users' ) ) {
					return new WP_Error( 'woocommerce_api_user_cannot_delete_customer', _e( 'You do not have permission to delete this customer', 'woocommerce' ), array( 'status' => 401 ) );
				}
				break;
		}

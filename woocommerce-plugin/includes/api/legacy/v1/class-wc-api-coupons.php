...
class WC_API_Coupons extends WC_API_Resource {
	...
	public function get_coupons_count( $filter = array() ) {
		...
		if ( ! current_user_can( 'read_private_shop_coupons' ) ) {
			return new WP_Error( 'woocommerce_api_user_cannot_read_coupons_count', _e( 'You do not have permission to read the coupons count', 'woocommerce' ), array( 'status' => 401 ) );
		}
	...
	public function get_coupon_by_code( $code, $fields = null ) {
		...
		if ( is_null( $id ) ) {
			return new WP_Error( 'woocommerce_api_invalid_coupon_code', _e( 'Invalid coupon code', 'woocommerce' ), array( 'status' => 404 ) );
		}

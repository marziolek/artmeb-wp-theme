...
class WC_API_Server {
	...
	public function check_authentication() {
		...
		if ( is_a( $user, 'WP_User' ) ) {
			...
			$user = new WP_Error( 'woocommerce_api_authentication_error', _e( 'Invalid authentication method', 'woocommerce' ), array( 'code' => 500 ) );
	...
	public function dispatch() {
		...
		switch ( $this->method ) {
			...
			default :
				return new WP_Error( 'woocommerce_api_unsupported_method', _e( 'Unsupported request method', 'woocommerce' ), array( 'status' => 400 ) );
		}
		...
		foreach ( $this->get_routes() as $route => $handlers ) {
			foreach ( $handlers as $handler ) {
				...
				if ( ! is_callable( $callback ) ) {
					return new WP_Error( 'woocommerce_api_invalid_handler', _e( 'The handler for the route is invalid', 'woocommerce' ), array( 'status' => 500 ) );
				}
		...
		return new WP_Error( 'woocommerce_api_no_route', _e( 'No route was found matching the URL and request method', 'woocommerce' ), array( 'status' => 404 ) );
	}
	...
	protected function sort_callback_params( $callback, $provided ) {
		...
		foreach ( $wanted as $param ) {
			if ( isset( $provided[ $param->getName() ] ) ) {
				...
				return new WP_Error( 'woocommerce_api_missing_callback_param', sprintf( _e( 'Missing parameter %s', 'woocommerce' ), $param->getName() ), array( 'status' => 400 ) );
			}
		}

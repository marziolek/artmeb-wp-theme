...
class WC_API_Orders extends WC_API_Resource {
	...
	public function get_orders_count( $status = null, $filter = array() ) {
		...
		try {
			if ( ! current_user_can( 'read_private_shop_orders' ) ) {
				throw new WC_API_Exception( 'woocommerce_api_user_cannot_read_orders_count', _e( 'You do not have permission to read the orders count', 'woocommerce' ), 401 );
			}
	...
	public function create_order( $data ) {
		...
		try {
			if ( ! isset( $data['order'] ) ) {
				throw new WC_API_Exception( 'woocommerce_api_missing_order_data', sprintf( _e( 'No %1$s data specified to create %1$s', 'woocommerce' ), 'order' ), 400 );
			}
			...
			if ( ! current_user_can( 'publish_shop_orders' ) ) {
				throw new WC_API_Exception( 'woocommerce_api_user_cannot_create_order', _e( 'You do not have permission to create orders', 'woocommerce' ), 401 );
			}
			...
			if ( ! empty( $data['customer_id'] ) ) {
				...
				if ( false === get_user_by( 'id', $data['customer_id'] ) ) {
					throw new WC_API_Exception( 'woocommerce_api_invalid_customer_id', _e( 'Customer ID is invalid.', 'woocommerce' ), 400 );
				}
			...
			if ( is_wp_error( $order ) ) {
				throw new WC_API_Exception( 'woocommerce_api_cannot_create_order', sprintf( _e( 'Cannot create order: %s', 'woocommerce' ), implode( ', ', $order->get_error_messages() ) ), 400 );
			}
			...
			if ( isset( $data['payment_details'] ) && is_array( $data['payment_details'] ) ) {
				...
				if ( empty( $data['payment_details']['method_id'] ) || empty( $data['payment_details']['method_title'] ) ) {
					throw new WC_API_Exception( 'woocommerce_invalid_payment_details', _e( 'Payment method ID and title are required', 'woocommerce' ), 400 );
				}
			...
			if ( isset( $data['currency'] ) ) {
				...
				if ( ! array_key_exists( $data['currency'], get_woocommerce_currencies() ) ) {
					throw new WC_API_Exception( 'woocommerce_invalid_order_currency', _e( 'Provided order currency is invalid.', 'woocommerce' ), 400 );
				}
	...
	public function edit_order( $id, $data ) {
		try {
			if ( ! isset( $data['order'] ) ) {
				throw new WC_API_Exception( 'woocommerce_api_missing_order_data', sprintf( _e( 'No %1$s data specified to edit %1$s', 'woocommerce' ), 'order' ), 400 );
			}
			...
			if ( empty( $order ) ) {
				throw new WC_API_Exception( 'woocommerce_api_invalid_order_id', _e( 'Order ID is invalid', 'woocommerce' ), 400 );
			}
	...
	protected function set_item( $order, $item_type, $item, $action ) {
		...
		if ( 'update' === $action ) {
			...
			if ( is_null( $result ) ) {
				throw new WC_API_Exception( 'woocommerce_invalid_item_id', _e( 'Order item ID provided is not associated with order.', 'woocommerce' ), 400 );
			}
		}
	...
	protected function set_line_item( $order, $item, $action ) {
		...
		if ( ! isset( $item['product_id'] ) && ! isset( $item['sku'] ) ) {
			throw new WC_API_Exception( 'woocommerce_api_invalid_product_id', _e( 'Product ID or SKU is required', 'woocommerce' ), 400 );
		}
		...
		if ( 'update' === $action ) {
			...
			if ( $item['product_id'] != $item_product_id && $item['product_id'] != $item_variation_id ) {
				throw new WC_API_Exception( 'woocommerce_api_invalid_product_id', _e( 'Product ID provided does not match this line item', 'woocommerce' ), 400 );
			}
		}
		...
		if ( isset( $item['variations'] ) && is_array( $item['variations'] ) ) {
			foreach ( $item['variations'] as $key => $value ) {
				if ( ! $key || ! $value ) {
					throw new WC_API_Exception( 'woocommerce_api_invalid_product_variation', _e( 'The product variation is invalid', 'woocommerce' ), 400 );
				}
			}
		...
		if ( ! is_object( $product ) ) {
			throw new WC_API_Exception( 'woocommerce_api_invalid_product', _e( 'Product is invalid.', 'woocommerce' ), 400 );
		}

		// quantity must be positive float
		if ( isset( $item['quantity'] ) && floatval( $item['quantity'] ) <= 0 ) {
			throw new WC_API_Exception( 'woocommerce_api_invalid_product_quantity', _e( 'Product quantity must be a positive float.', 'woocommerce' ), 400 );
		}

		// quantity is required when creating
		if ( $creating && ! isset( $item['quantity'] ) ) {
			throw new WC_API_Exception( 'woocommerce_api_invalid_product_quantity', _e( 'Product quantity is required.', 'woocommerce' ), 400 );
		}
		...
		if ( $creating ) {
			...
			if ( ! $item_id ) {
				throw new WC_API_Exception( 'woocommerce_cannot_create_line_item', _e( 'Cannot create line item, try again.', 'woocommerce' ), 500 );
			}
		}
	}
	...
	protected function set_shipping( $order, $shipping, $action ) {
		...
		if ( isset( $shipping['total'] ) && floatval( $shipping['total'] ) < 0 ) {
			throw new WC_API_Exception( 'woocommerce_invalid_shipping_total', _e( 'Shipping total must be a positive amount.', 'woocommerce' ), 400 );
		}
		...
		if ( 'create' === $action ) {
			...
			if ( ! isset( $shipping['method_id'] ) ) {
				throw new WC_API_Exception( 'woocommerce_invalid_shipping_item', _e( 'Shipping method ID is required.', 'woocommerce' ), 400 );
			}
			...
			if ( ! $shipping_id ) {
				throw new WC_API_Exception( 'woocommerce_cannot_update_shipping', _e( 'Cannot update shipping method, try again.', 'woocommerce' ), 500 );
			}
		}
	}
	...
	protected function set_fee( $order, $fee, $action ) {
		...
		if ( 'create' === $action ) {
			...
			if ( ! isset( $fee['title'] ) ) {
				throw new WC_API_Exception( 'woocommerce_invalid_fee_item', _e( 'Fee title is required', 'woocommerce' ), 400 );
			}
			...
			if ( ! empty( $fee['taxable'] ) ) {
				if ( ! isset( $fee['tax_class'] ) ) {
					throw new WC_API_Exception( 'woocommerce_invalid_fee_item', _e( 'Fee tax class is required when fee is taxable.', 'woocommerce' ), 400 );
				}
			...
			if ( ! $fee_id ) {
				throw new WC_API_Exception( 'woocommerce_cannot_update_fee', _e( 'Cannot update fee, try again.', 'woocommerce' ), 500 );
			}
		}
	}
	...
	protected function set_coupon( $order, $coupon, $action ) {
		...
		if ( isset( $coupon['amount'] ) && floatval( $coupon['amount'] ) < 0 ) {
			throw new WC_API_Exception( 'woocommerce_invalid_coupon_total', _e( 'Coupon discount total must be a positive amount.', 'woocommerce' ), 400 );
		}
		...
		if ( 'create' === $action ) {
			...
			if ( empty( $coupon['code'] ) ) {
				throw new WC_API_Exception( 'woocommerce_invalid_coupon_coupon', _e( 'Coupon code is required.', 'woocommerce' ), 400 );
			}
			...
			if ( ! $coupon_id ) {
				throw new WC_API_Exception( 'woocommerce_cannot_update_order_coupon', _e( 'Cannot update coupon, try again.', 'woocommerce' ), 500 );
			}
		}
	}
	...
	public function get_order_note( $order_id, $id, $fields = null ) {
		try {
			...
			if ( empty( $id ) ) {
				throw new WC_API_Exception( 'woocommerce_api_invalid_order_note_id', _e( 'Invalid order note ID', 'woocommerce' ), 400 );
			}

			$note = get_comment( $id );

			if ( is_null( $note ) ) {
				throw new WC_API_Exception( 'woocommerce_api_invalid_order_note_id', _e( 'An order note with the provided ID could not be found', 'woocommerce' ), 404 );
			}
	...
	public function create_order_note( $order_id, $data ) {
		try {
			if ( ! isset( $data['order_note'] ) ) {
				throw new WC_API_Exception( 'woocommerce_api_missing_order_note_data', sprintf( _e( 'No %1$s data specified to create %1$s', 'woocommerce' ), 'order_note' ), 400 );
			}
			...
			// permission check
			if ( ! current_user_can( 'publish_shop_orders' ) ) {
				throw new WC_API_Exception( 'woocommerce_api_user_cannot_create_order_note', _e( 'You do not have permission to create order notes', 'woocommerce' ), 401 );
			}
			...
			if ( ! isset( $data['note'] ) ) {
				throw new WC_API_Exception( 'woocommerce_api_invalid_order_note', _e( 'Order note is required', 'woocommerce' ), 400 );
			}
			...
			if ( ! $note_id ) {
				throw new WC_API_Exception( 'woocommerce_api_cannot_create_order_note', _e( 'Cannot create order note, please try again.', 'woocommerce' ), 500 );
			}
	...
	public function edit_order_note( $order_id, $id, $data ) {
		try {
			if ( ! isset( $data['order_note'] ) ) {
				throw new WC_API_Exception( 'woocommerce_api_missing_order_note_data', sprintf( _e( 'No %1$s data specified to edit %1$s', 'woocommerce' ), 'order_note' ), 400 );
			}
			...
			if ( $note->comment_post_ID != $order->get_id() ) {
				throw new WC_API_Exception( 'woocommerce_api_invalid_order_note_id', _e( 'The order note ID provided is not associated with the order', 'woocommerce' ), 400 );
			}
	...
	public function delete_order_note( $order_id, $id ) {
		try {
			...
			if ( ! $result ) {
				throw new WC_API_Exception( 'woocommerce_api_cannot_delete_order_note', _e( 'This order note cannot be deleted', 'woocommerce' ), 500 );
			}
	...
	public function get_order_refund( $order_id, $id, $fields = null ) {
		try {
			...
			if ( empty( $id ) ) {
				throw new WC_API_Exception( 'woocommerce_api_invalid_order_refund_id', _e( 'Invalid order refund ID.', 'woocommerce' ), 400 );
			}
			...
			if ( ! $refund ) {
				throw new WC_API_Exception( 'woocommerce_api_invalid_order_refund_id', _e( 'An order refund with the provided ID could not be found.', 'woocommerce' ), 404 );
			}
	...
	public function create_order_refund( $order_id, $data, $api_refund = true ) {
		try {
			if ( ! isset( $data['order_refund'] ) ) {
				throw new WC_API_Exception( 'woocommerce_api_missing_order_refund_data', sprintf( _e( 'No %1$s data specified to create %1$s', 'woocommerce' ), 'order_refund' ), 400 );
			}
			...
			// Permission check
			if ( ! current_user_can( 'publish_shop_orders' ) ) {
				throw new WC_API_Exception( 'woocommerce_api_user_cannot_create_order_refund', _e( 'You do not have permission to create order refunds', 'woocommerce' ), 401 );
			}
			...
			if ( ! isset( $data['amount'] ) ) {
				throw new WC_API_Exception( 'woocommerce_api_invalid_order_refund', _e( 'Refund amount is required.', 'woocommerce' ), 400 );
			} elseif ( 0 > $data['amount'] ) {
				throw new WC_API_Exception( 'woocommerce_api_invalid_order_refund', _e( 'Refund amount must be positive.', 'woocommerce' ), 400 );
			}
			...
			if ( ! $refund ) {
				throw new WC_API_Exception( 'woocommerce_api_cannot_create_order_refund', _e( 'Cannot create order refund, please try again.', 'woocommerce' ), 500 );
			}
			...
			if ( $api_refund ) {
				...
				if ( isset( $payment_gateways[ $order->get_payment_method() ] ) && $payment_gateways[ $order->get_payment_method() ]->supports( 'refunds' ) ) {
					...
					if ( is_wp_error( $result ) ) {
						...
						throw new WC_API_Exception( 'woocommerce_api_create_order_refund_api_failed', _e( 'An error occurred while attempting to create the refund using the payment gateway API.', 'woocommerce' ), 500 );
					}
				}
			}
	...
	public function edit_order_refund( $order_id, $id, $data ) {
		try {
			if ( ! isset( $data['order_refund'] ) ) {
				throw new WC_API_Exception( 'woocommerce_api_missing_order_refund_data', sprintf( _e( 'No %1$s data specified to edit %1$s', 'woocommerce' ), 'order_refund' ), 400 );
			}
			...
			if ( $refund->post_parent != $order_id ) {
				throw new WC_API_Exception( 'woocommerce_api_invalid_order_refund_id', _e( 'The order refund ID provided is not associated with the order.', 'woocommerce' ), 400 );
			}
	...
	public function bulk( $data ) {
		...
		try {
			if ( ! isset( $data['orders'] ) ) {
				throw new WC_API_Exception( 'woocommerce_api_missing_orders_data', sprintf( _e( 'No %1$s data specified to create/edit %1$s', 'woocommerce' ), 'orders' ), 400 );
			}
			...
			if ( count( $data ) > $limit ) {
				throw new WC_API_Exception( 'woocommerce_api_orders_request_entity_too_large', sprintf( _e( 'Unable to accept more than %s items for this request.', 'woocommerce' ), $limit ), 413 );
			}

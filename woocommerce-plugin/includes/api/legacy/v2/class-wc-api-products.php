...
class WC_API_Products extends WC_API_Resource {
	...
	public function upload_product_image( $image_url ) {
		...
		if ( ! $parsed_url || ! is_array( $parsed_url ) ) {
			throw new WC_API_Exception( 'woocommerce_api_invalid_product_image', sprintf( _e( 'Invalid URL %s.', 'woocommerce' ), $image_url ), 400 );
		}
		...
		if ( is_wp_error( $response ) ) {
			throw new WC_API_Exception( 'woocommerce_api_invalid_remote_product_image', sprintf( _e( 'Error getting remote image %s.', 'woocommerce' ), $image_url ) . ' ' . sprintf( _e( 'Error: %s.', 'woocommerce' ), $response->get_error_message() ), 400 );
		} elseif ( 200 !== wp_remote_retrieve_response_code( $response ) ) {
			throw new WC_API_Exception( 'woocommerce_api_invalid_remote_product_image', sprintf( _e( 'Error getting remote image %s.', 'woocommerce' ), $image_url ), 400 );
		}
		...
		if ( ! $wp_filetype['type'] ) {
			...
			if ( ! $wp_filetype['type'] ) {
				throw new WC_API_Exception( 'woocommerce_api_invalid_product_image', _e( 'Invalid image type.', 'woocommerce' ), 400 );
			}
		}
		...
		if ( 0 == $filesize ) {
			@unlink( $upload['file'] );
			unset( $upload );
			throw new WC_API_Exception( 'woocommerce_api_product_image_upload_file_error', _e( 'Zero size file downloaded.', 'woocommerce' ), 400 );
		}
	...
	public function get_product_attribute( $id, $fields = null ) {
		...
		try {
			...
			// Permissions check
			if ( ! current_user_can( 'manage_product_terms' ) ) {
				throw new WC_API_Exception( 'woocommerce_api_user_cannot_read_product_categories', _e( 'You do not have permission to read product attributes', 'woocommerce' ), 401 );
			}
	...
	public function get_product_by_sku( $sku, $fields = null ) {
		try {
			...
			if ( empty( $id ) ) {
				throw new WC_API_Exception( 'woocommerce_api_invalid_product_sku', _e( 'Invalid product SKU', 'woocommerce' ), 404 );
			}

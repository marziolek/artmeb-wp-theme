...
class WC_API_Coupons extends WC_API_Resource {
	...
	public function get_coupon( $id, $fields = null ) {
		try {
			...
			if ( 0 === $coupon->get_id() ) {
				throw new WC_API_Exception( 'woocommerce_api_invalid_coupon_id', _e( 'Invalid coupon ID', 'woocommerce' ), 404 );
			}
	...
	public function get_coupons_count( $filter = array() ) {
		try {
			if ( ! current_user_can( 'read_private_shop_coupons' ) ) {
				throw new WC_API_Exception( 'woocommerce_api_user_cannot_read_coupons_count', _e( 'You do not have permission to read the coupons count', 'woocommerce' ), 401 );
			}
	...
	public function get_coupon_by_code( $code, $fields = null ) {
		...
		try {
			...
			if ( is_null( $id ) ) {
				throw new WC_API_Exception( 'woocommerce_api_invalid_coupon_code', _e( 'Invalid coupon code', 'woocommerce' ), 404 );
			}
	...
	public function create_coupon( $data ) {
		...
		try {
			if ( ! isset( $data['coupon'] ) ) {
				throw new WC_API_Exception( 'woocommerce_api_missing_coupon_data', sprintf( _e( 'No %1$s data specified to create %1$s', 'woocommerce' ), 'coupon' ), 400 );
			}
			...
			if ( ! current_user_can( 'publish_shop_coupons' ) ) {
				throw new WC_API_Exception( 'woocommerce_api_user_cannot_create_coupon', _e( 'You do not have permission to create coupons', 'woocommerce' ), 401 );
			}
			...
			if ( ! isset( $data['code'] ) ) {
				throw new WC_API_Exception( 'woocommerce_api_missing_coupon_code', sprintf( _e( 'Missing parameter %s', 'woocommerce' ), 'code' ), 400 );
			}
			...
			if ( $id_from_code ) {
				throw new WC_API_Exception( 'woocommerce_api_coupon_code_already_exists', _e( 'The coupon code already exists', 'woocommerce' ), 400 );
			}
			...
			if ( ! in_array( wc_clean( $coupon_data['type'] ), array_keys( wc_get_coupon_types() ) ) ) {
				throw new WC_API_Exception( 'woocommerce_api_invalid_coupon_type', sprintf( _e( 'Invalid coupon type - the coupon type must be any of these: %s', 'woocommerce' ), implode( ', ', array_keys( wc_get_coupon_types() ) ) ), 400 );
			}
	...
	public function edit_coupon( $id, $data ) {
		...
		try {
			if ( ! isset( $data['coupon'] ) ) {
				throw new WC_API_Exception( 'woocommerce_api_missing_coupon_data', sprintf( _e( 'No %1$s data specified to edit %1$s', 'woocommerce' ), 'coupon' ), 400 );
			}
			...
			if ( isset( $data['code'] ) ) {
				...
				if ( 0 === $updated ) {
					throw new WC_API_Exception( 'woocommerce_api_cannot_update_coupon', _e( 'Failed to update coupon', 'woocommerce' ), 400 );
				}
			}
	...
	public function bulk( $data ) {
		...
		try {
			if ( ! isset( $data['coupons'] ) ) {
				throw new WC_API_Exception( 'woocommerce_api_missing_coupons_data', sprintf( _e( 'No %1$s data specified to create/edit %1$s', 'woocommerce' ), 'coupons' ), 400 );
			}
			...
			if ( count( $data ) > $limit ) {
				throw new WC_API_Exception( 'woocommerce_api_coupons_request_entity_too_large', sprintf( _e( 'Unable to accept more than %s items for this request.', 'woocommerce' ), $limit ), 413 );
			}

...
class WC_REST_Shipping_Zones_Controller extends WC_REST_Shipping_Zones_Controller_Base {
	...
	public function create_item( $request ) {
		...
		if ( $zone->get_id() !== 0 ) {
			...
			return new WP_Error( 'woocommerce_rest_shipping_zone_not_created', _e( "Resource cannot be created. Check to make sure 'order' and 'name' are present.", 'woocommerce' ), array( 'status' => 500 ) );
		}
	}
	...
	public function update_item( $request ) {
		...
		if ( 0 === $zone->get_id() ) {
			return new WP_Error( "woocommerce_rest_shipping_zone_invalid_zone", _e( 'The "locations not covered by your other zones" zone cannot be updated.', 'woocommerce' ), array( 'status' => 403 ) );
		}
	...
	public function delete_item( $request ) {
		...
		if ( $force ) {
			...
			return new WP_Error( 'rest_trash_not_supported', _e( 'Shipping zones do not support trashing.', 'woocommerce' ), array( 'status' => 501 ) );
		}

...
class WC_REST_Customers_V1_Controller extends WC_REST_Controller {
	...
	public function create_item( $request ) {
		try {
			if ( ! empty( $request['id'] ) ) {
				throw new WC_REST_Exception( 'woocommerce_rest_customer_exists', _e( 'Cannot create existing resource.', 'woocommerce' ), 400 );
			}
			...
			if ( ! $customer->get_id() ) {
				throw new WC_REST_Exception( 'woocommerce_rest_cannot_create', _e( 'This resource cannot be created.', 'woocommerce' ), 400 );
			}
	...
	public function get_item( $request ) {
		...
		if ( empty( $id ) || empty( $user_data->ID ) ) {
			return new WP_Error( 'woocommerce_rest_invalid_id', _e( 'Invalid resource ID.', 'woocommerce' ), array( 'status' => 404 ) );
		}
	...
	public function update_item( $request ) {
		try {
			...
			if ( ! $customer->get_id() ) {
				throw new WC_REST_Exception( 'woocommerce_rest_invalid_id', _e( 'Invalid resource ID.', 'woocommerce' ), 400 );
			}

			if ( ! empty( $request['email'] ) && email_exists( $request['email'] ) && $request['email'] !== $customer->get_email() ) {
				throw new WC_REST_Exception( 'woocommerce_rest_customer_invalid_email', _e( 'Email address is invalid.', 'woocommerce' ), 400 );
			}

			if ( ! empty( $request['username'] ) && $request['username'] !== $customer->get_username() ) {
				throw new WC_REST_Exception( 'woocommerce_rest_customer_invalid_argument', _e( "Username isn't editable.", 'woocommerce' ), 400 );
			}
	...
	public function delete_item( $request ) {
		...
		if ( ! $force ) {
			return new WP_Error( 'woocommerce_rest_trash_not_supported', _e( 'Customers do not support trashing.', 'woocommerce' ), array( 'status' => 501 ) );
		}

		$user_data = get_userdata( $id );
		if ( ! $user_data ) {
			return new WP_Error( 'woocommerce_rest_invalid_id', _e( 'Invalid resource id.', 'woocommerce' ), array( 'status' => 400 ) );
		}

		if ( ! empty( $reassign ) ) {
			if ( $reassign === $id || ! get_userdata( $reassign ) ) {
				return new WP_Error( 'woocommerce_rest_customer_invalid_reassign', _e( 'Invalid resource id for reassignment.', 'woocommerce' ), array( 'status' => 400 ) );
			}
		}
		...
		if ( ! $result ) {
			return new WP_Error( 'woocommerce_rest_cannot_delete', _e( 'The resource cannot be deleted.', 'woocommerce' ), array( 'status' => 500 ) );
		}

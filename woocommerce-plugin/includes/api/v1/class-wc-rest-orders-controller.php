...
class WC_REST_Orders_V1_Controller extends WC_REST_Posts_Controller {
	...
	protected function create_order( $request ) {
		try {
			...
			if ( ! is_null( $request['customer_id'] ) && 0 !== $request['customer_id'] && false === get_user_by( 'id', $request['customer_id'] ) ) {
				throw new WC_REST_Exception( 'woocommerce_rest_invalid_customer_id',_e( 'Customer ID is invalid.', 'woocommerce' ), 400 );
			}
	...
	protected function get_product_id( $posted ) {
		if ( ! empty( $posted['sku'] ) ) {
			...
			throw new WC_REST_Exception( 'woocommerce_rest_required_product_reference', _e( 'Product ID or SKU is required.', 'woocommerce' ), 400 );
		}
		return $product_id;
	}
	...
	protected function prepare_shipping_lines( $posted, $action ) {
		...
		if ( 'create' === $action ) {
			if ( empty( $posted['method_id'] ) ) {
				throw new WC_REST_Exception( 'woocommerce_rest_invalid_shipping_item', _e( 'Shipping method ID is required.', 'woocommerce' ), 400 );
			}
		}
	...
	protected function prepare_fee_lines( $posted, $action ) {
		...
		if ( 'create' === $action ) {
			if ( empty( $posted['name'] ) ) {
				throw new WC_REST_Exception( 'woocommerce_rest_invalid_fee_item', _e( 'Fee name is required.', 'woocommerce' ), 400 );
			}
		}
	...
	protected function prepare_coupon_lines( $posted, $action ) {
		...
		if ( 'create' === $action ) {
			if ( empty( $posted['code'] ) ) {
				throw new WC_REST_Exception( 'woocommerce_rest_invalid_coupon_coupon', _e( 'Coupon code is required.', 'woocommerce' ), 400 );
			}
		}
	...
	protected function set_item( $order, $item_type, $posted ) {
		...
		if ( 'update' === $action ) {
			...
			if ( is_null( $result ) ) {
				throw new WC_REST_Exception( 'woocommerce_rest_invalid_item_id', _e( 'Order item ID provided is not associated with order.', 'woocommerce' ), 400 );
			}
		}

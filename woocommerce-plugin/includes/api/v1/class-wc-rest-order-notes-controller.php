...
class WC_REST_Order_Notes_V1_Controller extends WC_REST_Controller {
	...
	public function get_items( $request ) {
		...
		if ( ! $order || $this->post_type !== $order->get_type() ) {
			return new WP_Error( "woocommerce_rest_{$this->post_type}_invalid_id", _e( 'Invalid order ID.', 'woocommerce' ), array( 'status' => 404 ) );
		}
	...
	public function create_item( $request ) {
		...
		if ( ! $order || $this->post_type !== $order->get_type() ) {
			return new WP_Error( 'woocommerce_rest_order_invalid_id', _e( 'Invalid order ID.', 'woocommerce' ), array( 'status' => 404 ) );
		}
		...
		if ( ! $note_id ) {
			return new WP_Error( 'woocommerce_api_cannot_create_order_note', _e( 'Cannot create order note, please try again.', 'woocommerce' ), array( 'status' => 500 ) );
		}
	...
	public function delete_item( $request ) {
		...
		if ( ! $force ) {
			return new WP_Error( 'woocommerce_rest_trash_not_supported', _e( 'Webhooks do not support trashing.', 'woocommerce' ), array( 'status' => 501 ) );
		}
		...
		if ( ! $result ) {
			return new WP_Error( 'woocommerce_rest_cannot_delete', sprintf( _e( 'The %s cannot be deleted.', 'woocommerce' ), 'order_note' ), array( 'status' => 500 ) );
		}

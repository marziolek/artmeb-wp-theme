...
class WC_REST_Coupons_V1_Controller extends WC_REST_Posts_Controller {
	...
	protected function prepare_item_for_database( $request ) {
		...
		if ( 'POST' === $request->get_method() && 0 === $coupon->get_id() ) {
			if ( empty( $request['code'] ) ) {
				return new WP_Error( 'woocommerce_rest_empty_coupon_code', sprintf( _e( 'The coupon code cannot be empty.', 'woocommerce' ), 'code' ), array( 'status' => 400 ) );
			}
		}
			...
			if ( ! is_null( $value ) ) {
				switch ( $key ) {
					case 'code' :
						...
						if ( $id_from_code ) {
							return new WP_Error( 'woocommerce_rest_coupon_code_already_exists', _e( 'The coupon code already exists', 'woocommerce' ), array( 'status' => 400 ) );
						}

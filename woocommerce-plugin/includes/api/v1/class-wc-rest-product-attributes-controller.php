...
class WC_REST_Product_Attributes_V1_Controller extends WC_REST_Controller {
	...
	public function create_item_permissions_check( $request ) {
		if ( ! wc_rest_check_manager_permissions( 'attributes', 'create' ) ) {
			return new WP_Error( 'woocommerce_rest_cannot_create', _e( 'Sorry, you cannot create new resource.', 'woocommerce' ), array( 'status' => rest_authorization_required_code() ) );
		}
	...
	public function get_item_permissions_check( $request ) {
		if ( ! $this->get_taxonomy( $request ) ) {
			return new WP_Error( 'woocommerce_rest_taxonomy_invalid', _e( 'Resource does not exist.', 'woocommerce' ), array( 'status' => 404 ) );
		}
	...
	public function update_item_permissions_check( $request ) {
		...
		if ( ! wc_rest_check_manager_permissions( 'attributes', 'edit' ) ) {
			return new WP_Error( 'woocommerce_rest_cannot_update', _e( 'Sorry, you cannot update resource.', 'woocommerce' ), array( 'status' => rest_authorization_required_code() ) );
		}
	...
	public function update_item( $request ) {
		...
		if ( $args ) {
			...
			// Checks for errors.
			if ( false === $update ) {
				return new WP_Error( 'woocommerce_rest_cannot_edit', _e( 'Could not edit the attribute.', 'woocommerce' ), array( 'status' => 400 ) );
			}
		}
	...
	public function delete_item( $request ) {
		...
		if ( ! $force ) {
			return new WP_Error( 'woocommerce_rest_trash_not_supported', _e( 'Resource does not support trashing.', 'woocommerce' ), array( 'status' => 501 ) );
		}
	...
	protected function get_attribute( $id ) {
		...
		if ( is_wp_error( $attribute ) || is_null( $attribute ) ) {
			return new WP_Error( 'woocommerce_rest_attribute_invalid', _e( 'Resource does not exist.', 'woocommerce' ), array( 'status' => 404 ) );
		}
	...
	protected function validate_attribute_slug( $slug, $new_data = true ) {
		if ( strlen( $slug ) >= 28 ) {
			return new WP_Error( 'woocommerce_rest_invalid_product_attribute_slug_too_long', sprintf( _e( 'Slug "%s" is too long (28 characters max). Shorten it, please.', 'woocommerce' ), $slug ), array( 'status' => 400 ) );
		} elseif ( wc_check_if_attribute_name_is_reserved( $slug ) ) {
			return new WP_Error( 'woocommerce_rest_invalid_product_attribute_slug_reserved_name', sprintf( _e( 'Slug "%s" is not allowed because it is a reserved term. Change it, please.', 'woocommerce' ), $slug ), array( 'status' => 400 ) );
		} elseif ( $new_data && taxonomy_exists( wc_attribute_taxonomy_name( $slug ) ) ) {
			return new WP_Error( 'woocommerce_rest_invalid_product_attribute_slug_already_exists', sprintf( _e( 'Slug "%s" is already in use. Change it, please.', 'woocommerce' ), $slug ), array( 'status' => 400 ) );
		}

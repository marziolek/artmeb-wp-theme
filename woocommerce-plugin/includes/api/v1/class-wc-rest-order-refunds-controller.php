...
class WC_REST_Order_Refunds_V1_Controller extends WC_REST_Orders_V1_Controller {
	...
	public function prepare_item_for_response( $post, $request ) {
		...
		if ( ! $order ) {
			return new WP_Error( 'woocommerce_rest_invalid_order_id', _e( 'Invalid order ID.', 'woocommerce' ), 404 );
		}

		$refund = wc_get_order( $post );

		if ( ! $refund || $refund->get_parent_id() !== $order->get_id() ) {
			return new WP_Error( 'woocommerce_rest_invalid_order_refund_id', _e( 'Invalid order refund ID.', 'woocommerce' ), 404 );
		}
	...
	public function create_item( $request ) {
		...
		if ( empty( $order_data ) ) {
			return new WP_Error( 'woocommerce_rest_invalid_order', _e( 'Order is invalid', 'woocommerce' ), 400 );
		}

		if ( 0 > $request['amount'] ) {
			return new WP_Error( 'woocommerce_rest_invalid_order_refund', _e( 'Refund amount must be greater than zero.', 'woocommerce' ), 400 );
		}
		...
		if ( ! $refund ) {
			return new WP_Error( 'woocommerce_rest_cannot_create_order_refund', _e( 'Cannot create order refund, please try again.', 'woocommerce' ), 500 );
		}

...
class WC_REST_Product_Reviews_V1_Controller extends WC_REST_Controller {
	...
	public function update_item_permissions_check( $request ) {
		...
		if ( $post && ! wc_rest_check_post_permissions( 'product', 'edit', $post->ID ) ) {
			return new WP_Error( 'woocommerce_rest_cannot_edit', _e( 'Sorry, you cannot edit this resource.', 'woocommerce' ), array( 'status' => rest_authorization_required_code() ) );
		}
		return true;
	}
	...
	public function delete_item_permissions_check( $request ) {
		...
		if ( $post && ! wc_rest_check_post_permissions( 'product', 'delete', $post->ID ) ) {
			return new WP_Error( 'woocommerce_rest_cannot_edit', _e( 'Sorry, you cannot delete this resource.', 'woocommerce' ), array( 'status' => rest_authorization_required_code() ) );
		}
		return true;
	}
	...
	public function get_items( $request ) {
		...
		if ( 'product' !== get_post_type( $product_id ) ) {
			return new WP_Error( 'woocommerce_rest_product_invalid_id', _e( 'Invalid product ID.', 'woocommerce' ), array( 'status' => 404 ) );
		}
	...
	public function create_item( $request ) {
		...
		if ( ! $product_review_id ) {
			return new WP_Error( 'rest_product_review_failed_create', _e( 'Creating product review failed.', 'woocommerce' ), array( 'status' => 500 ) );
		}
	...
	public function update_item( $request ) {
		...
		if ( empty( $product_review_id ) || empty( $review ) || intval( $review->comment_post_ID ) !== $product_id ) {
			return new WP_Error( 'woocommerce_rest_product_review_invalid_id', _e( 'Invalid resource ID.', 'woocommerce' ), array( 'status' => 404 ) );
		}
		...
		if ( 0 === $updated ) {
			return new WP_Error( 'rest_product_review_failed_edit', _e( 'Updating product review failed.', 'woocommerce' ), array( 'status' => 500 ) );
		}
	...
	public function delete_item( $request ) {
		...
		if ( empty( $product_review_id ) || empty( $product_review->comment_ID ) || empty( $product_review->comment_post_ID ) ) {
			return new WP_Error( 'woocommerce_rest_product_review_invalid_id', _e( 'Invalid product review ID.', 'woocommerce' ), array( 'status' => 404 ) );
		}
		...
		if ( $force ) {
			...
			if ( ! $supports_trash ) {
				return new WP_Error( 'rest_trash_not_supported', _e( 'The product review does not support trashing.', 'woocommerce' ), array( 'status' => 501 ) );
			}

			if ( 'trash' === $product_review->comment_approved ) {
				return new WP_Error( 'rest_already_trashed', _e( 'The comment has already been trashed.', 'woocommerce' ), array( 'status' => 410 ) );
			}
		...
		if ( ! $result ) {
			return new WP_Error( 'rest_cannot_delete', _e( 'The product review cannot be deleted.', 'woocommerce' ), array( 'status' => 500 ) );
		}

...
class WC_REST_Webhooks_V1_Controller extends WC_REST_Posts_Controller {
	...
	public function create_item( $request ) {
		...
		if ( empty( $request['topic'] ) || ! wc_is_webhook_valid_topic( strtolower( $request['topic'] ) ) ) {
			return new WP_Error( "woocommerce_rest_{$this->post_type}_invalid_topic", _e( 'Webhook topic is required and must be valid.', 'woocommerce' ), array( 'status' => 400 ) );
		}

		// Validate delivery URL.
		if ( empty( $request['delivery_url'] ) || ! wc_is_valid_url( $request['delivery_url'] ) ) {
			return new WP_Error( "woocommerce_rest_{$this->post_type}_invalid_delivery_url", _e( 'Webhook delivery URL must be a valid URL starting with http:// or https://.', 'woocommerce' ), array( 'status' => 400 ) );
		}
	...
	public function update_item( $request ) {
		...
		if ( ! empty( $request['topic'] ) ) {
			if ( wc_is_webhook_valid_topic( strtolower( $request['topic'] ) ) ) {
				...
				return new WP_Error( "woocommerce_rest_{$this->post_type}_invalid_topic", _e( 'Webhook topic must be valid.', 'woocommerce' ), array( 'status' => 400 ) );
			}
		}
	...
	protected function prepare_item_for_database( $request ) {
		...
		if ( 'POST' === $request->get_method() && empty( $data->ID ) ) {
			...
			$data->post_title = ! empty( $request['name'] ) ? $request['name'] : sprintf( _e( 'Webhook created on %s', 'woocommerce' ), strftime( _x( '%b %d, %Y @ %I:%M %p', 'Webhook created on date parsed by strftime', 'woocommerce' ) ) );

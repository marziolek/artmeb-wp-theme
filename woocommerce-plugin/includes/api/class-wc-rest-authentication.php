...
class WC_REST_Authentication {
	...
	private function perform_basic_authentication() {
		...
		if ( ! hash_equals( $this->user->consumer_secret, $consumer_secret ) ) {
			$this->set_error( new WP_Error( 'woocommerce_rest_authentication_error', _e( 'Consumer secret is invalid.', 'woocommerce' ), array( 'status' => 401 ) ) );
	...
	private function perform_oauth_authentication() {
		...
		if ( empty( $this->user ) ) {
			$this->set_error( new WP_Error( 'woocommerce_rest_authentication_error', _e( 'Consumer key is invalid.', 'woocommerce' ), array( 'status' => 401 ) ) );
	...
	private function check_oauth_signature( $user, $params ) {
		...
		if ( ! uksort( $params, 'strcmp' ) ) {
			return new WP_Error( 'woocommerce_rest_authentication_error', _e( 'Invalid signature - failed to sort parameters.', 'woocommerce' ), array( 'status' => 401 ) );
		}
		...
		if ( 'HMAC-SHA1' !== $params['oauth_signature_method'] && 'HMAC-SHA256' !== $params['oauth_signature_method'] ) {
			return new WP_Error( 'woocommerce_rest_authentication_error', _e( 'Invalid signature - signature method is invalid.', 'woocommerce' ), array( 'status' => 401 ) );
		}
		...
		if ( ! hash_equals( $signature, $consumer_signature ) ) {
			return new WP_Error( 'woocommerce_rest_authentication_error', _e( 'Invalid signature - provided signature does not match.', 'woocommerce' ), array( 'status' => 401 ) );
		}
	...
	private function check_oauth_timestamp_and_nonce( $user, $timestamp, $nonce ) {
		...
		if ( ( $timestamp < time() - $valid_window ) || ( $timestamp > time() + $valid_window ) ) {
			return new WP_Error( 'woocommerce_rest_authentication_error', _e( 'Invalid timestamp.', 'woocommerce' ), array( 'status' => 401 ) );
		}
		...
		if ( in_array( $nonce, $used_nonces ) ) {
			return new WP_Error( 'woocommerce_rest_authentication_error', _e( 'Invalid nonce - nonce has already been used.', 'woocommerce' ), array( 'status' => 401 ) );
		}
		...
		switch ( $method ) {
			...
			case 'GET' :
				if ( 'read' !== $permissions && 'read_write' !== $permissions ) {
					return new WP_Error( 'woocommerce_rest_authentication_error', _e( 'The API key provided does not have read permissions.', 'woocommerce' ), array( 'status' => 401 ) );
				}
				break;
			case 'POST' :
			case 'PUT' :
			...
			case 'DELETE' :
				if ( 'write' !== $permissions && 'read_write' !== $permissions ) {
					return new WP_Error( 'woocommerce_rest_authentication_error', _e( 'The API key provided does not have write permissions.', 'woocommerce' ), array( 'status' => 401 ) );
				}
				break;

			default :
				return new WP_Error( 'woocommerce_rest_authentication_error', _e( 'Unknown request method.', 'woocommerce' ), array( 'status' => 401 ) );
		}
	...
	public function send_unauthorized_headers( $response ) {
		if ( is_wp_error( $this->get_error() ) && 'basic_auth' === $this->auth_method ) {
			$auth_message = _e( 'WooCommerce API. Use a consumer key in the username field and a consumer secret in the password field.', 'woocommerce' );

...
class WC_REST_System_Status_Tools_Controller extends WC_REST_Controller {
		...
		$tools = array(
			...
			'clear_sessions' => array(
				...
				'desc'    => sprintf(
					...
					_e( 'Note:', 'woocommerce' ),
					_e( 'This tool will delete all customer session data from the database, including any current live carts.', 'woocommerce' )
				),
			),
			'install_pages' => array(
				...
				'desc'    => sprintf(
					'<strong class="red">%1$s</strong> %2$s',
					_e( 'Note:', 'woocommerce' ),
					_e( 'This tool will install all the missing WooCommerce pages. Pages already defined and set up will not be replaced.', 'woocommerce' )
				),
			),
			'delete_taxes' => array(
				...
				'desc'    => sprintf(
					'<strong class="red">%1$s</strong> %2$s',
					_e( 'Note:', 'woocommerce' ),
					_e( 'This option will delete ALL of your tax rates, use with caution.', 'woocommerce' )
				),
			),
	...
	public function get_item( $request ) {
		...
		if ( empty( $tools[ $request['id'] ] ) ) {
			return new WP_Error( 'woocommerce_rest_system_status_tool_invalid_id', _e( 'Invalid tool ID.', 'woocommerce' ), array( 'status' => 404 ) );
		}
	...
	public function execute_tool( $tool ) {
		...
		switch ( $tool ) {
			case 'clear_transients' :
				...
				$message = _e( 'Product transients cleared', 'woocommerce' );
			break;
			case 'clear_expired_transients' :
				...
				$message = sprintf( _e( '%d transients rows cleared', 'woocommerce' ), $rows + $rows2 );
			break;
			case 'delete_orphaned_variations' :
				...
				$message = sprintf( _e( '%d orphaned variations deleted', 'woocommerce' ), $result );
			break;
			case 'reset_roles' :
				// Remove then re-add caps and roles
				WC_Install::remove_roles();
				WC_Install::create_roles();
				$message = _e( 'Roles successfully reset', 'woocommerce' );
			break;
			case 'recount_terms' :
				...
				$message = _e( 'Terms successfully recounted', 'woocommerce' );
			break;
			case 'clear_sessions' :
				$wpdb->query( "TRUNCATE {$wpdb->prefix}woocommerce_sessions" );
				wp_cache_flush();
				$message = _e( 'Sessions successfully cleared', 'woocommerce' );
			break;
			case 'install_pages' :
				WC_Install::create_pages();
				$message = _e( 'All missing WooCommerce pages successfully installed', 'woocommerce' );
			break;
			case 'delete_taxes' :
				...
				$message = _e( 'Tax rates successfully deleted', 'woocommerce' );
			break;
			case 'reset_tracking' :
				delete_option( 'woocommerce_allow_tracking' );
				WC_Admin_Notices::add_notice( 'tracking' );
				$message = _e( 'Usage tracking settings successfully reset.', 'woocommerce' );
			break;
			default :
				$tools = $this->get_tools();
				if ( isset( $tools[ $tool ]['callback'] ) ) {
					...
					if ( is_string( $return ) ) {
						...
						$ran = false;
						$message = sprintf( _e( 'There was an error calling %s', 'woocommerce' ), $callback_string );
					} else {
						$message = _e( 'Tool ran.', 'woocommerce' );
					}
				} else {
					$ran     = false;
					$message = _e( 'There was an error calling this tool. There is no callback present.', 'woocommerce' );
				}
			break;
		}

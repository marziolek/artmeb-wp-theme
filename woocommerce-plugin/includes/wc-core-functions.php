...
function wc_update_order( $args ) {
	if ( ! $args['order_id'] ) {
		return new WP_Error( _e( 'Invalid order ID.', 'woocommerce' ) );
	}
...
function wc_get_template( $template_name, $args = array(), $template_path = '', $default_path = '' ) {
	...
	if ( ! file_exists( $located ) ) {
		wc_doing_it_wrong( __FUNCTION__, sprintf( _e( '%s does not exist.', 'woocommerce' ), '<code>' . $located . '</code>' ), '2.1' );
		return;
	}
	...
	if ( null === $logger ) {
		...
		if ( is_array( $implements ) && in_array( 'WC_Logger_Interface', $implements ) ) {
			...
			wc_doing_it_wrong(
				__FUNCTION__,
				sprintf(
					_e( 'The class <code>%s</code> provided by woocommerce_logging_class filter must implement <code>WC_Logger_Interface</code>.', 'woocommerce' ),

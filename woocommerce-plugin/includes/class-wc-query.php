...
class WC_Query {
	...
	public function get_endpoint_title( $endpoint ) {
		...
		switch ( $endpoint ) {
			case 'order-pay' :
				$title = _e( 'Pay for order', 'woocommerce' );
			break;
			case 'order-received' :
				$title = _e( 'Order received', 'woocommerce' );
			break;
			case 'orders' :
				if ( ! empty( $wp->query_vars['orders'] ) ) {
					/* translators: %s: page */
					$title = sprintf( _e( 'Orders (page %d)', 'woocommerce' ), intval( $wp->query_vars['orders'] ) );
				} else {
					$title = _e( 'Orders', 'woocommerce' );
				}
			break;
			case 'view-order' :
				$order = wc_get_order( $wp->query_vars['view-order'] );
				/* translators: %s: order number */
				$title = ( $order ) ? sprintf( _e( 'Order #%s', 'woocommerce' ), $order->get_order_number() ) : '';
			break;
			case 'downloads' :
				$title = _e( 'Downloads', 'woocommerce' );
			break;
			case 'edit-account' :
				$title = _e( 'Account details', 'woocommerce' );
			break;
			case 'edit-address' :
				$title = _e( 'Addresses', 'woocommerce' );
			break;
			case 'payment-methods' :
				$title = _e( 'Payment methods', 'woocommerce' );
			break;
			case 'add-payment-method' :
				$title = _e( 'Add payment method', 'woocommerce' );
			break;
			case 'lost-password' :
				$title = _e( 'Lost password', 'woocommerce' );
			break;
			default :
				$title = '';
			break;

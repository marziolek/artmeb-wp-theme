...
class WC_Webhook {
	...
	public function get_new_delivery_id() {
		...
		$comment_data = apply_filters( 'woocommerce_new_webhook_delivery_data', array(
			...
			'comment_author_email' => sanitize_email( sprintf( '%s@%s', strtolower( _e( 'WooCommerce', 'woocommerce' ) ), isset( $_SERVER['HTTP_HOST'] ) ? str_replace( 'www.', '', $_SERVER['HTTP_HOST'] ) : 'noreply.com' ) ),
	...
	public function deliver_ping() {
		...
		if ( is_wp_error( $test ) ) {
			return new WP_Error( 'error', sprintf( _e( 'Error: Delivery URL cannot be reached: %s', 'woocommerce' ), $test->get_error_message() ) );
		}

		if ( 200 !== $response_code ) {
			return new WP_Error( 'error', sprintf( _e( 'Error: Delivery URL returned response code: %s', 'woocommerce' ), absint( $response_code ) ) );
		}

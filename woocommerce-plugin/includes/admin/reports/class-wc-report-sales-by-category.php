...
class WC_Report_Sales_By_Category extends WC_Admin_Report {
	...
	public function get_chart_legend() {
		...
		foreach ( $this->show_categories as $category ) {
			...
			$legend[] = array(
				...
				'title'            => sprintf( _e( '%1$s sales in %2$s', 'woocommerce' ), '<strong>' . wc_price( $total ) . '</strong>', $category->name ),
	...
	public function category_widget() {
		...
		<form method="GET">
			<div>
				<select multiple="multiple" data-placeholder="<?php esc_attr__( 'Select categories&hellip;', 'woocommerce' ); ?>" class="wc-enhanced-select" id="show_categories" name="show_categories[]" style="width: 205px;">
					<?php
						$r = array();
						$r['pad_counts'] 	= 1;
						$r['hierarchical'] 	= 1;

...
class WC_Report_Sales_By_Product extends WC_Admin_Report {
	...
	public function get_chart_legend() {
		...
		$legend[] = array(
			...
			'title' => sprintf( _e( '%s sales for the selected items', 'woocommerce' ), '<strong>' . wc_price( $total_sales ) . '</strong>' ),
		...
		$legend[] = array(
			...
			'title' => sprintf( _e( '%s purchases for the selected items', 'woocommerce' ), '<strong>' . ( $total_items ) . '</strong>' ),
	...
	public function current_filters() {
		...
		echo '<p><a class="button" href="' . esc_url( remove_query_arg( 'product_ids' ) ) . '">' . _e( 'Reset', 'woocommerce' ) . '</a></p>';
	}
	...
	public function products_widget() {
		...
		<div class="section">
			<form method="GET">
				<div>
					<select class="wc-product-search" style="width:203px;" multiple="multiple" id="product_ids" name="product_ids[]" data-placeholder="<?php esc_attr__( 'Search for a product&hellip;', 'woocommerce' ); ?>" data-action="woocommerce_json_search_products_and_variations"></select>
					<input type="submit" class="submit button" value="<?php esc_attr__( 'Show', 'woocommerce' ); ?>" />
		...
		<div class="section">
			<table cellspacing="0">
				...
				if ( $top_sellers ) {
					...
					echo '<tr><td colspan="3">' . _e( 'No products found in range', 'woocommerce' ) . '</td></tr>';
				}
				?>
			</table>
		</div>
				...
				jQuery(function(){
					...
					var drawGraph = function( highlight ) {
						...
						var series = [
							...
							{
								label: "<?php echo esc_js( _e( 'Sales amount', 'woocommerce' ) ) ?>",

...
class WC_Report_Customer_List extends WP_List_Table {
	...
	public function output_report() {
		...
		if ( ! empty( $_GET['refresh'] ) && wp_verify_nonce( $_REQUEST['_wpnonce'], 'refresh' ) ) {
			...
			echo '<div class="updated"><p>' . sprintf( _e( 'Refreshed stats for %s', 'woocommerce' ), $user->display_name ) . '</p></div>';
		}

		echo '<form method="post" id="woocommerce_customers">';

		$this->search_box( _e( 'Search customers', 'woocommerce' ), 'customer_search' );
		$this->display();

	...
	public function admin_scripts() {
		...
		if ( in_array( $screen_id, wc_get_screen_ids() ) ) {
			...
			$params = array(
				...
				'i18n_decimal_error'                => sprintf( _e( 'Please enter in decimal (%s) format without thousand separators.', 'woocommerce' ), $decimal ),
				/* translators: %s: price decimal separator */
				'i18n_mon_decimal_error'            => sprintf( _e( 'Please enter in monetary decimal (%s) format without thousand separators and currency symbols.', 'woocommerce' ), wc_get_price_decimal_separator() ),
		...
		if ( in_array( $screen_id, array( 'edit-product' ) ) ) {
			...
			$params = array(
				'strings' => array(
					'allow_reviews' => esc_js( _e( 'Enable reviews', 'woocommerce' ) ),
				),
			);
		...
		if ( in_array( $screen_id, array( 'product', 'edit-product' ) ) ) {
			...
			$params = array(
				...
				'i18n_link_all_variations'            => esc_js( sprintf( _e( 'Are you sure you want to link all variations? This will create a new variation for each and every possible combination of variation attributes (max %d per run).', 'woocommerce' ), defined( 'WC_MAX_LINKED_VARIATIONS' ) ? WC_MAX_LINKED_VARIATIONS : 50 ) ),
				'i18n_enter_a_value'                  => esc_js( _e( 'Enter a value', 'woocommerce' ) ),
				'i18n_enter_menu_order'               => esc_js( _e( 'Variation menu order (determines position in the list of variations)', 'woocommerce' ) ),
				'i18n_enter_a_value_fixed_or_percent' => esc_js( _e( 'Enter a value (fixed or %)', 'woocommerce' ) ),
				'i18n_delete_all_variations'          => esc_js( _e( 'Are you sure you want to delete all variations? This cannot be undone.', 'woocommerce' ) ),
				'i18n_last_warning'                   => esc_js( _e( 'Last warning, are you sure?', 'woocommerce' ) ),
				'i18n_choose_image'                   => esc_js( _e( 'Choose an image', 'woocommerce' ) ),
				'i18n_set_image'                      => esc_js( _e( 'Set variation image', 'woocommerce' ) ),
				'i18n_variation_added'                => esc_js( _e( "variation added", 'woocommerce' ) ),
				'i18n_variations_added'               => esc_js( _e( "variations added", 'woocommerce' ) ),
				'i18n_no_variations_added'            => esc_js( _e( "No variations added", 'woocommerce' ) ),
				'i18n_remove_variation'               => esc_js( _e( 'Are you sure you want to remove this variation?', 'woocommerce' ) ),
				'i18n_scheduled_sale_start'           => esc_js( _e( 'Sale start date (YYYY-MM-DD format or leave blank)', 'woocommerce' ) ),
				'i18n_scheduled_sale_end'             => esc_js( _e( 'Sale end date (YYYY-MM-DD format or leave blank)', 'woocommerce' ) ),
				'i18n_edited_variations'              => esc_js( _e( 'Save changes before changing page?', 'woocommerce' ) ),
				'i18n_variation_count_single'         => esc_js( _e( '%qty% variation', 'woocommerce' ) ),
				'i18n_variation_count_plural'         => esc_js( _e( '%qty% variations', 'woocommerce' ) ),
		...
		if ( $wc_screen_id . '_page_wc-settings' === $screen_id && isset( $_GET['section'] ) && 'keys' == $_GET['section'] ) {
			...
			wp_localize_script(
				...
				array(
					...
					'clipboard_failed' => esc_html_e( 'Copying to clipboard failed. Please press Ctrl/Cmd+C to copy.', 'woocommerce' ),
				)
			);
		}
		...
		if ( $wc_screen_id . '_page_wc-status' === $screen_id ) {
			...
			wp_localize_script(
				...
				array(
					'delete_log_confirmation' => esc_js( _e( 'Are you sure you want to delete this log?', 'woocommerce' ) ),
				)
			);
		}

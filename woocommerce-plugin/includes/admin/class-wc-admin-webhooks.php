...
class WC_Admin_Webhooks {
	...
	private function update_name( $webhook_id ) {
		...
		/* translators: %s: date` */
		$name = ! empty( $_POST['webhook_name'] ) ? $_POST['webhook_name'] : sprintf( _e( 'Webhook created on %s', 'woocommerce' ), strftime( _x( '%b %d, %Y @ %I:%M %p', 'Webhook created on date parsed by strftime', 'woocommerce' ) ) );
	...
	private function save() {
		if ( empty( $_REQUEST['_wpnonce'] ) || ! wp_verify_nonce( $_REQUEST['_wpnonce'], 'woocommerce-settings' ) ) {
			wp_die( _e( 'Action failed. Please refresh the page and retry.', 'woocommerce' ) );
		}
	...
	private function create() {
		...
		if ( ! current_user_can( 'publish_shop_webhooks' ) ) {
			wp_die( _e( 'You do not have permissions to create Webhooks!', 'woocommerce' ) );
		}
		...
		$webhook_id = wp_insert_post( array(
			...
			// @codingStandardsIgnoreStart
			/* translators: %s: date */
			'post_title'    => sprintf( _e( 'Webhook created on %s', 'woocommerce' ), strftime( _x( '%b %d, %Y @ %I:%M %p', 'Webhook created on date parsed by strftime', 'woocommerce' ) ) ),
			// @codingStandardsIgnoreEnd
			'comment_status' => 'open',
		) );
	...
	private function bulk_actions() {
		...
		if ( ! current_user_can( 'edit_shop_webhooks' ) ) {
			wp_die( _e( 'You do not have permissions to edit Webhooks!', 'woocommerce' ) );
		}
	...
	private function empty_trash() {
		...
		if ( ! current_user_can( 'delete_shop_webhooks' ) ) {
			wp_die( _e( 'You do not have permissions to delete Webhooks!', 'woocommerce' ) );
		}
	...
	public static function notices() {
		...
		if ( isset( $_GET['updated'] ) ) {
			WC_Admin_Settings::add_message( _e( 'Webhook updated successfully.', 'woocommerce' ) );
		}

		if ( isset( $_GET['created'] ) ) {
			WC_Admin_Settings::add_message( _e( 'Webhook created successfully.', 'woocommerce' ) );
		}
	...
	private static function table_list_output() {
		echo '<h2>' . _e( 'Webhooks', 'woocommerce' ) . ' <a href="' . esc_url( wp_nonce_url( admin_url( 'admin.php?page=wc-settings&tab=api&section=webhooks&create-webhook=1' ), 'create-webhook' ) ) . '" class="add-new-h2">' . _e( 'Add webhook', 'woocommerce' ) . '</a></h2>';
		...
		if ( absint( $count ) && $count > 0 ) {
			...
			$webhooks_table_list->search_box( _e( 'Search webhooks', 'woocommerce' ), 'webhook' );
	...
	public static function logs_output( $webhook ) {
		...
		if ( $logs ) {
			...
			echo '<p>' . _e( 'This Webhook has no log yet.', 'woocommerce' ) . '</p>';
		}
	}
	...
	public static function get_logs_navigation( $total, $webhook ) {
		...
		$html = '<div class="webhook-logs-navigation">';
			...
			$html .= sprintf(
				_e( '%1$s &ndash; Page %2$d of %3$d', 'woocommerce' ),
			...
			if ( 1 < $pages ) {
				$html .= '<p class="tools" style="float: right;">';
					if ( 1 == $current ) {
						$html .= '<button class="button-primary" disabled="disabled">' . _e( '&lsaquo; Previous', 'woocommerce' ) . '</button> ';
					} else {
						$html .= '<a class="button-primary" href="' . admin_url( 'admin.php?page=wc-settings&tab=api&section=webhooks&edit-webhook=' . $webhook->id . '&log_page=' . ( $current - 1 ) ) . '#webhook-logs">' . _e( '&lsaquo; Previous', 'woocommerce' ) . '</a> ';
					}

					if ( $pages == $current ) {
						$html .= '<button class="button-primary" disabled="disabled">' . _e( 'Next &rsaquo;', 'woocommerce' ) . '</button>';
					} else {
						$html .= '<a class="button-primary" href="' . admin_url( 'admin.php?page=wc-settings&tab=api&section=webhooks&edit-webhook=' . $webhook->id . '&log_page=' . ( $current + 1 ) ) . '#webhook-logs">' . _e( 'Next &rsaquo;', 'woocommerce' ) . '</a>';
					}
				$html .= '</p>';
			}

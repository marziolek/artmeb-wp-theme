...
class WC_Tax_Rate_Importer extends WP_Importer {
	...
	public function import( $file ) {
		if ( ! is_file( $file ) ) {
			$this->import_error( _e( 'The file does not exist, please try again.', 'woocommerce' ) );
		}
		...
		if ( ( $handle = fopen( $file, "r" ) ) !== false ) {
			...
			if ( 10 === sizeof( $header ) ) {
				...
				$this->import_error( _e( 'The CSV is invalid.', 'woocommerce' ) );
			}
		...
		printf(
			_e( 'Import complete - imported %s tax rates.', 'woocommerce' ),
			'<strong>' . $loop . '</strong>'
		);
		echo '</p></div>';
	...
	public function import_end() {
		echo '<p>' . _e( 'All done!', 'woocommerce' ) . ' <a href="' . admin_url( 'admin.php?page=wc-settings&tab=tax' ) . '">' . _e( 'View tax rates', 'woocommerce' ) . '</a>' . '</p>';
	...
	public function header() {
		echo '<div class="wrap">';
		echo '<h1>' . _e( 'Import tax rates', 'woocommerce' ) . '</h1>';
	}
	...
	public function greet() {
		...
		echo '<p>' . _e( 'Hi there! Upload a CSV file containing tax rates to import the contents into your shop. Choose a .csv file to upload, then click "Upload file and import".', 'woocommerce' ) . '</p>';

		echo '<p>' . sprintf( _e( 'Tax rates need to be defined with columns in a specific order (10 columns). <a href="%s">Click here to download a sample</a>.', 'woocommerce' ), WC()->plugin_url() . '/dummy-data/sample_tax_rates.csv' ) . '</p>';
		...
		if ( ! empty( $upload_dir['error'] ) ) :
			...
			<form enctype="multipart/form-data" id="import-upload-form" method="post" action="<?php echo esc_attr( wp_nonce_url( $action, 'import-upload' ) ); ?>">
				...
				<p class="submit">
					<input type="submit" class="button" value="<?php esc_attr__( 'Upload file and import', 'woocommerce' ); ?>" />
				</p>
			</form>
			<?php
		endif;
	...
	private function import_error( $message = '' ) {
		echo '<p><strong>' . _e( 'Sorry, there has been an error.', 'woocommerce' ) . '</strong><br />';
		if ( $message ) {

...
class WC_Product_CSV_Importer_Controller {
	...
	public function handle_upload() {
		...
		if ( empty( $_POST['file_url'] ) ) {
			if ( ! isset( $_FILES['import'] ) ) {
				return new WP_Error( 'woocommerce_product_csv_importer_upload_file_empty', _e( 'File is empty. Please upload something more substantial. This error could also be caused by uploads being disabled in your php.ini or by post_max_size being defined as smaller than upload_max_filesize in php.ini.', 'woocommerce' ) );
			}

			$filetype = wp_check_filetype( $_FILES['import']['name'], $valid_filetypes );
			if ( ! in_array( $filetype['type'], $valid_filetypes ) ) {
				return new WP_Error( 'woocommerce_product_csv_importer_upload_file_invalid', _e( 'Invalid file type. The importer supports CSV and TXT file formats.', 'woocommerce' ) );
			}
		...
		return new WP_Error( 'woocommerce_product_csv_importer_upload_invalid_file', _e( 'Please upload or provide the link to a valid CSV file.', 'woocommerce' ) );
	}
	...
	protected function mapping_form() {
		...
		if ( empty( $sample ) ) {
			$this->add_error( _e( 'The file is empty, please try again with a new file.', 'woocommerce' ) );
			return;
		}
	...
	public function import() {
		if ( ! is_file( $this->file ) ) {
			$this->add_error( _e( 'The file does not exist, please try again.', 'woocommerce' ) );
			return;
		}
	...
	protected function auto_map_columns( $raw_headers, $num_indexes = true ) {
		...
		$default_columns = apply_filters( 'woocommerce_csv_product_import_mapping_default_columns', array(
			_e( 'ID', 'woocommerce' )                                      => 'id',
			_e( 'Type', 'woocommerce' )                                    => 'type',
			_e( 'SKU', 'woocommerce' )                                     => 'sku',
			_e( 'Name', 'woocommerce' )                                    => 'name',
			_e( 'Published', 'woocommerce' )                               => 'published',
			_e( 'Is featured?', 'woocommerce' )                            => 'featured',
			_e( 'Visibility in catalog', 'woocommerce' )                   => 'catalog_visibility',
			_e( 'Short description', 'woocommerce' )                       => 'short_description',
			_e( 'Description', 'woocommerce' )                             => 'description',
			_e( 'Date sale price starts', 'woocommerce' )                  => 'date_on_sale_from',
			_e( 'Date sale price ends', 'woocommerce' )                    => 'date_on_sale_to',
			_e( 'Tax status', 'woocommerce' )                              => 'tax_status',
			_e( 'Tax class', 'woocommerce' )                               => 'tax_class',
			_e( 'In stock?', 'woocommerce' )                               => 'stock_status',
			_e( 'Stock', 'woocommerce' )                                   => 'stock_quantity',
			_e( 'Backorders allowed?', 'woocommerce' )                     => 'backorders',
			_e( 'Sold individually?', 'woocommerce' )                      => 'sold_individually',
			sprintf( _e( 'Weight (%s)', 'woocommerce' ), $weight_unit )    => 'weight',
			sprintf( _e( 'Length (%s)', 'woocommerce' ), $dimension_unit ) => 'length',
			sprintf( _e( 'Width (%s)', 'woocommerce' ), $dimension_unit )  => 'width',
			sprintf( _e( 'Height (%s)', 'woocommerce' ), $dimension_unit ) => 'height',
			_e( 'Allow customer reviews?', 'woocommerce' )                 => 'reviews_allowed',
			_e( 'Purchase note', 'woocommerce' )                           => 'purchase_note',
			_e( 'Sale price', 'woocommerce' )                              => 'sale_price',
			_e( 'Regular price', 'woocommerce' )                           => 'regular_price',
			_e( 'Categories', 'woocommerce' )                              => 'category_ids',
			_e( 'Tags', 'woocommerce' )                                    => 'tag_ids',
			_e( 'Shipping class', 'woocommerce' )                          => 'shipping_class_id',
			_e( 'Images', 'woocommerce' )                                  => 'images',
			_e( 'Download limit', 'woocommerce' )                          => 'download_limit',
			_e( 'Download expiry days', 'woocommerce' )                    => 'download_expiry',
			_e( 'Parent', 'woocommerce' )                                  => 'parent_id',
			_e( 'Upsells', 'woocommerce' )                                 => 'upsell_ids',
			_e( 'Cross-sells', 'woocommerce' )                             => 'cross_sell_ids',
			_e( 'Grouped products', 'woocommerce' )                        => 'grouped_products',
			_e( 'External URL', 'woocommerce' )                            => 'product_url',
			_e( 'Button text', 'woocommerce' )                             => 'button_text',
		) );

		$special_columns = $this->get_special_columns( apply_filters( 'woocommerce_csv_product_import_mapping_special_columns',
			array(
				_e( 'Attribute %d name', 'woocommerce' )     => 'attributes:name',
				_e( 'Attribute %d value(s)', 'woocommerce' ) => 'attributes:value',
				_e( 'Attribute %d visible', 'woocommerce' )  => 'attributes:visible',
				_e( 'Attribute %d global', 'woocommerce' )   => 'attributes:taxonomy',
				_e( 'Attribute %d default', 'woocommerce' )  => 'attributes:default',
				_e( 'Download %d name', 'woocommerce' )      => 'downloads:name',
				_e( 'Download %d URL', 'woocommerce' )       => 'downloads:url',
				_e( 'Meta: %s', 'woocommerce' )              => 'meta:',
			)
		) );
	...
	protected function get_mapping_options( $item = '' ) {
		...
		$options        = array(
			...
			'weight'             => sprintf( _e( 'Weight (%s)', 'woocommerce' ), $weight_unit ),
			'dimensions'         => array(
				'name'    => __( 'Dimensions', 'woocommerce' ),
				'options' => array(
					/* translators: %s: dimension unit */
					'length'             => sprintf( _e( 'Length (%s)', 'woocommerce' ), $dimension_unit ),
					/* translators: %s: dimension unit */
					'width'              => sprintf( _e( 'Width (%s)', 'woocommerce' ), $dimension_unit ),
					/* translators: %s: dimension unit */
					'height'             => sprintf( _e( 'Height (%s)', 'woocommerce' ), $dimension_unit ),
				),
			),

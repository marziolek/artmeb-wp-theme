...
class WC_Admin_Attributes {
	...
	private static function valid_attribute_name( $attribute_name ) {
		if ( strlen( $attribute_name ) > 28 ) {
			...
			return new WP_Error( 'error', sprintf( _e( 'Slug "%s" is too long (28 characters max). Shorten it, please.', 'woocommerce' ), sanitize_title( $attribute_name ) ) );
		} elseif ( wc_check_if_attribute_name_is_reserved( $attribute_name ) ) {
			/* translators: %s: attribute name */
			return new WP_Error( 'error', sprintf( _e( 'Slug "%s" is not allowed because it is a reserved term. Change it, please.', 'woocommerce' ), sanitize_title( $attribute_name ) ) );
		}
	...
	private static function process_add_attribute() {
		...
		if ( empty( $attribute['attribute_name'] ) || empty( $attribute['attribute_label'] ) ) {
			return new WP_Error( 'error', _e( 'Please, provide an attribute name and slug.', 'woocommerce' ) );
			...
			/* translators: %s: attribute name */
			return new WP_Error( 'error', sprintf( _e( 'Slug "%s" is already in use. Change it, please.', 'woocommerce' ), sanitize_title( $attribute['attribute_name'] ) ) );
		}
	...
	private static function process_edit_attribute() {
		...
		echo '<div class="updated"><p>' . _e( 'Attribute updated successfully', 'woocommerce' ) . '</p><p><a href="' . esc_url( admin_url( 'edit.php?post_type=product&amp;page=product_attributes' ) ) . '">' . _e( 'Back to Attributes', 'woocommerce' ) . '</a></p></div>';
		...
		<div class="wrap woocommerce">
			...
			<?php
				...
				if ( ! $attribute_to_edit ) {
					echo '<div id="woocommerce_errors" class="error"><p>' . _e( 'Error: non-existing attribute ID.', 'woocommerce' ) . '</p></div>';
				} else {
				...
				<form action="edit.php?post_type=product&amp;page=product_attributes&amp;edit=<?php echo absint( $edit ); ?>" method="post">
					...
					<p class="submit"><input type="submit" name="save_attribute" id="submit" class="button-primary" value="<?php esc_attr__( 'Update', 'woocommerce' ); ?>"></p>
									...
									if ( $attribute_taxonomies = wc_get_attribute_taxonomies() ) :
										foreach ( $attribute_taxonomies as $tax ) :
											?><tr>
												...
												<td><?php echo esc_html( wc_get_attribute_type_label( $tax->attribute_type ) ); ?> <?php echo $tax->attribute_public ? _e( '(Public)', 'woocommerce' ) : ''; ?></td>
												<td><?php
					...
					<div class="col-wrap">
						<div class="form-wrap">
							...
							<form action="edit.php?post_type=product&amp;page=product_attributes" method="post">
								...
								<p class="submit"><input type="submit" name="add_new_attribute" id="submit" class="button button-primary" value="<?php esc_attr__( 'Add attribute', 'woocommerce' ); ?>"></p>

...
class WC_Helper {
	...
	public static function render_helper_output() {
		...
		foreach ( $subscriptions as &$subscription ) {
			...
			if ( $subscription['has_update'] && ! $subscription['expired'] ) {
				$action = array(
					...
					'message' => sprintf( _e( 'Version %s is <strong>available</strong>.', 'woocommerce' ), esc_html( $updates[ $subscription['product_id'] ]['version'] ) ),
				...
				if ( ! $subscription['active'] ) {
					$action['message'] .= ' ' . _e( 'To enable this update you need to <strong>activate</strong> this subscription.', 'woocommerce' );
			...
			if ( $subscription['has_update'] && $subscription['expired'] ) {
				...
				$action['message'] .= ' ' . _e( 'To enable this update you need to <strong>purchase</strong> a new subscription.', 'woocommerce' );
				$action['button_label'] = _e( 'Purchase', 'woocommerce' );
				$action['button_url'] = $subscription['product_url'];

				$subscription['actions'][] = $action;
			} elseif ( $subscription['expired'] && ! empty( $subscription['master_user_email'] ) ) {
				$action = array(
					'message' => sprintf( _e( 'This subscription has expired. Contact the owner to <strong>renew</strong> the subscription to receive updates and support.', 'woocommerce' ) ),
					'status' => 'expired',
					'icon' => 'dashicons-info',
				);
				...
				$action = array(
					'message' => sprintf( _e( 'This subscription has expired. Please <strong>renew</strong> to receive updates and support.', 'woocommerce' ) ),
			...
			if ( $subscription['expiring'] && ! $subscription['autorenew'] ) {
				...
				$action = array(
					'message' => sprintf( _e( 'This subscription is expiring soon. Please <strong>renew</strong> to continue receiving updates and support.', 'woocommerce' ) ),
					'button_label' => __( 'Renew', 'woocommerce' ),
					'button_url' => 'https://woocommerce.com/my-account/my-subscriptions/',
					'status' => 'expired',
					'icon' => 'dashicons-info',
		...
		foreach ( array_merge( $woo_plugins, $woo_themes ) as $filename => $data ) {
			...
			if ( $data['_has_update'] ) {
				$action = array(
					'message' => sprintf( _e( 'Version %s is <strong>available</strong>. To enable this update you need to <strong>purchase</strong> a new subscription.', 'woocommerce' ), esc_html( $updates[ $data['_product_id'] ]['version'] ) ),
				...
				$action = array(
					'message' => sprintf( _e( 'To receive updates and support for this extension, you need to <strong>purchase</strong> a new subscription or <a href="%1$s" target="_blank">be added as a collaborator</a>.', 'woocommerce' ), 'https://docs.woocommerce.com/document/adding-collaborators/' ),
					'button_label' => __( 'Purchase', 'woocommerce' ),
					'button_url' => $data['_product_url'],
					'status' => 'expired',
					'icon' => 'dashicons-info',
	...
	public static function admin_enqueue_scripts() {
		...
		$wc_screen_id = sanitize_title( _e( 'WooCommerce', 'woocommerce' ) );
		...
		switch ( $return_status ) {
			case 'activate-success':
				...
				$notices[] = array(
					...
					'message' => sprintf( _e( '%s activated successfully. You will now receive updates for this product.', 'woocommerce' ),
			...
			case 'activate-error':
				...
				$notices[] = array(
					'type' => 'error',
					/* translators: %s: product name */
					'message' => sprintf( _e( 'An error has occurred when activating %s. Please try again later.', 'woocommerce' ),
						'<strong>' . esc_html( $subscription['product_name'] ) . '</strong>' ),
				);
				break;
			...
			case 'deactivate-success':
				...
				/* translators: %s: product name */
				$message = sprintf( _e( 'Subscription for %s deactivated successfully. You will no longer receive updates for this product.', 'woocommerce' ),
					'<strong>' . esc_html( $subscription['product_name'] ) . '</strong>' );
				...
				if ( $local && is_plugin_active( $local['_filename'] ) && current_user_can( 'activate_plugins' ) ) {
					...
					$message = sprintf( _e( 'Subscription for %1$s deactivated successfully. You will no longer receive updates for this product. <a href="%2$s">Click here</a> if you wish to deactive the plugin as well.', 'woocommerce' ),
			...
			case 'deactivate-error':
				...
				$notices[] = array(
					'type' => 'error',
					/* translators: %s: product name */
					'message' => sprintf( _e( 'An error has occurred when deactivating the subscription for %s. Please try again later.', 'woocommerce' ),
						'<strong>' . esc_html( $subscription['product_name'] ) . '</strong>' ),
				);
				break;
			...
			case 'deactivate-plugin-success':
				...
				$notices[] = array(
					'type' => 'updated',
					/* translators: %s: product name */
					'message' => sprintf( _e( 'The extension %s has been deactivated successfully.', 'woocommerce' ),
						'<strong>' . esc_html( $subscription['product_name'] ) . '</strong>' ),
				);
				break;
			...
			case 'deactivate-plugin-error':
				...
				$notices[] = array(
					...
					'message' => sprintf( _e( 'An error has occurred when deactivating the extension %1$s. Please proceed to the <a href="%2$s">Plugins screen</a> to deactivate it manually.', 'woocommerce' ),

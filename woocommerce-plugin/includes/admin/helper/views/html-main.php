						...
						<div class="wp-list-table__ext-description">
							...
							<?php
								if ( ! $subscription['active'] && $subscription['maxed'] ) {
									...
									printf( _e( 'Subscription: Not available - %1$d of %2$d already in use', 'woocommerce' ), absint( $subscription['sites_active'] ), absint( $subscription['sites_max'] ) );
								} elseif ( $subscription['sites_max'] > 0 ) {
									/* translators: %1$d: sites active, %2$d max sites active */
									printf( _e( 'Subscription: Using %1$d of %2$d sites available', 'woocommerce' ), absint( $subscription['sites_active'] ), absint( $subscription['sites_max'] ) );
								} else {
									_e( 'Subscription: Unlimited', 'woocommerce' );
								}
								if ( isset( $subscription['master_user_email'] ) ) {
									printf( '</br>' . _e( 'Shared by %s', 'woocommerce' ), esc_html( $subscription['master_user_email'] ) );
								}
							?>
							</span>
						</div>

...
class WC_Admin_API_Keys_Table_List extends WP_List_Table {
	...
	public function column_description( $key ) {
		...
		if ( empty( $key['description'] ) ) {
			$output .= esc_html_e( 'API key', 'woocommerce' );
		} else {
		...
		// Get actions
		$actions = array(
			'id'    => sprintf( _e( 'ID: %d', 'woocommerce' ), $key['key_id'] ),
			'edit'  => '<a href="' . esc_url( $url ) . '">' . _e( 'View/Edit', 'woocommerce' ) . '</a>',
			'trash' => '<a class="submitdelete" aria-label="' . esc_attr__( 'Revoke API key', 'woocommerce' ) . '" href="' . esc_url( wp_nonce_url( add_query_arg( array( 'revoke-key' => $key['key_id'] ), admin_url( 'admin.php?page=wc-settings&tab=api&section=keys' ) ), 'revoke' ) ) . '">' . _e( 'Revoke', 'woocommerce' ) . '</a>',
		);
	...
	public function column_last_access( $key ) {
		if ( ! empty( $key['last_access'] ) ) {
			...
			$date = sprintf( _e( '%1$s at %2$s', 'woocommerce' ), date_i18n( wc_date_format(), strtotime( $key['last_access'] ) ), date_i18n( wc_time_format(), strtotime( $key['last_access'] ) ) );

			return apply_filters( 'woocommerce_api_key_last_access_datetime', $date, $key['last_access'] );
		}

		return _e( 'Unknown', 'woocommerce' );
	}

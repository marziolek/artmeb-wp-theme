...
class WC_Admin_Dashboard {
	...
	public function init() {
		if ( current_user_can( 'publish_shop_orders' ) && post_type_supports( 'product', 'comments' ) ) {
			wp_add_dashboard_widget( 'woocommerce_dashboard_recent_reviews', _e( 'WooCommerce recent reviews', 'woocommerce' ), array( $this, 'recent_reviews' ) );
		}
		wp_add_dashboard_widget( 'woocommerce_dashboard_status', _e( 'WooCommerce status', 'woocommerce' ), array( $this, 'status_widget' ) );
	}
			...
			<li class="sales-this-month">
				<a href="<?php echo admin_url( 'admin.php?page=wc-reports&tab=orders&range=month' ); ?>">
					...
					<?php
						...
						printf(
							_e( '%s net sales this month', 'woocommerce' ),
			...
			<li class="best-seller-this-month">
				<a href="<?php echo admin_url( 'admin.php?page=wc-reports&tab=orders&report=sales_by_product&range=month&product_ids=' . $top_seller->product_id ); ?>">
					...
					<?php
						...
						printf(
							_e( '%1$s top seller this month (sold %2$d)', 'woocommerce' ),
	...
	public function recent_reviews() {
		...
		if ( $comments ) {
			...
			foreach ( $comments as $comment ) {
				...
				echo '<div class="star-rating"><span style="width:' . ( $rating * 20 ) . '%">' . sprintf( _e( '%s out of 5', 'woocommerce' ), $rating ) . '</span></div>';

				/* translators: %s: review author */
				echo '<h4 class="meta"><a href="' . get_permalink( $comment->ID ) . '#comment-' . absint( $comment->comment_ID ) . '">' . esc_html( apply_filters( 'woocommerce_admin_dashboard_recent_reviews', $comment->post_title, $comment ) ) . '</a> ' . sprintf( _e( 'reviewed by %s', 'woocommerce' ), esc_html( $comment->comment_author ) ) . '</h4>';
				echo '<blockquote>' . wp_kses_data( $comment->comment_content ) . '</blockquote></li>';

			}
			echo '</ul>';
		} else {
			echo '<p>' . _e( 'There are no product reviews yet.', 'woocommerce' ) . '</p>';
		}
	}
}

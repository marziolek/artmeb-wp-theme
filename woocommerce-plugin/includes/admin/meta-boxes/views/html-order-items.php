				...
				<?php
					if ( ! empty( $order_taxes ) ) :
						foreach ( $order_taxes as $tax_id => $tax_item ) :
							...
							$tax_class_name = isset( $classes_options[ $tax_class ] ) ? $classes_options[ $tax_class ] : _e( 'Tax', 'woocommerce' );
							$column_label   = ! empty( $tax_item['label'] ) ? $tax_item['label'] : _e( 'Tax', 'woocommerce' );
							$column_tip     = sprintf( esc_html_e( '%1$s (%2$s)', 'woocommerce' ), $tax_item['name'], $tax_class_name );
							?>
...
<div class="wc-order-data-row wc-order-totals-items wc-order-items-editable">
		...
		if ( $coupons ) {
			...
			<div class="wc-used-coupons">
				<ul class="wc_coupon_list"><?php
					echo '<li><strong>' . _e( 'Coupon(s)', 'woocommerce' ) . '</strong></li>';
	...
	<table class="wc-order-totals">
		<tr>
			<td class="label"><?php echo wc_help_tip( _e( 'This is the total discount. Discounts are defined per line item.', 'woocommerce' ) ); ?> <?php _e( 'Discount:', 'woocommerce' ); ?></td>
			<td width="1%"></td>
			<td class="total">
		...
		<tr>
			<td class="label"><?php echo wc_help_tip( _e( 'This is the shipping and handling total costs for the order.', 'woocommerce' ) ); ?> <?php _e( 'Shipping:', 'woocommerce' ); ?></td>
			<td width="1%"></td>
			<td class="total"><?php
...
<div class="wc-order-data-row wc-order-bulk-actions wc-order-data-row-toggle">
	<p class="add-items">
		...
		<?php else : ?>
			<span class="description"><?php echo wc_help_tip( _e( 'To edit this order change the status back to "Pending"', 'woocommerce' ) ); ?> <?php _e( 'This order is no longer editable.', 'woocommerce' ); ?></span>
		<?php endif; ?>
...
<div class="wc-order-data-row wc-order-refund-items wc-order-data-row-toggle" style="display: none;">
	<table class="wc-order-totals">
		...
		<tr>
			<td class="label"><label for="refund_reason"><?php echo wc_help_tip( _e( 'Note: the refund reason will be visible by the customer.', 'woocommerce' ) ); ?> <?php _e( 'Reason for refund (optional):', 'woocommerce' ); ?></label></td>
			<td class="total">
	...
	<div class="refund-actions">
		...
		$gateway_name             = false !== $payment_gateway ? ( ! empty( $payment_gateway->method_title ) ? $payment_gateway->method_title : $payment_gateway->get_title() ) : _e( 'Payment gateway', 'woocommerce' );
		?>
		<button type="button" class="button <?php echo $gateway_supports_refunds ? 'button-primary do-api-refund' : 'tips disabled'; ?>" <?php echo $gateway_supports_refunds ? '' : 'data-tip="' . esc_attr__( 'The payment gateway used to place this order does not support automatic refunds.', 'woocommerce' ) . '"'; ?>><?php printf( _e( 'Refund %1$s via %2$s', 'woocommerce' ), $refund_amount, $gateway_name ); ?></button>
		<button type="button" class="button button-primary do-manual-refund tips" data-tip="<?php esc_attr__( 'You will need to manually issue a refund through your payment gateway after using this.', 'woocommerce' ); ?>"><?php printf( _e( 'Refund %s manually', 'woocommerce' ), $refund_amount ); ?></button>
		<button type="button" class="button cancel-action"><?php _e( 'Cancel', 'woocommerce' ); ?></button>
		<div class="clear"></div>
	</div>
</div>
			...
			<section class="wc-backbone-modal-main" role="main">
				...
				<article>
					<form action="" method="post">
						<select class="wc-product-search" multiple="multiple" style="width: 50%;" id="add_item_id" name="add_order_items[]" data-placeholder="<?php esc_attr__( 'Search for a product&hellip;', 'woocommerce' ); ?>"></select>
					</form>
				</article>
				<footer>
					<div class="inner">
					...
					<form action="" method="post">
						...
						<?php if ( absint( $wpdb->get_var( "SELECT COUNT(tax_rate_id) FROM {$wpdb->prefix}woocommerce_tax_rates;" ) ) > 100 ) : ?>
							<p>
								...
								<input type="number" name="manual_tax_rate_id" id="manual_tax_rate_id" step="1" placeholder="<?php esc_attr__( 'Optional', 'woocommerce' ); ?>" />
							</p>
						<?php endif; ?>
					</form>
				</article>

...
<div class="woocommerce_variation wc-metabox closed">
	<h3>
		...
		<div class="handlediv" aria-label="<?php esc_attr__( 'Click to toggle', 'woocommerce' ); ?>"></div>
		<div class="tips sort" data-tip="<?php esc_attr__( 'Drag and drop, or click to set admin variation order', 'woocommerce' ); ?>"></div>
			...
			foreach ( $product_object->get_attributes( 'edit' ) as $attribute ) {
				...
				<select name="attribute_<?php echo sanitize_title( $attribute->get_name() ) . "[{$loop}]"; ?>">
					<option value=""><?php
						...
						printf( esc_html_e( 'Any %s&hellip;', 'woocommerce' ), wc_attribute_label( $attribute->get_name() ) );
						?></option>
		...
		<div class="data">
			...
			<?php
				if ( wc_product_sku_enabled() ) {
					woocommerce_wp_text_input( array(
						...
						'label'         => '<abbr title="' . esc_attr__( 'Stock Keeping Unit', 'woocommerce' ) . '">' . esc_html_e( 'SKU', 'woocommerce' ) . '</abbr>',
						'desc_tip'      => true,
			...
			<div class="variable_pricing">
				<?php
					...
					$label = sprintf(
						_e( 'Regular price (%s)', 'woocommerce' ),
					...
					woocommerce_wp_text_input( array(
						...
						'data_type'     => 'price',
						'label'         => $label . ' <a href="#" class="sale_schedule">' . _e( 'Schedule', 'woocommerce' ) . '</a><a href="#" class="cancel_sale_schedule hidden">' . _e( 'Cancel schedule', 'woocommerce' ) . '</a>',
						'wrapper_class' => 'form-row form-row-last',
					) );
					...
					echo '<div class="form-field sale_price_dates_fields hidden">
						<p class="form-row form-row-first">
							<label>' . _e( 'Sale start date', 'woocommerce' ) . '</label>
						...
						<p class="form-row form-row-last">
							<label>' . _e( 'Sale end date', 'woocommerce' ) . '</label>
					...
					if ( wc_product_dimensions_enabled() ) {
						?><p class="form-field form-row dimensions_field hide_if_variation_virtual form-row-last">
							<label for="product_length"><?php
								...
								printf(
									_e( 'Dimensions (L&times;W&times;H) (%s)', 'woocommerce' ),
									get_option( 'woocommerce_dimension_unit' )
								);
							?></label>
							<?php echo wc_help_tip( _e( 'Length x width x height in decimal form', 'woocommerce' ) ); ?>
							<span class="wrap">
								<input id="product_length" placeholder="<?php esc_attr__( 'Length', 'woocommerce' ); ?>" class="input-text wc_input_decimal" size="6" type="text" name="variable_length[<?php echo $loop; ?>]" value="<?php echo esc_attr( wc_format_localized_decimal( $variation_object->get_length( 'edit' ) ) ); ?>" />
								<input placeholder="<?php esc_attr__( 'Width', 'woocommerce' ); ?>" class="input-text wc_input_decimal" size="6" type="text" name="variable_width[<?php echo $loop; ?>]" value="<?php echo esc_attr( wc_format_localized_decimal( $variation_object->get_width( 'edit' ) ) ); ?>" />
								<input placeholder="<?php esc_attr__( 'Height', 'woocommerce' ); ?>" class="input-text wc_input_decimal last" size="6" type="text" name="variable_height[<?php echo $loop; ?>]" value="<?php echo esc_attr( wc_format_localized_decimal( $variation_object->get_height( 'edit' ) ) ); ?>" />
							</span>
						</p><?php
					}
			...
			<div class="show_if_variation_downloadable" style="display: none;">
				<div class="form-row form-row-full downloadable_files">
					...
					<table class="widefat">
						<thead>
							<div>
								<th><?php _e( 'Name', 'woocommerce' ); ?> <?php echo wc_help_tip( _e( 'This is the name of the download shown to the customer.', 'woocommerce' ) ); ?></th>
								<th colspan="2"><?php _e( 'File URL', 'woocommerce' ); ?> <?php echo wc_help_tip( _e( 'This is the URL or absolute path to the file which customers will get access to. URLs entered here should already be encoded.', 'woocommerce' ) ); ?></th>
								<th>&nbsp;</th>
							</div>
						</thead>
						<tbody>

				...
				<select name="wc_order_action">
					<option value=""><?php _e( 'Actions', 'woocommerce' ); ?></option>
						...
						if ( ! empty( $mails ) && ! empty( $available_emails ) ) { ?>
							<optgroup label="<?php esc_attr__( 'Resend order emails', 'woocommerce' ); ?>">
							<?php
							foreach ( $mails as $mail ) {
								if ( in_array( $mail->id, $available_emails ) && 'no' !== $mail->enabled ) {
									echo '<option value="send_email_' . esc_attr( $mail->id ) . '">' . sprintf( _e( 'Resend %s', 'woocommerce' ), esc_html( $mail->title ) ) . '</option>';
								}
							} ?>
							</optgroup>
							<?php
			...
			<li class="wide">
				<div id="delete-action"><?php
					...
					if ( current_user_can( 'delete_post', $post->ID ) ) {
						...
						if ( ! EMPTY_TRASH_DAYS ) {
							$delete_text = _e( 'Delete permanently', 'woocommerce' );
						} else {
							$delete_text = _e( 'Move to trash', 'woocommerce' );
						}
				...
				if ( ! empty( $mails ) ) {
					foreach ( $mails as $mail ) {
						if ( $mail->id == $email_to_send ) {
							...
							$order->add_order_note( sprintf( _e( '%s email notification manually sent.', 'woocommerce' ), $mail->title ), false, true );
						}
					}
				}

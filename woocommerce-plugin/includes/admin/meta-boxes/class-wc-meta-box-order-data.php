...
class WC_Meta_Box_Order_Data {
	...
	public static function output( $post ) {
		...
		<div class="panel-wrap woocommerce">
			<input name="post_title" type="hidden" value="<?php echo empty( $post->post_title ) ? _e( 'Order', 'woocommerce' ) : esc_attr( $post->post_title ); ?>" />
			...
			<div id="order_data" class="panel">
				...
				<h2><?php
					...
					printf(
						esc_html_e( '%1$s #%2$s details', 'woocommerce' ),
				...
				<p class="order_number"><?php
					...
					if ( $payment_method ) {
						...
						if ( $order->get_date_paid() ) {
							...
							printf( ' ' . _e( 'on %1$s @ %2$s', 'woocommerce' ), wc_format_datetime( $order->get_date_paid() ), wc_format_datetime( $order->get_date_paid(), get_option( 'time_format' ) ) );
						}
				...
				<div class="order_data_column_container">
					<div class="order_data_column">
						...
						<p class="form-field form-field-wide"><label for="order_date"><?php _e( 'Order date:', 'woocommerce' ) ?></label>
							<input type="text" class="date-picker" name="order_date" id="order_date" maxlength="10" value="<?php echo date_i18n( 'Y-m-d', strtotime( $post->post_date ) ); ?>" pattern="<?php echo esc_attr( apply_filters( 'woocommerce_date_input_html_pattern', '[0-9]{4}-(0[1-9]|1[012])-(0[1-9]|1[0-9]|2[0-9]|3[01])' ) ); ?>" />@&lrm;<input type="number" class="hour" placeholder="<?php esc_attr__( 'h', 'woocommerce' ) ?>" name="order_date_hour" id="order_date_hour" min="0" max="23" step="1" value="<?php echo date_i18n( 'H', strtotime( $post->post_date ) ); ?>" pattern="([01]?[0-9]{1}|2[0-3]{1})" />:<input type="number" class="minute" placeholder="<?php esc_attr__( 'm', 'woocommerce' ) ?>" name="order_date_minute" id="order_date_minute" min="0" max="59" step="1" value="<?php echo date_i18n( 'i', strtotime( $post->post_date ) ); ?>" pattern="[0-5]{1}[0-9]{1}" />&lrm;
						</p>
						...
						<p class="form-field form-field-wide wc-order-status"><label for="order_status"><?php _e( 'Order status:', 'woocommerce' ) ?> <?php
							if ( $order->needs_payment() ) {
								printf( '<a href="%s">%s</a>',
									...
									_e( 'Customer payment page &rarr;', 'woocommerce' )
								);
							}
						?></label>
						...
						<p class="form-field form-field-wide wc-customer-user">
							...
							<label for="customer_user"><?php _e( 'Customer:', 'woocommerce' ) ?> <?php
								if ( $order->get_user_id( 'edit' ) ) {
									...
									printf( '<a href="%s">%s</a>',
										...
										_e( 'View other orders &rarr;', 'woocommerce' )
									);
								}
							?></label>
							<?php
							...
							?>
							<select class="wc-customer-search" id="customer_user" name="customer_user" data-placeholder="<?php esc_attr__( 'Guest', 'woocommerce' ); ?>" data-allow_clear="true">
						...
						<?php
							...
							echo '<div class="address">';
								...
								if ( $order->get_formatted_billing_address() ) {
									echo '<p><strong>' . _e( 'Address:', 'woocommerce' ) . '</strong>' . wp_kses( $order->get_formatted_billing_address(), array( 'br' => array() ) ) . '</p>';
								} else {
									echo '<p class="none_set"><strong>' . _e( 'Address:', 'woocommerce' ) . '</strong> ' . _e( 'No billing address set.', 'woocommerce' ) . '</p>';
								}
								...
								<select name="_payment_method" id="_payment_method" class="first">
									...
									<?php
										...
										if ( ! $found_method && ! empty( $payment_method ) ) {
											echo '<option value="' . esc_attr( $payment_method ) . '" selected="selected">' . _e( 'Other', 'woocommerce' ) . '</option>';
										} else {
											echo '<option value="other">' . _e( 'Other', 'woocommerce' ) . '</option>';
										}
									?>
								</select>
							</p>
						...
						<?php
							...
							echo '<div class="address">';
								...
								if ( $order->get_formatted_shipping_address() ) {
									echo '<p><strong>' . _e( 'Address:', 'woocommerce' ) . '</strong>' . wp_kses( $order->get_formatted_shipping_address(), array( 'br' => array() ) ) . '</p>';
								} else {
									echo '<p class="none_set"><strong>' . _e( 'Address:', 'woocommerce' ) . '</strong> ' . _e( 'No shipping address set.', 'woocommerce' ) . '</p>';
								}
								...
								if ( apply_filters( 'woocommerce_enable_order_notes_field', 'yes' == get_option( 'woocommerce_enable_order_comments', 'yes' ) ) && $post->post_excerpt ) {
									echo '<p><strong>' . _e( 'Customer provided note:', 'woocommerce' ) . '</strong> ' . nl2br( esc_html( $post->post_excerpt ) ) . '</p>';
								}
							...
							if ( apply_filters( 'woocommerce_enable_order_notes_field', 'yes' == get_option( 'woocommerce_enable_order_comments', 'yes' ) ) ) {
								...
								<textarea rows="1" cols="40" name="excerpt" tabindex="6" id="excerpt" placeholder="<?php esc_attr__( 'Customer notes about the order', 'woocommerce' ); ?>"><?php echo wp_kses_post( $post->post_excerpt ); ?></textarea></p>
								<?php
							}

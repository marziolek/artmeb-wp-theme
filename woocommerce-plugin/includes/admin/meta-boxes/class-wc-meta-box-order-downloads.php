...
class WC_Meta_Box_Order_Downloads {
	...
	public static function output( $post ) {
		...
		<div class="order_download_permissions wc-metaboxes-wrapper">
			...
			<div class="wc-metaboxes"><?php
				...
				if ( $download_permissions && sizeof( $download_permissions ) > 0 ) {
					foreach ( $download_permissions as $download ) {
						...
						$file_count = isset( $file['name'] ) ? $file['name'] : sprintf( _e( 'File %d', 'woocommerce' ), $file_counter );
			...
			<div class="toolbar">
				<p class="buttons">
					<select id="grant_access_id" class="wc-product-search" name="grant_access_id[]" multiple="multiple" style="width: 400px;" data-placeholder="<?php esc_attr__( 'Search for a downloadable product&hellip;', 'woocommerce' ); ?>" data-action="woocommerce_json_search_downloadable_products_and_variations"></select>

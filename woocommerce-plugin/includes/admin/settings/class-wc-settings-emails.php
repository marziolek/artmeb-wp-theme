...
class WC_Settings_Emails extends WC_Settings_Page {
	...
	public function __construct() {
		$this->id    = 'email';
		$this->label = _e( 'Emails', 'woocommerce' );
	...
	public function get_settings() {
		$settings = apply_filters( 'woocommerce_email_settings', array(
			...
			array( 'title' => __( 'Email template', 'woocommerce' ), 'type' => 'title', 'desc' => sprintf( _e( 'This section lets you customize the WooCommerce emails. <a href="%s" target="_blank">Click here to preview your email template</a>.', 'woocommerce' ), wp_nonce_url( admin_url( '?preview_woocommerce_mail=true' ), 'preview-mail' ) ), 'id' => 'email_template_options' ),
			...
			array(
				...
				'default'     => sprintf( _e( '%s - Powered by WooCommerce', 'woocommerce' ), get_bloginfo( 'name', 'display' ) ),
				'autoload'    => false,
				'desc_tip'    => true,
			),
			...
			array(
				...
				'desc'     => sprintf( _e( 'The base color for WooCommerce email templates. Default %s.', 'woocommerce' ), '<code>#96588a</code>' ),
			...
			array(
				...
				/* translators: %s: default color */
				'desc'     => sprintf( _e( 'The background color for WooCommerce email templates. Default %s.', 'woocommerce' ), '<code>#f7f7f7</code>' ),
			...
			array(
				...
				/* translators: %s: default color */
				'desc'     => sprintf( _e( 'The main body background color. Default %s.', 'woocommerce' ), '<code>#ffffff</code>' ),
			...
			array(
				...
				/* translators: %s: default color */
				'desc'     => sprintf( _e( 'The main body text color. Default %s.', 'woocommerce' ), '<code>#3c3c3c</code>' ),
						...
						foreach ( $email_templates as $email_key => $email ) {
							...
							foreach ( $columns as $key => $column ) {
								...
								switch ( $key ) {
									...
									case 'recipient' :
										echo '<td class="wc-email-settings-table-' . esc_attr( $key ) . '">
											' . esc_html( $email->is_customer_email() ? _e( 'Customer', 'woocommerce' ) : $email->get_recipient() ) . '
										</td>';
									break;
									case 'status' :
										echo '<td class="wc-email-settings-table-' . esc_attr( $key ) . '">';
										...
										if ( $email->is_manual() ) {
											echo '<span class="status-manual tips" data-tip="' . esc_attr__( 'Manually sent', 'woocommerce' ) . '">' . esc_html_e( 'Manual', 'woocommerce' ) . '</span>';
										} elseif ( $email->is_enabled() ) {
											echo '<span class="status-enabled tips" data-tip="' . esc_attr__( 'Enabled', 'woocommerce' ) . '">' . esc_html_e( 'Yes', 'woocommerce' ) . '</span>';
										} else {
									...
									case 'actions' :
										echo '<td class="wc-email-settings-table-' . esc_attr( $key ) . '">
											<a class="button alignright tips" data-tip="' . esc_attr__( 'Configure', 'woocommerce' ) . '" href="' . admin_url( 'admin.php?page=wc-settings&tab=email&section=' . strtolower( $email_key ) ) . '">' . esc_html_e( 'Configure', 'woocommerce' ) . '</a>
										</td>';
									break;
									default :

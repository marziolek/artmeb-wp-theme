		...
		<tbody>
			<tr valign="top">
				<th scope="row" class="titledesc">
					...
					echo wc_help_tip( sprintf( _e( 'Friendly name for identifying this webhook, defaults to Webhook created on %s.', 'woocommerce' ), strftime( _x( '%b %d, %Y @ %I:%M %p', 'Webhook created on date parsed by strftime', 'woocommerce' ) ) ) );
			...
			<tr valign="top">
				<th scope="row" class="titledesc">
					...
					<?php wc_help_tip( _e( 'The options are &quot;Active&quot; (delivers payload), &quot;Paused&quot; (does not deliver), or &quot;Disabled&quot; (does not deliver due delivery failures).', 'woocommerce' ) ); ?>
				</th>
				<td class="forminp">
			...
			<tr valign="top">
				<th scope="row" class="titledesc">
					...
					<?php echo wc_help_tip( _e( 'Select when the webhook will fire.', 'woocommerce' ) ); ?>
				</th>
				<td class="forminp">
			...
			<tr valign="top" id="webhook-action-event-wrap">
				<th scope="row" class="titledesc">
					...
					<?php echo wc_help_tip( _e( 'Enter the action that will trigger this webhook.', 'woocommerce' ) ); ?>
				</th>
				<td class="forminp">
			...
			<tr valign="top" id="webhook-custom-topic-wrap">
				<th scope="row" class="titledesc">
					...
					<?php echo wc_help_tip( _e( 'Enter the custom topic that will trigger this webhook.', 'woocommerce' ) ); ?>
				</th>
				<td class="forminp">
			...
			<tr valign="top">
				<th scope="row" class="titledesc">
					...
					<?php echo wc_help_tip( _e( 'URL where the webhook payload is delivered.', 'woocommerce' ) ); ?>
				</th>
				<td class="forminp">
			...
			<tr valign="top">
				<th scope="row" class="titledesc">
					...
					<?php echo wc_help_tip( _e( 'The secret key is used to generate a hash of the delivered webhook and provided in the request headers.', 'woocommerce' ) ); ?>
				</th>
				<td class="forminp">
			...
			<tr valign="top">
				<th scope="row" class="titledesc">
					...
					<?php echo wc_help_tip( _e( 'REST API version used in the webhook deliveries.', 'woocommerce' ) ); ?>
				</th>
				<td class="forminp">
		...
		<tbody>
			<?php if ( '0000-00-00 00:00:00' != $webhook->post_data->post_modified_gmt ) : ?>
				<?php if ( '0000-00-00 00:00:00' == $webhook->post_data->post_date_gmt ) : ?>
					<tr valign="top">
						...
						<td class="forminp">
							<?php echo date_i18n( _e( 'M j, Y @ G:i', 'woocommerce' ), strtotime( $webhook->post_data->post_modified_gmt ) ); ?>
						</td>
					</tr>
				<?php else : ?>
				<tr valign="top">
						...
						<td class="forminp">
							<?php echo date_i18n( _e( 'M j, Y @ G:i', 'woocommerce' ), strtotime( $webhook->post_data->post_date_gmt ) ); ?>
						</td>
					</tr>
				<tr valign="top">
						<th scope="row" class="titledesc">
			...
			<tr valign="top">
				<td colspan="2" scope="row" style="padding-left: 0;">
					<p class="submit">
						<input type="submit" class="button button-primary button-large" name="save" id="publish" accesskey="p" value="<?php esc_attr__( 'Save webhook', 'woocommerce' ); ?>" />
						<?php if ( current_user_can( 'delete_post', $webhook->id ) ) : ?>
							<a style="color: #a00; text-decoration: none; margin-left: 10px;" href="<?php echo esc_url( get_delete_post_link( $webhook->id ) ); ?>"><?php echo ( ! EMPTY_TRASH_DAYS ) ? _e( 'Delete permanently', 'woocommerce' ) : _e( 'Move to trash', 'woocommerce' ); ?></a>
						<?php endif; ?>
					</p>
				</td>
			</tr>

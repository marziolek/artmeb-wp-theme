...
<h2>
	...
	<?php echo wc_help_tip( _e( 'Shipping classes can be used to group products of similar type and can be used by some Shipping Methods (such as "Flat rate shipping") to provide different rates to different classes of product.', 'woocommerce' ) ); ?>
</h2>
	...
	<tfoot>
		<tr>
			<td colspan="<?php echo absint( sizeof( $shipping_class_columns ) ); ?>">
				<input type="submit" name="save" class="button button-primary wc-shipping-class-save" value="<?php esc_attr__( 'Save shipping classes', 'woocommerce' ); ?>" disabled />
			...
			foreach ( $shipping_class_columns as $class => $heading ) {
				...
				switch ( $class ) {
					case 'wc-shipping-class-name' :
						...
						<div class="edit">
							<input type="text" name="name[{{ data.term_id }}]" data-attribute="name" value="{{ data.name }}" placeholder="<?php esc_attr__( 'Shipping class name', 'woocommerce' ); ?>" />
							<div class="row-actions">
					...
					case 'wc-shipping-class-slug' :
						...
						<div class="edit"><input type="text" name="slug[{{ data.term_id }}]" data-attribute="slug" value="{{ data.slug }}" placeholder="<?php esc_attr__( 'Slug', 'woocommerce' ); ?>" /></div>
						<?php
					break;
					case 'wc-shipping-class-description' :
						?>
						<div class="view">{{ data.description }}</div>
						<div class="edit"><input type="text" name="description[{{ data.term_id }}]" data-attribute="description" value="{{ data.description }}" placeholder="<?php esc_attr__( 'Description for your reference', 'woocommerce' ); ?>" /></div>
						<?php
					break;

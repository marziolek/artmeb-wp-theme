...
class WC_Settings_Tax extends WC_Settings_Page {
	...
	public function get_sections() {
		...
		foreach ( $tax_classes as $class ) {
			$sections[ sanitize_title( $class ) ] = sprintf( _e( '%s rates', 'woocommerce' ), $class );
		}
	...
	public function output_tax_rates() {
		...
		wp_localize_script( 'wc-settings-tax', 'htmlSettingsTaxLocalizeScript', array(
			...
			'strings'       => array(
				...
				'csv_data_cols' => array(
					_e( 'Country code', 'woocommerce' ),
					_e( 'State code', 'woocommerce' ),
					_e( 'Postcode / ZIP', 'woocommerce' ),
					_e( 'City', 'woocommerce' ),
					_e( 'Rate %', 'woocommerce' ),
					_e( 'Tax name', 'woocommerce' ),
					_e( 'Priority', 'woocommerce' ),

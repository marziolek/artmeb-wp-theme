...
class WC_Settings_Products extends WC_Settings_Page {
	...
	public function __construct() {
		...
		$this->id    = 'products';
		$this->label = _e( 'Products', 'woocommerce' );
		...
		if ( 'display' == $current_section ) {
			...
			$settings = apply_filters( 'woocommerce_product_settings', array(
				...
				array(
					...
					'desc'     => '<br/>' . sprintf( _e( 'The base page can also be used in your <a href="%s">product permalinks</a>.', 'woocommerce' ), admin_url( 'options-permalink.php' ) ),
				...
				array(
					...
					'type' 	=> 'title',
					'desc' 	=> sprintf( _e( 'These settings affect the display and dimensions of images in your catalog - the display on the front-end will still be affected by CSS styles. After changing these settings you may need to <a target="_blank" href="%s">regenerate your thumbnails</a>.', 'woocommerce' ), 'https://wordpress.org/plugins/regenerate-thumbnails/' ),
					'id' 	=> 'image_options',
				),

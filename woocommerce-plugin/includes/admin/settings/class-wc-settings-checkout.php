		...
		if ( '' === $current_section ) {
			$settings = apply_filters( 'woocommerce_payment_gateways_settings', array(
				...
				array(
					...
					'desc_tip'        => sprintf( _e( 'Force SSL (HTTPS) on the checkout pages (<a href="%s" target="_blank">an SSL Certificate is required</a>).', 'woocommerce' ), 'https://docs.woocommerce.com/document/ssl-and-https/#section-3' ),
				),
				...
				array(
					...
					'desc'     => sprintf( _e( 'Page contents: [%s]', 'woocommerce' ), apply_filters( 'woocommerce_cart_shortcode_tag', 'woocommerce_cart' ) ),
				...
				array(
					...
					'desc'     => sprintf( _e( 'Page contents: [%s]', 'woocommerce' ), apply_filters( 'woocommerce_checkout_shortcode_tag', 'woocommerce_checkout' ) ),
						...
						foreach ( WC()->payment_gateways->payment_gateways() as $gateway ) {
							...
							foreach ( $columns as $key => $column ) {
								...
								switch ( $key ) {
									...
									case 'name' :
										$method_title = $gateway->get_title() ? $gateway->get_title() : _e( '(no title)', 'woocommerce' );
										echo '<td class="name">
									...
									case 'status' :
										...
										echo ( 'yes' === $gateway->enabled ) ? '<span class="status-enabled tips" data-tip="' . esc_attr__( 'Yes', 'woocommerce' ) . '">' . esc_html_e( 'Yes', 'woocommerce' ) . '</span>' : '-';
										echo '</td>';
										break;

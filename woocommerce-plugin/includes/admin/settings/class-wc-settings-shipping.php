...
class WC_Settings_Shipping extends WC_Settings_Page {
	...
	public function __construct() {
		$this->id    = 'shipping';
		$this->label = _e( 'Shipping', 'woocommerce' );
	...
	protected function zone_methods_screen( $zone_id ) {
		...
		if ( ! $zone ) {
			wp_die( _e( 'Zone does not exist!', 'woocommerce' ) );
		}
	...
	protected function instance_settings_screen( $instance_id ) {
		...
		if ( ! $shipping_method ) {
			wp_die( _e( 'Invalid shipping method!', 'woocommerce' ) );
		}
		if ( ! $zone ) {
			wp_die( _e( 'Zone does not exist!', 'woocommerce' ) );
		}
		if ( ! $shipping_method->has_settings() ) {
			wp_die( _e( 'This shipping method does not have any settings to configure.', 'woocommerce' ) );
		}

		if ( ! empty( $_POST['save'] ) ) {

			if ( empty( $_REQUEST['_wpnonce'] ) || ! wp_verify_nonce( $_REQUEST['_wpnonce'], 'woocommerce-settings' ) ) {
				echo '<div class="updated error"><p>' . _e( 'Edit failed. Please try again.', 'woocommerce' ) . '</p></div>';
			}

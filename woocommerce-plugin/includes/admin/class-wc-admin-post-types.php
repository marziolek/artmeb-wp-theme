...
class WC_Admin_Post_Types {
	...
	public function post_updated_messages( $messages ) {
		...
		$messages['product'] = array(
			...
			1 => sprintf( _e( 'Product updated. <a href="%s">View Product</a>', 'woocommerce' ), esc_url( get_permalink( $post_ID ) ) ),
			...
			5 => isset( $_GET['revision'] ) ? sprintf( _e( 'Product restored to revision from %s', 'woocommerce' ), wp_post_revision_title( (int) $_GET['revision'], false ) ) : false,
			/* translators: %s: product url */
			6 => sprintf( _e( 'Product published. <a href="%s">View Product</a>', 'woocommerce' ), esc_url( get_permalink( $post_ID ) ) ),
			7 => __( 'Product saved.', 'woocommerce' ),
			/* translators: %s: product url */
			8 => sprintf( _e( 'Product submitted. <a target="_blank" href="%s">Preview product</a>', 'woocommerce' ), esc_url( add_query_arg( 'preview', 'true', get_permalink( $post_ID ) ) ) ),
			/* translators: 1: date 2: product url */
			9 => sprintf( _e( 'Product scheduled for: <strong>%1$s</strong>. <a target="_blank" href="%2$s">Preview product</a>', 'woocommerce' ),
			  date_i18n( _e( 'M j, Y @ G:i', 'woocommerce' ), strtotime( $post->post_date ) ), esc_url( get_permalink( $post_ID ) ) ),
			/* translators: %s: product url */
			10 => sprintf( _e( 'Product draft updated. <a target="_blank" href="%s">Preview product</a>', 'woocommerce' ), esc_url( add_query_arg( 'preview', 'true', get_permalink( $post_ID ) ) ) ),
		);
		...
		$messages['shop_order'] = array(
			...
			/* translators: %s: revision title */
			5 => isset( $_GET['revision'] ) ? sprintf( _e( 'Order restored to revision from %s', 'woocommerce' ), wp_post_revision_title( (int) $_GET['revision'], false ) ) : false,
			6 => __( 'Order updated.', 'woocommerce' ),
			7 => __( 'Order saved.', 'woocommerce' ),
			8 => __( 'Order submitted.', 'woocommerce' ),
			/* translators: %s: date */
			9 => sprintf( _e( 'Order scheduled for: <strong>%1$s</strong>.', 'woocommerce' ),
			  date_i18n( _e( 'M j, Y @ G:i', 'woocommerce' ), strtotime( $post->post_date ) ) ),
		...
		$messages['shop_coupon'] = array(
			...
			/* translators: %s: revision title */
			5 => isset( $_GET['revision'] ) ? sprintf( _e( 'Coupon restored to revision from %s', 'woocommerce' ), wp_post_revision_title( (int) $_GET['revision'], false ) ) : false,
			6 => __( 'Coupon updated.', 'woocommerce' ),
			7 => __( 'Coupon saved.', 'woocommerce' ),
			8 => __( 'Coupon submitted.', 'woocommerce' ),
			/* translators: %s: date */
			9 => sprintf( _e( 'Coupon scheduled for: <strong>%1$s</strong>.', 'woocommerce' ),
			  date_i18n( _e( 'M j, Y @ G:i', 'woocommerce' ), strtotime( $post->post_date ) ) ),
	...
	public function product_columns( $existing_columns ) {
		...
		$columns['thumb'] = '<span class="wc-image tips" data-tip="' . esc_attr__( 'Image', 'woocommerce' ) . '">' . _e( 'Image', 'woocommerce' ) . '</span>';
		$columns['name']  = _e( 'Name', 'woocommerce' );

		if ( wc_product_sku_enabled() ) {
			$columns['sku'] = _e( 'SKU', 'woocommerce' );
		}

		if ( 'yes' === get_option( 'woocommerce_manage_stock' ) ) {
			$columns['is_in_stock'] = _e( 'Stock', 'woocommerce' );
		}

		$columns['price']        = _e( 'Price', 'woocommerce' );
		$columns['product_cat']  = _e( 'Categories', 'woocommerce' );
		$columns['product_tag']  = _e( 'Tags', 'woocommerce' );
		$columns['featured']     = '<span class="wc-featured parent-tips" data-tip="' . esc_attr__( 'Featured', 'woocommerce' ) . '">' . _e( 'Featured', 'woocommerce' ) . '</span>';
		$columns['product_type'] = '<span class="wc-type parent-tips" data-tip="' . esc_attr__( 'Type', 'woocommerce' ) . '">' . _e( 'Type', 'woocommerce' ) . '</span>';
		$columns['date']         = _e( 'Date', 'woocommerce' );
	...
	public function shop_coupon_columns( $existing_columns ) {
		...
		$columns['coupon_code'] = _e( 'Code', 'woocommerce' );
		$columns['type']        = _e( 'Coupon type', 'woocommerce' );
		$columns['amount']      = _e( 'Coupon amount', 'woocommerce' );
		$columns['description'] = _e( 'Description', 'woocommerce' );
		$columns['products']    = _e( 'Product IDs', 'woocommerce' );
		$columns['usage']       = _e( 'Usage / Limit', 'woocommerce' );
		$columns['expiry_date'] = _e( 'Expiry date', 'woocommerce' );
	...
	public function shop_order_columns( $existing_columns ) {
		...
		$columns['order_title']      = _e( 'Order', 'woocommerce' );
		$columns['billing_address']  = _e( 'Billing', 'woocommerce' );
		$columns['shipping_address'] = _e( 'Ship to', 'woocommerce' );
		...
		$columns['order_date']       = _e( 'Date', 'woocommerce' );
		$columns['order_total']      = _e( 'Total', 'woocommerce' );
		$columns['order_actions']    = _e( 'Actions', 'woocommerce' );
	...
	public function render_product_columns( $column ) {
		...
		switch ( $column ) {
			...
			case 'featured' :
				...
				echo '<a href="' . esc_url( $url ) . '" aria-label="' . _e( 'Toggle featured', 'woocommerce' ) . '">';
				if ( $the_product->is_featured() ) {
					echo '<span class="wc-featured tips" data-tip="' . esc_attr__( 'Yes', 'woocommerce' ) . '">' . _e( 'Yes', 'woocommerce' ) . '</span>';
				} else {
					echo '<span class="wc-featured not-featured tips" data-tip="' . esc_attr__( 'No', 'woocommerce' ) . '">' . _e( 'No', 'woocommerce' ) . '</span>';
				}
				echo '</a>';
				break;
			case 'is_in_stock' :
				...
				if ( $the_product->is_in_stock() ) {
					$stock_html = '<mark class="instock">' . _e( 'In stock', 'woocommerce' ) . '</mark>';
				} else {
					$stock_html = '<mark class="outofstock">' . _e( 'Out of stock', 'woocommerce' ) . '</mark>';
				}
		...
		switch ( $column ) {
			...
			case 'customer_message' :
				if ( $the_order->get_customer_note() ) {
					echo '<span class="note-on tips" data-tip="' . wc_sanitize_tooltip( $the_order->get_customer_note() ) . '">' . _e( 'Yes', 'woocommerce' ) . '</span>';
				} else {
					echo '<span class="na">&ndash;</span>';
				}
			...
			case 'billing_address' :
				...
				if ( $the_order->get_billing_phone() ) {
					echo '<small class="meta">' . _e( 'Phone:', 'woocommerce' ) . ' ' . esc_html( $the_order->get_billing_phone() ) . '</small>';
				}
			...
			case 'shipping_address' :
				...
				if ( $the_order->get_shipping_method() ) {
					echo '<small class="meta">' . _e( 'Via', 'woocommerce' ) . ' ' . esc_html( $the_order->get_shipping_method() ) . '</small>';
				}
			...
			case 'order_notes' :
				...
				if ( $post->comment_count ) {
					...
					if ( isset( $latest_note->comment_content ) && 1 == $post->comment_count ) {
						echo '<span class="note-on tips" data-tip="' . wc_sanitize_tooltip( $latest_note->comment_content ) . '">' . _e( 'Yes', 'woocommerce' ) . '</span>';
					} elseif ( isset( $latest_note->comment_content ) ) {
						/* translators: %d: notes count */
						echo '<span class="note-on tips" data-tip="' . wc_sanitize_tooltip( $latest_note->comment_content . '<br/><small style="display:block">' . sprintf( _n( 'plus %d other note', 'plus %d other notes', ( $post->comment_count - 1 ), 'woocommerce' ), $post->comment_count - 1 ) . '</small>' ) . '">' . _e( 'Yes', 'woocommerce' ) . '</span>';
					} else {
						/* translators: %d: notes count */
						echo '<span class="note-on tips" data-tip="' . wc_sanitize_tooltip( sprintf( _n( '%d note', '%d notes', $post->comment_count, 'woocommerce' ), $post->comment_count ) ) . '">' . _e( 'Yes', 'woocommerce' ) . '</span>';
					}
				} else {
					echo '<span class="na">&ndash;</span>';
				}
			...
			case 'order_total' :
				...
				if ( $the_order->get_payment_method_title() ) {
					echo '<small class="meta">' . _e( 'Via', 'woocommerce' ) . ' ' . esc_html( $the_order->get_payment_method_title() ) . '</small>';
				}
			break;
			case 'order_title' :
				...
				echo '<button type="button" class="toggle-row"><span class="screen-reader-text">' . _e( 'Show more details', 'woocommerce' ) . '</span></button>';
	...
	public function product_views( $views ) {
		...
		if ( current_user_can( 'edit_others_pages' ) ) {
			...
			$views['byorder'] = '<a href="' . esc_url( $query_string ) . '" class="' . esc_attr( $class ) . '">' . _e( 'Sorting', 'woocommerce' ) . '</a>';
		}
	...
	public function shop_order_bulk_actions( $actions ) {
		...
		$actions['mark_processing'] = _e( 'Mark processing', 'woocommerce' );
		$actions['mark_on-hold']    = _e( 'Mark on-hold', 'woocommerce' );
		$actions['mark_completed']  = _e( 'Mark complete', 'woocommerce' );
	...
	public function handle_shop_order_bulk_actions( $redirect_to, $action, $ids ) {
		...
		foreach ( $ids as $id ) {
			...
			$order->update_status( $new_status, _e( 'Order status changed by bulk edit:', 'woocommerce' ), true );
	...
	public function product_filters() {
		...
		$output .= '<option value="">' . _e( 'Filter by product type', 'woocommerce' ) . '</option>';
		...
		foreach ( $terms as $term ) {
			...
			switch ( $term->name ) {
				case 'grouped' :
					$output .= _e( 'Grouped product', 'woocommerce' );
					break;
				case 'external' :
					$output .= _e( 'External/Affiliate product', 'woocommerce' );
					break;
				case 'variable' :
					$output .= _e( 'Variable product', 'woocommerce' );
					break;
				case 'simple' :
					$output .= _e( 'Simple product', 'woocommerce' );
					break;
				default :
					// Assuming that we have other types in future
			...
			if ( 'simple' == $term->name ) {
				...
				$output .= '> ' . ( is_rtl() ? '&larr;' : '&rarr;' ) . ' ' . _e( 'Downloadable', 'woocommerce' ) . '</option>';
				...
				$output .= '> ' . ( is_rtl() ? '&larr;' : '&rarr;' ) . ' ' . _e( 'Virtual', 'woocommerce' ) . '</option>';
			}
		}
	...
	public function shop_order_filters() {
		...
		if ( ! empty( $_GET['_customer_user'] ) ) {
			...
			$user_string = sprintf(
				esc_html_e( '%1$s (#%2$s &ndash; %3$s)', 'woocommerce' ),
				$user->display_name,
				absint( $user->ID ),
				$user->user_email
			);
		...
		?>
		<select class="wc-customer-search" name="_customer_user" data-placeholder="<?php esc_attr__( 'Search for a customer&hellip;', 'woocommerce' ); ?>" data-allow_clear="true">
	...
	public function enter_title_here( $text, $post ) {
		switch ( $post->post_type ) {
			case 'product' :
				$text = _e( 'Product name', 'woocommerce' );
			break;
			case 'shop_coupon' :
				$text = _e( 'Coupon code', 'woocommerce' );
			break;
		}
	...
	public function edit_form_after_title( $post ) {
		if ( 'shop_coupon' === $post->post_type ) {
			?>
			<textarea id="woocommerce-coupon-description" name="excerpt" cols="5" rows="2" placeholder="<?php esc_attr__( 'Description (optional)', 'woocommerce' ); ?>"><?php echo $post->post_excerpt; // This is already escaped in core ?></textarea>
			<?php
		}
	}
	...
	public function product_data_visibility() {
		...
		<div class="misc-pub-section" id="catalog-visibility">
			...
			<div id="catalog-visibility-select" class="hide-if-js">
				...
				<?php
					echo '<p>' . _e( 'This setting determines which shop pages products will be listed on.', 'woocommerce' ) . '</p>';

					foreach ( $visibility_options as $name => $label ) {
						echo '<input type="radio" name="_visibility" id="_visibility_' . esc_attr( $name ) . '" value="' . esc_attr( $name ) . '" ' . checked( $current_visibility, $name, false ) . ' data-label="' . esc_attr( $label ) . '" /> <label for="_visibility_' . esc_attr( $name ) . '" class="selectit">' . esc_html( $label ) . '</label><br />';
					}

					echo '<br /><input type="checkbox" name="_featured" id="_featured" ' . checked( $current_featured, 'yes', false ) . ' /> <label for="_featured">' . _e( 'This is a featured product', 'woocommerce' ) . '</label><br />';
				?>
				<p>
	...
	public function show_cpt_archive_notice( $post ) {
		...
		if ( $post && absint( $shop_page_id ) === absint( $post->ID ) ) {
			...
			<div class="notice notice-info">
				<p><?php printf( _e( 'This is the WooCommerce shop page. The shop page is a special archive that lists your products. <a href="%s">You can read more about this here</a>.', 'woocommerce' ), 'https://docs.woocommerce.com/document/woocommerce-pages/#section-4' ); ?></p>
			</div>
			<?php
		}
	}

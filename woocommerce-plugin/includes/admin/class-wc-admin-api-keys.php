...
class WC_Admin_API_Keys {
	...
	private static function table_list_output() {
		...
		echo '<h2>' . _e( 'Keys/Apps', 'woocommerce' ) . ' <a href="' . esc_url( admin_url( 'admin.php?page=wc-settings&tab=api&section=keys&create-key=1' ) ) . '" class="add-new-h2">' . _e( 'Add key', 'woocommerce' ) . '</a></h2>';
		...
		if ( absint( $count ) && $count > 0 ) {
			...
			$keys_table_list->search_box( _e( 'Search key', 'woocommerce' ), 'key' );
	...
	public static function notices() {
		if ( isset( $_GET['revoked'] ) && 1 == $_GET['revoked'] ) {
			WC_Admin_Settings::add_message( _e( 'API key revoked successfully.', 'woocommerce' ) );
		}
	}

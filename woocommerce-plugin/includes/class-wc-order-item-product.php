	...
	*/
	...
	public function set_tax_class( $value ) {
		if ( $value && ! in_array( $value, WC_Tax::get_tax_class_slugs() ) ) {
			$this->error( 'order_item_product_invalid_tax_class', _e( 'Invalid tax class', 'woocommerce' ) );
		}
	...
	public function set_product_id( $value ) {
		if ( $value > 0 && 'product' !== get_post_type( absint( $value ) ) ) {
			$this->error( 'order_item_product_invalid_product_id', _e( 'Invalid product ID', 'woocommerce' ) );
		}
	...
	public function set_variation_id( $value ) {
		if ( $value > 0 && 'product_variation' !== get_post_type( $value ) ) {
			$this->error( 'order_item_product_invalid_variation_id', _e( 'Invalid variation ID', 'woocommerce' ) );
		}
	...
	public function set_product( $product ) {
		if ( ! is_a( $product, 'WC_Product' ) ) {
			$this->error( 'order_item_product_invalid_product', _e( 'Invalid product', 'woocommerce' ) );
		}
	...
	public function set_backorder_meta() {
		...
		if ( $product && $product->backorders_require_notification() && $product->is_on_backorder( $this->get_quantity() ) ) {
			$this->add_meta_data( apply_filters( 'woocommerce_backordered_item_meta_name', _e( 'Backordered', 'woocommerce' ) ), $this->get_quantity() - max( 0, $product->get_stock_quantity() ), true );
		}
	}

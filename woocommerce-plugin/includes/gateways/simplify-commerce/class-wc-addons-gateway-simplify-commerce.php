...
class WC_Addons_Gateway_Simplify_Commerce extends WC_Gateway_Simplify_Commerce {
	...
	protected function process_subscription( $order, $cart_token = '' ) {
		try {
			...
			if ( is_object( $customer ) && '' != $customer->id ) {
				...
				$error_msg = _e( 'Error creating user in Simplify Commerce.', 'woocommerce' );
		...
		if ( WC_Pre_Orders_Order::order_requires_payment_tokenization( $order->get_id() ) ) {
			...
			try {
				if ( $order->get_total() * 100 < 50 ) {
					$error_msg = _e( 'Sorry, the minimum allowed order total is 0.50 to use this payment method.', 'woocommerce' );
	...
	public function process_subscription_payment( $order, $amount = 0 ) {
		...
		if ( ! $customer_id ) {
			return new WP_Error( 'simplify_error', _e( 'Customer not found.', 'woocommerce' ) );
		}
	...
	public function scheduled_subscription_payment( $amount_to_charge, $renewal_order ) {
		...
		if ( is_wp_error( $result ) ) {
			$renewal_order->update_status( 'failed', sprintf( _e( 'Simplify Transaction Failed (%s)', 'woocommerce' ), $result->get_error_message() ) );
		}
	}
	...
	public function process_pre_order_release_payment( $order ) {
		...
		try {
			...
			$pre_order_name = sprintf(
				_e( '%1$s - Pre-order for "%2$s" (Order #%3$s)', 'woocommerce' ),
			...
			if ( 'APPROVED' == $payment->paymentStatus ) {
				...
				return new WP_Error( 'simplify_payment_declined', _e( 'Payment was declined - the customer need to try another card.', 'woocommerce' ) );
			}
		} catch ( Exception $e ) {
			$order_note = sprintf( _e( 'Simplify Transaction Failed (%s)', 'woocommerce' ), $e->getMessage() );

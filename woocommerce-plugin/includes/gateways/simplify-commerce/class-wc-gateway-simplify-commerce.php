...
class WC_Gateway_Simplify_Commerce extends WC_Payment_Gateway_CC {
	...
	public function __construct() {
		...
		$this->method_title       = _e( 'Simplify Commerce', 'woocommerce' );
		$this->method_description = _e( 'Take payments via Simplify Commerce - uses simplify.js to create card tokens and the Simplify Commerce SDK. Requires SSL when sandbox is disabled.', 'woocommerce' );
		$this->new_method_label   = _e( 'Use a new card', 'woocommerce' );
	...
	public function checks() {
		...
		if ( version_compare( phpversion(), '5.3', '<' ) ) {
			...
			// PHP Version
			echo '<div class="error"><p>' . sprintf( _e( 'Simplify Commerce Error: Simplify commerce requires PHP 5.3 and above. You are using version %s.', 'woocommerce' ), phpversion() ) . '</p></div>';

		} elseif ( ! $this->public_key || ! $this->private_key ) {

			// Check required fields
			echo '<div class="error"><p>' . _e( 'Simplify Commerce Error: Please enter your public and private keys', 'woocommerce' ) . '</p></div>';

		} elseif ( 'standard' == $this->mode && ! wc_checkout_is_https() ) {

			// Show message when using standard mode and no SSL on the checkout page
			echo '<div class="error"><p>' . sprintf( _e( 'Simplify Commerce is enabled, but the <a href="%s">force SSL option</a> is disabled; your checkout may not be secure! Please enable SSL and ensure your server has a valid SSL certificate - Simplify Commerce will only work in sandbox mode.', 'woocommerce' ), admin_url( 'admin.php?page=wc-settings&tab=checkout' ) ) . '</p></div>';
	...
	public function payment_fields() {
		...
		if ( 'yes' == $this->sandbox ) {
			$description .= ' ' . sprintf( _e( 'TEST MODE ENABLED. Use a test card: %s', 'woocommerce' ), '<a href="https://www.simplify.com/commerce/docs/tutorial/index#testing">https://www.simplify.com/commerce/docs/tutorial/index#testing</a>' );
		}
	...
	public function add_payment_method() {
		if ( empty( $_POST['simplify_token'] ) ) {
			wc_add_notice( _e( 'There was a problem adding this card.', 'woocommerce' ), 'error' );
			return;
		}
	...
	protected function process_standard_payments( $order, $cart_token = '', $customer_token = '' ) {
		try {
			...
			if ( empty( $cart_token ) && empty( $customer_token ) ) {
				$error_msg = _e( 'Please make sure your card details have been entered correctly and that your browser supports JavaScript.', 'woocommerce' );

				if ( 'yes' == $this->sandbox ) {
					$error_msg .= ' ' . _e( 'Developers: Please make sure that you are including jQuery and there are no JavaScript errors on the page.', 'woocommerce' );
				}
	...
	public function do_payment( $order, $amount = 0, $token = array() ) {
		if ( $amount * 100 < 50 ) {
			return new WP_Error( 'simplify_error', _e( 'Sorry, the minimum allowed order total is 0.50 to use this payment method.', 'woocommerce' ) );
		}
		...
		try {
			...
			$data = array(
				...
				'description'         => sprintf( _e( '%1$s - Order #%2$s', 'woocommerce' ), esc_html( get_bloginfo( 'name', 'display' ) ), $order->get_order_number() ),
			...
			$order->add_order_note( sprintf( _e( 'Simplify payment error: %s.', 'woocommerce' ), $error_message ) );
		...
		if ( 'APPROVED' == $payment->paymentStatus ) {
			...
			// Add order note
			$order->add_order_note( sprintf( _e( 'Simplify payment approved (ID: %1$s, Auth Code: %2$s)', 'woocommerce' ), $payment->id, $payment->authCode ) );

			return true;
		} else {
			$order->add_order_note( _e( 'Simplify payment declined', 'woocommerce' ) );

			return new WP_Error( 'simplify_payment_declined', _e( 'Payment was declined - please try another card.', 'woocommerce' ) );
		}
	}
	...
	public function process_payment( $order_id ) {
		...
		if ( isset( $_POST['wc-simplify_commerce-payment-token'] ) && 'new' !== $_POST['wc-simplify_commerce-payment-token'] ) {
			...
			if ( $token->get_user_id() !== get_current_user_id() ) {
				wc_add_notice( _e( 'Please make sure your card details have been entered correctly and that your browser supports JavaScript.', 'woocommerce' ), 'error' );
				return;
			}
	...
	protected function get_hosted_payments_args( $order ) {
		$args = apply_filters( 'woocommerce_simplify_commerce_hosted_args', array(
			...
			'description'     => sprintf( _e( 'Order #%s', 'woocommerce' ), $order->get_order_number() ),
			'receipt'         => 'false',
	...
	public function receipt_page( $order_id ) {
		...
		echo '<p>' . _e( 'Thank you for your order, please click the button below to pay with credit card using Simplify Commerce by MasterCard.', 'woocommerce' ) . '</p>';
		...
		echo '<script type="text/javascript" src="https://www.simplify.com/commerce/simplify.pay.js"></script>
			<button class="button alt" id="simplify-payment-button" ' . implode( ' ', $button_args ) . '>' . _e( 'Pay now', 'woocommerce' ) . '</button> <a class="button cancel" href="' . esc_url( $order->get_cancel_order_url() ) . '">' . _e( 'Cancel order &amp; restore cart', 'woocommerce' ) . '</a>
			';
	}
	...
	public function return_handler() {
		...
		if ( isset( $_REQUEST['reference'] ) && isset( $_REQUEST['paymentId'] ) && isset( $_REQUEST['signature'] ) ) {
			...
			if ( hash_equals( $signature, $_REQUEST['signature'] ) ) {
				...
				if ( ! $order_complete ) {
					$order->update_status( 'failed', _e( 'Payment was declined by Simplify Commerce.', 'woocommerce' ) );
				}
	...
	public function process_order_status( $order, $payment_id, $status, $auth_code ) {
		if ( 'APPROVED' == $status ) {
			...
			// Add order note
			$order->add_order_note( sprintf( _e( 'Simplify payment approved (ID: %1$s, Auth Code: %2$s)', 'woocommerce' ), $payment_id, $auth_code ) );
	...
	public function process_refund( $order_id, $amount = null, $reason = '' ) {
		try {
			...
			if ( 'APPROVED' == $refund->paymentStatus ) {
				...
				throw new Simplify_ApiException( _e( 'Refund was declined.', 'woocommerce' ) );
			}
		} catch ( Simplify_ApiException $e ) {
			if ( $e instanceof Simplify_BadRequestException && $e->hasFieldErrors() && $e->getFieldErrors() ) {
				foreach ( $e->getFieldErrors() as $error ) {

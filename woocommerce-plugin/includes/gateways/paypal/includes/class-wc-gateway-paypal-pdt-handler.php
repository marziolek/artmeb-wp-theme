		...
		if ( $transaction_result ) {
	 		if ( 'completed' === $status ) {
				if ( $order->get_total() != $amount ) {
					...
					$this->payment_on_hold( $order, sprintf( _e( 'Validation error: PayPal amounts do not match (amt %s).', 'woocommerce' ), $amount ) );
				} else {
					$this->payment_complete( $order, $transaction,  _e( 'PDT payment completed', 'woocommerce' ) );
				...
				if ( 'authorization' === $transaction_result['pending_reason'] ) {
					...
					$this->payment_on_hold( $order, sprintf( _e( 'Payment pending (%s).', 'woocommerce' ), $transaction_result['pending_reason'] ) );
				}
			}
		}
	}

...
class WC_Gateway_Paypal extends WC_Payment_Gateway {
	...
	public function __construct() {
		...
		$this->order_button_text  = _e( 'Proceed to PayPal', 'woocommerce' );
		$this->method_title       = _e( 'PayPal', 'woocommerce' );
		$this->method_description = sprintf( _e( 'PayPal Standard sends customers to PayPal to enter their payment information. PayPal IPN requires fsockopen/cURL support to update order statuses after payment. Check the <a href="%s">system status</a> page for more details.', 'woocommerce' ), admin_url( 'admin.php?page=wc-status' ) );
		$this->supports           = array(
			'products',
			'refunds',
		);
	...
	public function process_refund( $order_id, $amount = null, $reason = '' ) {
		...
		if ( ! $this->can_refund_order( $order ) ) {
			...
			return new WP_Error( 'error', _e( 'Refund failed: No transaction ID', 'woocommerce' ) );
		}
		...
		switch ( strtolower( $result->ACK ) ) {
			...
			case 'successwithwarning':
				$order->add_order_note( sprintf( _e( 'Refunded %1$s - Refund ID: %2$s', 'woocommerce' ), $result->GROSSREFUNDAMT, $result->REFUNDTRANSACTIONID ) );
				return true;
			break;
		}
	...
	public function capture_payment( $order_id ) {
		...
		if ( 'paypal' === $order->get_payment_method() && 'pending' === get_post_meta( $order->get_id(), '_paypal_status', true ) && $order->get_transaction_id() ) {
			...
			if ( is_wp_error( $result ) ) {
				...
				$order->add_order_note( sprintf( _e( 'Payment could not captured: %s', 'woocommerce' ), $result->get_error_message() ) );
				return;
			}
			...
			if ( ! empty( $result->PAYMENTSTATUS ) ) {
				switch ( $result->PAYMENTSTATUS ) {
					case 'Completed' :
						$order->add_order_note( sprintf( _e( 'Payment of %1$s was captured - Auth ID: %2$s, Transaction ID: %3$s', 'woocommerce' ), $result->AMT, $result->AUTHORIZATIONID, $result->TRANSACTIONID ) );
					...
					break;
					default :
						$order->add_order_note( sprintf( _e( 'Payment could not captured - Auth ID: %1$s, Status: %2$s', 'woocommerce' ), $result->AUTHORIZATIONID, $result->PAYMENTSTATUS ) );
					break;
				}
			}
		}

...
function wc_rest_upload_image_from_url( $image_url ) {
	...
	if ( ! $parsed_url || ! is_array( $parsed_url ) ) {
		return new WP_Error( 'woocommerce_rest_invalid_image_url', sprintf( _e( 'Invalid URL %s.', 'woocommerce' ), $image_url ), array( 'status' => 400 ) );
	}
	...
	if ( is_wp_error( $response ) ) {
		return new WP_Error( 'woocommerce_rest_invalid_remote_image_url', sprintf( _e( 'Error getting remote image %s.', 'woocommerce' ), $image_url ) . ' ' . sprintf( _e( 'Error: %s.', 'woocommerce' ), $response->get_error_message() ), array( 'status' => 400 ) );
	} elseif ( 200 !== wp_remote_retrieve_response_code( $response ) ) {
		return new WP_Error( 'woocommerce_rest_invalid_remote_image_url', sprintf( _e( 'Error getting remote image %s.', 'woocommerce' ), $image_url ), array( 'status' => 400 ) );
	}
	...
	if ( ! $wp_filetype['type'] ) {
		...
		if ( ! $wp_filetype['type'] ) {
			return new WP_Error( 'woocommerce_rest_invalid_image_type', _e( 'Invalid image type.', 'woocommerce' ), array( 'status' => 400 ) );
		}
	}
	...
	if ( 0 == $filesize ) {
		...
		return new WP_Error( 'woocommerce_rest_image_upload_file_error', _e( 'Zero size file downloaded.', 'woocommerce' ), array( 'status' => 400 ) );
	}
...
function wc_rest_validate_reports_request_arg( $value, $request, $param ) {
	...
	if ( 'string' === $args['type'] && ! is_string( $value ) ) {
		return new WP_Error( 'woocommerce_rest_invalid_param', sprintf( _e( '%1$s is not of type %2$s', 'woocommerce' ), $param, 'string' ) );
	}
	...
	if ( 'date' === $args['format'] ) {
		...
		if ( ! preg_match( $regex, $value, $matches ) ) {
			return new WP_Error( 'woocommerce_rest_invalid_date', _e( 'The date you provided is invalid.', 'woocommerce' ) );
		}
	}

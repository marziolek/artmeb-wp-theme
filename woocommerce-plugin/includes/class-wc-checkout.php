...
class WC_Checkout {
	...
	public function __clone() {
		wc_doing_it_wrong( __FUNCTION__, _e( 'Cheatin&#8217; huh?', 'woocommerce' ), '2.1' );
	}
			...
			foreach ( $fieldset as $key => $field ) {
				...
				switch ( $fieldset_key ) {
					case 'shipping' :
						...
						$field_label = sprintf( _e( 'Shipping %s', 'woocommerce' ), $field_label );
					break;
					case 'billing' :
						/* translators: %s: field name */
						$field_label = sprintf( _e( 'Billing %s', 'woocommerce' ), $field_label );
					break;
				}
				...
				if ( in_array( 'postcode', $format ) ) {
					...
					if ( '' !== $data[ $key ] && ! WC_Validation::is_postcode( $data[ $key ], $country ) ) {
						$errors->add( 'validation', _e( 'Please enter a valid postcode / ZIP.', 'woocommerce' ) );
					}
				}
				...
				if ( in_array( 'phone', $format ) ) {
					...
					if ( '' !== $data[ $key ] && ! WC_Validation::is_phone( $data[ $key ] ) ) {
						...
						$errors->add( 'validation', sprintf( _e( '%s is not a valid phone number.', 'woocommerce' ), '<strong>' . esc_html( $field_label ) . '</strong>' ) );
					}
				}
				...
				if ( in_array( 'email', $format ) && '' !== $data[ $key ] ) {
					...
					if ( ! is_email( $data[ $key ] ) ) {
						...
						$errors->add( 'validation', sprintf( _e( '%s is not a valid email address.', 'woocommerce' ), '<strong>' . $field_label . '</strong>' ) );
						continue;
					}
				}
				...
				if ( '' !== $data[ $key ] && in_array( 'state', $format ) ) {
					...
					if ( ! empty( $valid_states ) && is_array( $valid_states ) && sizeof( $valid_states ) > 0 ) {
						...
						if ( ! in_array( $data[ $key ], $valid_state_values ) ) {
							...
							$errors->add( 'validation', sprintf( _e( '%1$s is not valid. Please enter one of the following: %2$s', 'woocommerce' ), '<strong>' . $field_label . '</strong>', implode( ', ', $valid_states ) ) );
						}
					}
				}
				...
				if ( $required && '' === $data[ $key ] ) {
					/* translators: %s: field name */
					$errors->add( 'required-field', apply_filters( 'woocommerce_checkout_required_field_notice', sprintf( _e( '%s is a required field.', 'woocommerce' ), '<strong>' . $field_label . '</strong>' ), $field_label ) );
				}
			}
		}
	}
	...
	protected function validate_checkout( &$data, &$errors ) {
		...
		if ( empty( $data['woocommerce_checkout_update_totals'] ) && empty( $data['terms'] ) && apply_filters( 'woocommerce_checkout_show_terms', wc_get_page_id( 'terms' ) > 0 ) ) {
			$errors->add( 'terms', _e( 'You must accept our Terms &amp; Conditions.', 'woocommerce' ) );
		}
		...
		if ( WC()->cart->needs_shipping() ) {
			...
			if ( empty( $shipping_country ) ) {
				$errors->add( 'shipping', _e( 'Please enter an address to continue.', 'woocommerce' ) );
			} elseif ( ! in_array( WC()->customer->get_shipping_country(), array_keys( WC()->countries->get_shipping_countries() ) ) ) {
				$errors->add( 'shipping', sprintf( _e( 'Unfortunately <strong>we do not ship %s</strong>. Please enter an alternative shipping address.', 'woocommerce' ), WC()->countries->shipping_to_prefix() . ' ' . WC()->customer->get_shipping_country() ) );
			} else {
				$chosen_shipping_methods = WC()->session->get( 'chosen_shipping_methods' );

				foreach ( WC()->shipping->get_packages() as $i => $package ) {
					if ( ! isset( $chosen_shipping_methods[ $i ], $package['rates'][ $chosen_shipping_methods[ $i ] ] ) ) {
						$errors->add( 'shipping', _e( 'No shipping method has been selected. Please double check your address, or contact us if you need any help.', 'woocommerce' ) );
					}
				}
			}
		}
		...
		if ( WC()->cart->needs_payment() ) {
			...
			if ( ! isset( $available_gateways[ $data['payment_method'] ] ) ) {
				$errors->add( 'payment', _e( 'Invalid payment method.', 'woocommerce' ) );
			} else {
	...
	public function process_checkout() {
		try {
			if ( empty( $_POST['_wpnonce'] ) || ! wp_verify_nonce( $_POST['_wpnonce'], 'woocommerce-process_checkout' ) ) {
				...
				throw new Exception( _e( 'We were unable to process your order, please try again.', 'woocommerce' ) );
			}
			...
			if ( WC()->cart->is_empty() ) {
				throw new Exception( sprintf( _e( 'Sorry, your session has expired. <a href="%s" class="wc-backward">Return to shop</a>', 'woocommerce' ), esc_url( wc_get_page_permalink( 'shop' ) ) ) );
			}

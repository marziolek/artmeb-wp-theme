	...
	private static function order_pay( $order_id ) {
		...
		if ( isset( $_GET['pay_for_order'], $_GET['key'] ) && $order_id ) {
			...
			if ( ! $order || $order->get_id() !== $order_id || $order->get_order_key() !== $order_key ) {
				wc_add_notice( _e( 'Sorry, this order is invalid and cannot be paid for.', 'woocommerce' ), 'error' );
			...
			// Logged out customer does not have permission to pay for this order.
				...
				echo '<div class="woocommerce-info">' . _e( 'Please log in to your account below to continue to the payment form.', 'woocommerce' ) . '</div>';
			...
			// Logged in customer trying to pay for someone else's order.
				...
				wc_add_notice( _e( 'This order cannot be paid for. Please contact us if you need assistance.', 'woocommerce' ), 'error' );

			// Order does not need to be paid.
			} elseif ( ! $order->needs_payment() ) {
				wc_add_notice( sprintf( _e( 'This order&rsquo;s status is &ldquo;%s&rdquo;&mdash;it cannot be paid for. Please contact us if you need assistance.', 'woocommerce' ), wc_get_order_status_name( $order->get_status() ) ), 'error' );
			...
			// Show payment form.
				...
				wc_get_template( 'checkout/form-pay.php', array(
					...
					'order_button_text'  => apply_filters( 'woocommerce_pay_order_button_text', _e( 'Pay for order', 'woocommerce' ) ),
				) );
			}
		} elseif ( $order_id ) {
	...
	private static function checkout() {
		...
		if ( empty( $_POST ) && wc_notice_count( 'error' ) > 0 ) {
			...
			if ( wc_notice_count( 'error' ) == 0 && $non_js_checkout ) {
				wc_add_notice( _e( 'The order totals have been updated. Please confirm your order by pressing the "Place order" button at the bottom of the page.', 'woocommerce' ) );
			}

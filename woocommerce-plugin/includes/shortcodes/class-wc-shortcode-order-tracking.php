...
class WC_Shortcode_Order_Tracking {
	...
	public static function output( $atts ) {
		...
		if ( ! empty( $_REQUEST['orderid'] ) && isset( $_POST['_wpnonce'] ) && wp_verify_nonce( $_POST['_wpnonce'], 'woocommerce-order_tracking' ) ) {
			...
			if ( ! $order_id ) {
				wc_add_notice( _e( 'Please enter a valid order ID', 'woocommerce' ), 'error' );
			} elseif ( ! $order_email ) {
				wc_add_notice( _e( 'Please enter a valid order email', 'woocommerce' ), 'error' );
			} else {
				...
				if ( $order && $order->get_id() && $order_email ) {
					...
					wc_add_notice( _e( 'Sorry, we could not find that order ID in our database.', 'woocommerce' ), 'error' );
				}
			}
		}

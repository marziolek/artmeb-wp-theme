...
class WC_Shortcode_My_Account {
	...
	public static function output( $atts ) {
		...
		if ( ! is_user_logged_in() ) {
			...
			if ( ! empty( $_GET['password-reset'] ) ) {
				wc_add_notice( _e( 'Your password has been reset successfully.', 'woocommerce' ) );
			}
			...
			if ( isset( $wp->query_vars['customer-logout'] ) ) {
				wc_add_notice( sprintf( _e( 'Are you sure you want to log out? <a href="%s">Confirm and log out</a>', 'woocommerce' ), wc_logout_url() ) );
			}
	...
	public static function view_order( $order_id ) {
		...
		if ( ! current_user_can( 'view_order', $order_id ) ) {
			echo '<div class="woocommerce-error">' . _e( 'Invalid order.', 'woocommerce' ) . ' <a href="' . wc_get_page_permalink( 'myaccount' ) . '" class="wc-forward">' . _e( 'My account', 'woocommerce' ) . '</a>' . '</div>';
			return;
		}
	...
	public static function retrieve_password() {
		...
		if ( empty( $login ) ) {
			...
			wc_add_notice( _e( 'Enter a username or email address.', 'woocommerce' ), 'error' );
			return false;
		...
		if ( ! $user_data ) {
			wc_add_notice( _e( 'Invalid username or email.', 'woocommerce' ), 'error' );
			return false;
		}
		...
		if ( ! $allow ) {
			...
			wc_add_notice( _e( 'Password reset is not allowed for this user', 'woocommerce' ), 'error' );
			return false;

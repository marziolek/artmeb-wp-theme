...
class WC_Emails {
	...
	public function low_stock( $product ) {
		...
		$subject = sprintf( '[%s] %s', $this->get_blogname(), _e( 'Product low in stock', 'woocommerce' ) );
		/* translators: 1: product name 2: items in stock */
		$message = sprintf(
			_e( '%1$s is low in stock. There are %2$d left.', 'woocommerce' ),
	...
	public function no_stock( $product ) {
		...
		$subject = sprintf( '[%s] %s', $this->get_blogname(), _e( 'Product out of stock', 'woocommerce' ) );
		/* translators: %s: product name */
		$message = sprintf( _e( '%s is out of stock.', 'woocommerce' ), html_entity_decode( strip_tags( $product->get_formatted_name() ), ENT_QUOTES, get_bloginfo( 'charset' ) ) );
	...
	public function backorder( $args ) {
		...
		$subject = sprintf( '[%s] %s', $this->get_blogname(), _e( 'Product backorder', 'woocommerce' ) );
		$message = sprintf( _e( '%1$s units of %2$s have been backordered in order #%3$s.', 'woocommerce' ), $quantity, html_entity_decode( strip_tags( $product->get_formatted_name() ), ENT_QUOTES, get_bloginfo( 'charset' ) ), $order->get_order_number() );
